 ...
 * pKa calculation
 * <p>
 * Adam C. Lee, Jing-yu Yu, and Gordon M. Crippen, pKa Prediction of Monoprotic
 * Small Molecules the SMARTS Way
 * <p>
...
public class PKASmartsDescriptor implements IMolecularDescriptor {
	protected static Logger logger = Logger.getLogger(PKASmartsDescriptor.class
			.getName());
	...
	public DescriptorValue calculate(IAtomContainer arg0) {
		try {
			...
			return new DescriptorValue(getSpecification(), getParameterNames(),
					getParameters(),
					new VerboseDescriptorResult<String, DoubleResult>(
							new DoubleResult(node.getPka()), trace.toString()),
					title);

		} catch (Exception x) {
			return new DescriptorValue(getSpecification(), getParameterNames(),
					getParameters(), null, title, x);
		}
	}
	...
	protected PKANode traverse(IAtomContainer ac, PKANode node,
			ArrayList<String> trace) throws SMARTSException {
		...
		if (node.isTerminal()) {
			...
			for (boolean result : results) {
				...
				if (next.find(ac) == next.isPresent()) {
					trace.add(Integer.toString(next.getId())
							+ ((next.isPresent()) ? 'Y' : 'N'));
					if (next.isTerminal()) {
						return next;
					} else {
	...
	protected PKANode initialize() throws IOException {
		InputStream stream = PKASmartsDescriptor.class.getClassLoader()
				.getResourceAsStream("ambit2/descriptors/pkatree.txt");
		...
		while ((line = reader.readLine()) != null) {
			...
			if (record >= 9) {
				...
				while (st.hasMoreTokens()) {
					...
					case 1: {
						node.setParent(Integer.parseInt(s));
						break;
					}
					case 2: {
						node.setTerminal(Integer.parseInt(s) == 0);
						break;
					}
					case 3:
						break;
					...
					case 6: {
						node.setPka(Double.parseDouble(s));
						break;
					}
					case 7: {
						node.setPka_range(Double.parseDouble(s));
						break;
					}
					}
					column++;
