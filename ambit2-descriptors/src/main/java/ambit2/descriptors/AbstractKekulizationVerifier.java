...
public abstract class AbstractKekulizationVerifier implements IMolecularDescriptor {
	protected static final String version="1.4.9";
	protected static final String OK = "OK";
	protected enum names_index {
		NumSingleBondOriginal,
		NumDoubleBondOriginal,
		NumAromaticBondOriginal,
	...
	protected static String[] names = {
		...
		names_index.time_ms.toString(),
		names_index.status.toString()
	};
	
	protected abstract IAtomContainer transform2Kekule(IAtomContainer mol)  throws CDKException;
	...
	public DescriptorValue calculate(IAtomContainer mol) {
		...
		try {
	        ...
	        return new DescriptorValue(getSpecification(), getParameterNames(), 
	                getParameters(), new ArrayResult(results),getDescriptorNames()); 			
		} catch (Exception x) {
			for (IBond bond:mol.bonds()) {
				if (bond.getFlag(CDKConstants.ISAROMATIC)) 
	...
	public DescriptorSpecification getSpecification() {
        return new DescriptorSpecification(
            	String.format("http://www.opentox.org/algorithmTypes#%s",getTyperClass().toLowerCase()),
    		    this.getClass().getName(),
    		    String.format("$Id: %s.java,v 1.0 2010/08/05 8:12:00 Nina Jeliazkova Exp $",getTyperClass()),
