...
public class RDFAlgorithmIterator extends RDFObjectIterator<Algorithm> {
	...
	protected Template createTemplate() {
		return new Template(String.format("%s%s/{%s}",baseReference==null?"":baseReference,
				AlgorithmResource.algorithm,
				AlgorithmResource.algorithmKey));
	}
	...
	protected void parseObjectURI(RDFNode uri, Algorithm record) {
		...
		try {
			...
			record.setId(vars.get(AlgorithmResource.algorithmKey).toString()); } 
		catch (Exception x) { };
