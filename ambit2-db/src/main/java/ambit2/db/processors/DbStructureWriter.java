...
public class DbStructureWriter extends AbstractRepositoryWriter<IStructureRecord, IStructureRecord> {
    ...
    @Override
    public IStructureRecord write(IStructureRecord structure) throws SQLException, AmbitException,
    ...
    @Override
    public IStructureRecord update(IStructureRecord structure) throws SQLException, OperationNotSupportedException,
	    AmbitException {
	writeStructure(structure, true);
	return structure;
    }

    public List<IStructureRecord> writeStructure(IStructureRecord structure, boolean update) throws SQLException,
	    AmbitException, OperationNotSupportedException {
	...
	if (structure.getIdstructure() <= 0) {
		...
		if (update) {
			...
			updateStructure = new UpdateStructure();
	...
	if (getDataset().getID() <= 0) {
		...
		if (record > 1)
		    getDataset().setID(-1); // smth is wrong
	    } catch (Exception x) {
		getDataset().setID(-1);
	    }
	}
	...
	try {
	    ...
	    logger.log(Level.SEVERE, String.format(
		    "Error %s adding structure /compound/%d/conformer/%d to dataset [%d] %s", x.getMessage(),
