...
public class RetrieveDatasetsTest extends RetrieveTest<ISourceDataset> {
	...
	protected void verifyRows(AmbitRows<ISourceDataset> rows) throws Exception {
		...
		while (rows.next()) {
			...
			ITable table = 	c.createQueryTable("EXPECTED","SELECT id_srcdataset,name,user_name,idreference,title,url,licenseURI FROM src_dataset join catalog_references using(idreference) where name='"+ dataset.getName() +"'");		
