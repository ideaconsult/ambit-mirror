...
public class RetrieveFieldNamesByAliasTest extends RetrieveTest<Property> {
	...
	public void testGetFieldID() {
		Assert.assertEquals("idproperty",
				((RetrieveFieldNamesByAlias) query).getFieldID());
	}

	@Test
	public void testGetValueID() {
		Assert.assertEquals("name",
				((RetrieveFieldNamesByAlias) query).getValueID());
	}

	@Test
	public void testGetFieldType() throws Exception {
		Assert.assertEquals(String.class,
				((RetrieveFieldNamesByAlias) query).getFieldType());
	}

	@Test
	public void testGetValueType() {
		Assert.assertEquals(String.class,
				((RetrieveFieldNamesByAlias) query).getValueType());
	}
	...
	protected void verifyRows(IQueryRetrieval<Property> query, ResultSet rows)
		...
		while (rows.next()) {
			...
			ITable table = c
					.createQueryTable(
							"EXPECTED",
							"select idproperty,name,units,title,url,idreference,comments,null,islocal,type,rdf_type,predicate,object from properties join catalog_references using(idreference) left join property_annotation using(idproperty) where name='"
									+ p.getName()
									+ "' and title='"
