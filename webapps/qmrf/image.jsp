...
<!--
Params:
idstructure  - ambit_qmrf.structure.idstructure
idsubstance  - ambit_qmrf.structure.idsubstance
...
<c:when test="${!empty param.idstructure}">
	<c:set var="sql" value="${sql} where idstructure=${param.idstructure}"/>
</c:when>
<c:when test="${!empty param.idsubstance}">
	<c:set var="sql" value="${sql} where idsubstance=${param.idsubstance}"/>
</c:when>
<c:otherwise>
	<c:set var="sql" value=""/>
</c:otherwise>
