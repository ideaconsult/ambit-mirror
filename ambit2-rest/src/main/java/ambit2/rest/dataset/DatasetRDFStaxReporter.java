...
public class DatasetRDFStaxReporter<Q extends IQueryRetrieval<IStructureRecord>>
	...
	public DatasetRDFStaxReporter(String prefix, Request request,
			Template template, Profile groupedProperties) {
		super(prefix, request);
		recordWriter.setGroupProperties(groupedProperties);
		recordWriter.setTemplate(template == null ? new Template(null)
				: template);
		initProcessors();
	...
	@Override
	protected DatasetRDFWriter createRecordWriter(Request request,
			ResourceDoc doc) {
		return new DatasetRDFWriter(
				new CompoundURIReporter<IQueryRetrieval<IStructureRecord>>(
						compoundInDatasetPrefix, request, false),
	...
	protected void initProcessors() {
		...
		getProcessors().clear();
		if ((recordWriter.getGroupProperties() != null)
				&& (recordWriter.getGroupProperties().size() > 0))
			getProcessors().add(
					new ProcessorStructureRetrieval(
							new RetrieveGroupedValuesByAlias(recordWriter
									.getGroupProperties())) {
						/**
			     * 
			     */
		...
		if ((recordWriter.getTemplate() != null)
				&& (recordWriter.getTemplate().size() > 0))
			getProcessors().add(
					new ProcessorStructureRetrieval(new RetrieveProfileValues(
							SearchMode.idproperty, recordWriter.getTemplate(),
							true)) {
						/**
			     * 
			     */
