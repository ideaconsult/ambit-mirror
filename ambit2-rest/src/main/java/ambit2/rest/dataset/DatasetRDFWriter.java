...
public class DatasetRDFWriter extends
		AbstractStaxRDFWriter<IStructureRecord, IStructureRecord> {
	...
	public DatasetRDFWriter(Reference baseReference, ResourceDoc doc) {
		this(new ConformerURIReporter<IQueryRetrieval<IStructureRecord>>(
				baseReference), new PropertyURIReporter(baseReference),
				new ConformerURIReporter<IQueryRetrieval<IStructureRecord>>(
						baseReference));
	}

	public DatasetRDFWriter(Request request, ResourceDoc doc) {
		this(
				new CompoundURIReporter<IQueryRetrieval<IStructureRecord>>(
	...
	public IStructureRecord process(IStructureRecord item)
		...
		if (datasetIndividual == null)
			try {
				getOutput().writeStartElement(OT.NS, "Dataset");
				datasetIndividual = createDatasetURI(item.getDatasetID());
				if (datasetIndividual != null)
					getOutput().writeAttribute(RDF.getURI(), "about",
							datasetIndividual);
				else {
					if (uriReporter.getResourceRef() == null)
						datasetIndividual = "-";
					else
						datasetIndividual = String.format("%s:%s", uriReporter
								.getResourceRef().getScheme(), uriReporter
								.getResourceRef().getHierarchicalPart());
				}
				if (getLicenseURI() != null) {
					getOutput().writeStartElement("dcterms", "license",
							DCTerms.NS); // value
					getOutput().writeAttribute("rdf", RDF.getURI(), "resource",
							getLicenseURI());
		...
		try {
			...
			String uri = item.getType().equals(STRUC_TYPE.NA) ? compoundReporter
			...
			if (header != null)
				for (ambit2.base.data.Property p : header) {
					...
					try {
						...
						if (p.getId() > 0)
							getOutput().writeAttribute(RDF.getURI(),
									"resource", featureURI);
						else {
							/**
							 * <ot:Feature>
							 * <dc:creator>http://ambit.sourceforge.
							 * net</dc:creator>
							 * <ot:hasSource>Default</ot:hasSource> <owl:sameAs
							 * rdf:resource=
							 * "http://www.opentox.org/api/1.1#IUPACName"/>
							 * <ot:units></ot:units>
							 * <dc:title>http://www.opentox
							 * .org/api/1.1#IUPACName</dc:title> </ot:Feature>
							 */
					...
					// if (p.isNominal())
					// feature.addProperty(OTProperty.acceptValue.createProperty(getJenaModel()),
					// value.toString());

				}
			// getOutput().flush();
			/*
			 * Commented - not sure will have effect on the error, but whould be
			 * safe otherwise Caused by: javax.xml.stream.XMLStreamException:
			 * ClientAbortException: java.io.IOException at
			 * com.sun.xml.stream.writers
			 * .XMLStreamWriterImpl.flush(XMLStreamWriterImpl.java:404) at
			 * com.sun.xml.txw2.output.DelegatingXMLStreamWriter.flush(
			 * DelegatingXMLStreamWriter.java:56) at
			 * ambit2.rest.dataset.DatasetRDFWriter
			 * .process(DatasetRDFWriter.java:231)
			 */
			return item;
		} catch (Exception x) {
	...
	protected String createDatasetURI(int datasetID) {
		...
		if (uriReporter.getResourceRef() == null) {
			return null;
		} else if (uriReporter.getResourceRef().getQueryAsForm()
				.getFirstValue(OpenTox.params.feature_uris.toString()) != null) {
			return null;
		} else {
			if (datasetID <= 0)
				return String.format("%s:%s", uriReporter.getResourceRef()
						.getScheme(), uriReporter.getResourceRef()
						.getHierarchicalPart());
			else {
				String datasetPrefix = String.format("/%s/%d",
						OpenTox.URI.dataset.name(), datasetID);
				return String.format("%s%s", uriReporter.getBaseReference(),
						datasetPrefix);
			}
		}
	}
	...
	public void header(javax.xml.stream.XMLStreamWriter writer) {
		...
		try {
			...
			writeAnnotationPropertyTriple(writer,
			...
			for (ambit2.base.data.Property p : header)
				try {
					...
					if (p.isNominal()) {
						...
						getOutput()
								.writeAttribute(RDF.getURI(), "resource",
										"http://www.opentox.org/api/1.1#NominalFeature");
						getOutput().writeEndElement();

						if (p.getAllowedValues() != null)
							for (Comparable value : p.getAllowedValues()) {
								getOutput().writeStartElement(OT.NS,
										OTProperty.acceptValue.toString());
								getOutput().writeCharacters(value.toString()); // TODO
																				// make
																				// use
																				// of
																				// data
					...
					boolean numeric = (p.getClazz() == Number.class)
							|| (p.getClazz() == Double.class)
							|| (p.getClazz() == Float.class)
							|| (p.getClazz() == Integer.class)
					...
					if (p.getClazz().equals(Dictionary.class)) {
						...
						getOutput().writeAttribute(RDF.getURI(), "resource",
								"http://www.opentox.org/api/1.1#TupleFeature");
						getOutput().writeEndElement();
					}
					...
					if (uri == null)
						uri = Property.guessLabel(p.getName());
					if ((uri != null) && (uri.indexOf("http://") < 0)
							&& (uri.indexOf("https://") < 0)) {
						uri = String.format("%s%s", OT.NS,
	...
	protected String writeHasSource(Property item) throws Exception {
		...
		if ((uri.indexOf("http://") < 0) && (uri.indexOf("https://") < 0)) {
			...
			if (_type.Algorithm.equals(item.getReference().getType())) {
				...
				namespace = OT.NS;
				uri = String.format("%s/algorithm/%s",
						uriReporter.getBaseReference(), Reference.encode(uri));
			} else if (_type.Model.equals(item.getReference().getType())) {
				otclass = OTClass.Model.toString();
				namespace = OT.NS;
				uri = String.format("%s/model/%s",
						uriReporter.getBaseReference(), Reference.encode(uri));
			} else if (_type.Dataset.equals(item.getReference().getType())) {
				// otclass = "Class";
				// namespace = OWL.getURI();
				// otclass = OTClass.Dataset;
				// should be as above , but this seems to confuse everybody's
				// else parsers ...
				// uri =
				// String.format("%s/dataset/%s",uriReporter.getBaseReference(),Reference.encode(uri));
			}
		...
		if (otclass == null) {
			...
			getOutput().writeCharacters(uri); // TODO this is wrong, should be a
												// resource

		} else {
			getOutput().writeStartElement(namespace, otclass); // algorithm or
																// model
			getOutput().writeAttribute(RDF.getURI(), "about", uri);
			getOutput().writeEndElement();
		}
