...
/**
 * Convenience class for processing OpenTox resources (e.g. {@link OTAlgorithm} and {@link OTModel}
 * @author nina
 *
 * @param <T>
 */
...
public abstract class OTProcessingContainers<T extends OTProcessingResource> extends OTContainers<T> {
	...
	public OTProcessingContainers() {
		this(null);
	}
	public OTProcessingContainers(String uri) {
		super(uri);
	}
	@Override
	 ...
	 public OTDataset process(OTDataset inputDataset, boolean copyAlways) throws Exception  {

		 if (size()==0) return inputDataset;
		 
		 
		 OTDataset dataset_copy = !copyAlways&&(size()==1)?
				inputDataset:
		 ...
		 if (!copyAlways&&(datasets.size()==1)) for (OTDataset dataset: datasets.getItems()) return dataset;
