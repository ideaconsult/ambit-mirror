...
/**
 * OpenTox Superservice. 
 * Same as {@link OTModel}, but identifies and calculates descriptors, if necessary
 * @author nina
 *
 */
@Deprecated
public class OTSuperModel extends OTModel {
    ...
    public OTDataset process(OTDataset inputDataset) throws Exception {
		...
		try {
		 if (!subsetToCalculate.isEmpty()) {
			 ...
			 results = super.predict(calculateDescriptors(subset));
			}
		 } catch (Exception x) { //if empty check fail
			 results = super.predict(calculateDescriptors(inputDataset));
		 }
		//else evertyhing is already calculated, return URI of this dataset with predicted features
		 if (dataset_service.isParent(results.getUri()))
    ...
    public OTDataset process(OTDataset inputDataset) throws Exception {
		 ...
		 if (subsetToCalculate == null) {
			 ...
			 if (descriptors == null) throw new ResourceException(Status.SERVER_ERROR_BAD_GATEWAY,String.format("Descriptor calculation failed %s",subset));
			 
			 results = super.predict(descriptors);
			 
			 if (results == null) throw new ResourceException(Status.SERVER_ERROR_BAD_GATEWAY,String.format("Model prediction failed %s",descriptors));
		 } else //else evertyhing is already calculated, return URI of this dataset with predicted features
			 results = inputDataset;
		 
		 if ((dataset_service !=null) && dataset_service.isParent(results.getUri()) && (getPredictedVariables()!=null) && (getPredictedVariables().size()>0))
			 return OTDataset.dataset(inputDataset.getUri()).withDatasetService(dataset_service).
		 		removeColumns().addColumns(getPredictedVariables());	
		 else return results;
