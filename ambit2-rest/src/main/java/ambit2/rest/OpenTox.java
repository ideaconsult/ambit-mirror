...
public class OpenTox {
	public static enum URI {
		query, missingValues, task, algorithm, model, dataset {
			@Override
		...
		feature, bookmark {
			...
			public String getResourceID() {
				return String.format("%s/{creator}/entries/{%s}", getURI(),
						getKey());
			}
		},
		reference, compound, conformer {
			@Override
			public String getResourceID() {
				return String.format("%s%s/{%s}",
						OpenTox.URI.compound.getResourceID(), getURI(),
						getKey());
			}
			...
			public Object[] getIds(String uri, Template template) {
				...
				try {
					template.parse(uri, vars);
					try {
						ids[0] = Integer.parseInt(vars.get(
								OpenTox.URI.compound.getKey()).toString());
					} catch (Exception x) {
						ids[0] = null;
					}
					;
					...
					try {
						ids[1] = Integer.parseInt(vars.get(
								OpenTox.URI.conformer.getKey()).toString());
					} catch (Exception x) {
						ids[1] = null;
					}
					;
		...
		public Template getTemplate(Reference baseReference) {
			return new Template(
					String.format("%s%s", baseReference == null ? ""
							: baseReference, getResourceID()));
		}
	...
	public static String removeDatasetFragment(String uri) {
		...
		if ((posCompound > 0) && (posCompound > posDataset))
			return String.format("%s%s", uri.substring(0, posDataset),
					uri.substring(posCompound));
		return uri;
	}
}
