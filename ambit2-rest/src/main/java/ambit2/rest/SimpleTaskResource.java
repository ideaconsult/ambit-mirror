 ...
 * 
 */
public class SimpleTaskResource<USERID>
		extends
	...
	protected void doInit() throws ResourceException {
		...
		setPaging(form);
		try {
			searchStatus = Reference.decode(form
					.getFirstValue(AbstractResource.search_param));
		} catch (Exception x) {
			searchStatus = null;
		}

		try {
			max = Integer.parseInt(Reference.decode(form
					.getFirstValue(AbstractResource.max_hits)));
		} catch (Exception x) {
			max = 10;
		}
		try {
			...
			throw new ResourceException(Status.CLIENT_ERROR_BAD_REQUEST,
					String.format("Allowed status values: %s %s, %s, %s",
	...
	protected boolean filterTask(ITask<ITaskResult, USERID> task, int taskNumber) {
		if ((max > 0) && (taskNumber >= max))
			...
			return searchStatus == null ? true : searchStatus.equals(task
					.getStatus().toString());
	}

	protected Iterator<UUID> getTasks() {

		return new FilteredTasksIterator<USERID>(
				((ITaskApplication) getApplication()).getTaskStorage()) {
			@Override
	...
	@Override
	protected synchronized Iterator<UUID> createQuery(Context context,
		...
		try {
			...
			if (id == null) {
				...
				ITask<ITaskResult, USERID> task = ((ITaskApplication<USERID>) getApplication())
						.findTask(Reference.decode(id.toString()));
	...
	public synchronized IProcessor<Iterator<UUID>, Representation> createConvertor(
		...
		ITaskStorage<USERID> storage = ((ITaskApplication) getApplication())
				.getTaskStorage();
		FactoryTaskConvertor<USERID> tc = new FactoryTaskConvertor<USERID>(
				storage);

		return tc.createTaskConvertor(variant, getRequest(), null,
				DisplayMode.table);
