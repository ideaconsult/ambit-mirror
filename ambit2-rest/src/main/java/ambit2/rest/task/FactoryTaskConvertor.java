...
public class FactoryTaskConvertor<USERID> extends net.idea.restnet.c.task.FactoryTaskConvertor<USERID> {
    ...
    public synchronized IProcessor<Iterator<UUID>, Representation> createTaskConvertor(Variant variant,
	    Request request, ResourceDoc doc, DisplayMode _dmode) throws AmbitException, ResourceException {
	String filenamePrefix = "task";
	return new StringConvertor(createTaskReporter(variant, request, doc, _dmode), variant.getMediaType(),
		filenamePrefix);
    }

    public synchronized Reporter<Iterator<UUID>, Writer> createTaskReporter(Variant variant, Request request,
	    ResourceDoc doc, DisplayMode _dmode) throws AmbitException, ResourceException {
	...
	return new TaskURIReporter<USERID>(storage, request) {
	    @Override
    ...
    public synchronized Representation createTaskRepresentation(Iterator<UUID> tasks, Variant variant, Request request,
	    Response response, net.idea.restnet.c.ResourceDoc doc) throws ResourceException {
	try {
