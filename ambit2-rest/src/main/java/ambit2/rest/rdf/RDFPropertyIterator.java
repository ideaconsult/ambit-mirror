...
public class RDFPropertyIterator extends RDFObjectIterator<Property> {
	...
	public RDFPropertyIterator(Representation representation,
			MediaType mediaType) throws ResourceException {
		super(representation, mediaType, OT.OTClass.Feature.toString());
	}

	public RDFPropertyIterator(Reference reference, String referer)
			throws ResourceException, MalformedURLException, IOException {
		super(reference, OT.OTClass.Feature.toString(), referer);
	}

	public RDFPropertyIterator(Reference reference, MediaType mediaType,
			String referer) throws ResourceException, MalformedURLException,
			IOException {
		super(reference, mediaType, OT.OTClass.Feature.toString(), referer);
	}

	public RDFPropertyIterator(InputStream in, MediaType mediaType)
	...
	protected Property createRecord() {
		...
		return reference == null ? baseReference == null ? new Property("")
				: new Property("", new LiteratureEntry(
						baseReference.toString(), baseReference.toString()))
				: new Property("", new LiteratureEntry(reference.toString(),
						reference.toString()));
	}
	...
	protected void parseObjectURI(RDFNode uri, Property property) {
		...
		try {
			...
			property.setId(Integer.parseInt(vars.get(
					OpenTox.URI.feature.getKey()).toString()));
		} catch (Exception x) {
			property.setId(-1);
		}
		;
	...
	public Property parseRecord(RDFNode propertyEntry, Property property,
		...
		try {
			...
			if (t != null) {
				...
				if (resource.isLiteral())
					...
					label = resource.isURIResource() ? ((Resource) resource)
		...
		Statement t = ((Resource) propertyEntry).getProperty(OTProperty.smarts
				.createProperty(jenaModel));
		...
		if ((smarts != null) && smarts.isLiteral()) {
			...
			property.setName(String.format("%s#%s", name,
					Reference.encode(fragment)));
		} else
			property.setName(name == null ? thisurl == null ? label : thisurl
					.toString() : name);
		property.setLabel(label);

		try {
			property.setUnits(((Literal) newEntry.getProperty(
					OT.DataProperty.units.createProperty(jenaModel))
					.getObject()).getString());
		} catch (Exception x) {
			property.setUnits("");
		}
		...
		String creator = (stmt == null) ? "Default"
				: (stmt.getObject() == null) ? "Default" : stmt.getObject()
						.isLiteral() ? ((Literal) stmt.getObject()).getString()
		...
		try {
			it = jenaModel.listStatements(new SimpleSelector(newEntry,
			...
			while (it.hasNext()) {
				...
				else if (resource.hasURI(OT.OTClass.ModelConfidenceFeature
						.getNS())) {
					// TODO
				} else if (resource.hasURI(OT.OTClass.ModelPredictionFeature
						.getNS())) {
					// TODO
				}
		...
		// confidence
		try {
			t = newEntry.getProperty(OTProperty.confidenceOf
					.createProperty(jenaModel));
			if (t != null) {
				...
				if (resource == newEntry) {
					...
					if (resource.isLiteral())
						...
						psa.setObject(resource.isURIResource() ? ((Resource) resource)
								.getURI() : resource.toString());
					pa = psa;
				}
	...
	protected LiteratureEntry processSource(Resource newEntry,
			String hasSource, String creator) {
		_type sourceType = _type.Dataset;
		Statement stmt = newEntry.getProperty(OT.OTProperty.hasSource
				.createProperty(jenaModel));
		if ((stmt != null) && (stmt.getObject() != null)) {
			...
			if (source.isURIResource()) {
				...
				if (baseReference != null) {
					if (hasSource.startsWith(String.format("%s/algorithm/",
							baseReference.toString()))) {
						hasSource = Reference
								.decode(hasSource.substring(baseReference
										.toString().length() + 11));
					} else if (hasSource.startsWith(String.format("%s/model/",
							baseReference.toString()))) {
						hasSource = Reference
								.decode(hasSource.substring(baseReference
										.toString().length() + 7));
					}
					if (hasSource.startsWith(String.format("%s/dataset/",
							baseReference.toString()))) {
						hasSource = Reference
								.decode(hasSource.substring(baseReference
										.toString().length() + 9));
					}
					if (hasSource.startsWith(String.format("%s/feature/",
							baseReference.toString()))) {
						hasSource = Reference
								.decode(hasSource.substring(baseReference
										.toString().length() + 9));
				...
				if ((type != null) && type.isURIResource()) {
					...
					else if (type.equals(OT.OTClass.Model
							.getOntClass(jenaModel)))
						sourceType = _type.Model;
					else if (type.equals(OT.OTClass.Dataset
							.getOntClass(jenaModel)))
						sourceType = _type.Dataset;
					else if (type.equals(OT.OTClass.Feature
							.getOntClass(jenaModel)))
	...
	public static void readFeaturesRDF(String uri,
			final ambit2.base.data.Template profile, Reference rootRef,String referer) {
		if (uri == null)
			return;
		...
		try {
			...
			iterator = new RDFPropertyIterator(new Reference(uri),referer);
			jenaModel = iterator.getJenaModel();
			iterator.setBaseReference(uri.startsWith("riap://application/") ? new Reference(
					"riap://application/") : rootRef);
			while (iterator.hasNext()) {
	...
	protected boolean isFeatureSource(Resource entry) {
		StmtIterator iterator = jenaModel.listStatements(new SimpleSelector(
				null, OTProperty.hasSource.createProperty(jenaModel), entry));
		try {
			while (iterator.hasNext()) {
				return true;
			}
