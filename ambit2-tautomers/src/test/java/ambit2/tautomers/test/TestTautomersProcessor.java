...
public class TestTautomersProcessor {
	...
	public void test() throws Exception {
		...
		String[] smiles = new String[] {
				...
				"CCc1cc2c(cc1OC(C)=O)oc(C)c(-c1nc3ccccc3n1C)c2=O",
				"CCCCCCCN(C(=O)CCl)c1cccc(C)c1",
				"CCc1ccc(OCC(=O)Nc2nnc[nH]2)cc1",
				"CN(C)Cc1c(nnn1-c1nonc1N)C(=O)NN=Cc1ccccc1OCc1ccc(F)cc1",
		...
		for (String smi : smiles) {
			...
			Assert.assertFalse(smol.getProperties().get("http://www.opentox.org/api/1.1#InChI").toString().startsWith("InChI=1S/"));
