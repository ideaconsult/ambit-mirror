...
public class CallableModelPredictor<ModelItem, Predictor extends ModelPredictor, USERID>
	...
	public CallableModelPredictor(Form form, Reference appReference, Context context, Predictor predictor, USERID token,
	...
	protected IBatchStatistics runBatch(Object target) throws Exception {
		...
		if (rdfFileWriter != null)
			rdfFileWriter.close();
		// can't find good way to close processors(writers) ... TODO extension
		// of iprocessor interface & batch
		return stats;
	}
	...
	protected IProcessor<IStructureRecord, IStructureRecord> getWriter() throws Exception {
		if (foreignInputDataset) {
			...
			writer.setDataset(new SourceDataset(sourceReference.toString(), new LiteratureEntry(
					predictor.getModelReporter().getURI(predictor.getModel()), sourceReference.toString())));
			return writer;
		}
	}
	...
	protected TaskResult createReference(Connection connection) throws Exception {
		if (foreignInputDataset) {
			...
			if (!tmpFile.exists())
				throw new ResourceException(Status.SERVER_ERROR_INTERNAL, "No results available!");
			try {
			...
			TaskResult task = new TaskResult(String.format("%s%s%s", sourceReference.toString(), q, predicted));
