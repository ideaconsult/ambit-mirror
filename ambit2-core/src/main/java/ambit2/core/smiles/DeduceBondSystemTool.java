...
//				logger.debug("");
    ...
    private boolean isStructureOK(IMolecule molecule) {
        for (int i = 0; i <= molecule.getAtomCount() - 1; i++) {
            //logger.debug(mj.getBondOrderSum(mj.getAtom(i)));
            try {
...
//						logger.debug(counter+"\t"+"ring not aromatic"+"\t"+r.getAtomCount());
                    ...
                    */
                    if (ring.getAtomCount() != aromaticAtoms) {
						logger.debug(counter+"\t"+"ring not aromatic"+"\t"+ring.getAtomCount());
                        return false;
                    }
        ...
        try {
            ...
            for (int i = 0; i <= rs.getAtomContainerCount() - 1; i++) {
                ...
                for (int j = 0; j <= r.getAtomCount() - 1; j++) {
                    ...
                    //logger.debug(j+"\t"+r.getAtom(j).getSymbol()+"\t"+r.getAtom(j).getHybridization());
