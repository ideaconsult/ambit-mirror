...
public class StructureNormalizer extends
	...
	public IStructureRecord process(IStructureRecord structure)
		...
		if ((structure.getFormat() != null)
				&& MOL_TYPE.NANO.name().equals(structure.getFormat())) {
			try { // nanomaterial
					// NanoCMLReader already have set all the IStructureRecord
					// fields
				return structure;
				/*
				 * Class clazz =
		...
		if ((molecule == null) || (molecule.getAtomCount() == 0))
			...
			if (build2D)
				try {
					IAtomContainer newmol = build2d(molecule,
							structure.getType());
					if (newmol != null) {
						molecule = newmol;
			...
			if (structure.getInchi() == null) {
				...
				if ((molecule == null) || (molecule.getAtomCount() == 0)) {
					...
					try {
						...
						if (INCHI_RET.OKAY.equals(gen.getReturnStatus())
								|| INCHI_RET.WARNING.equals(gen
										.getReturnStatus())) {
							inchi = gen.getInchi();
							key = gen.getInchiKey();
						} else {
							inchi = null;
							key = String.format("ERROR:%s:%s",
									gen.getReturnStatus(), gen.getMessage());
						}
					} catch (Exception x) {
						inchi = null;
						key = null;
		...
		if ((molecule != null) && (molecule.getAtomCount() > 0))
			try {
				...
				structure.setFormula(formula == null ? null
	...
	protected IAtomContainer build2d(IAtomContainer molecule,
			STRUC_TYPE structype) throws AmbitException {
		if (!STRUC_TYPE.D1.equals(structype))
			return null;
		if (builder == null)
			builder = new Structure2DBuilder();
