...
public class StructureTypeProcessor extends
		DefaultAmbitProcessor<IAtomContainer, STRUC_TYPE> {
	...
	public STRUC_TYPE process(IAtomContainer target) throws AmbitException {
		...
		if (hasHydrogens(target))
			if (has2DCoordinates(target) > 0)
				return STRUC_TYPE.D2withH;
			else if (has3DCoordinates(target))
				return STRUC_TYPE.D3withH;
			else
				return STRUC_TYPE.D1;
		else if (has2DCoordinates(target) > 0)
			return STRUC_TYPE.D2noH;
		else if (has3DCoordinates(target))
			return STRUC_TYPE.D3noH;
		else
			return STRUC_TYPE.D1;
	}
	...
	public static int has2DCoordinates(IAtomContainer container) {
		...
		if ((minC != null) && (maxC != null))
			with2d = (((minC.x - maxC.x) == 0) && ((minC.y - maxC.y) == 0)) ? false
					: with2d;
		if (!no2d && with2d) {
			return 2;
		} else if (no2d && with2d) {
