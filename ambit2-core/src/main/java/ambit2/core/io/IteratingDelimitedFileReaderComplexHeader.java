...
public abstract class IteratingDelimitedFileReaderComplexHeader<COLUMN> extends
	...
	public Object next() {
		...
		/*
		 * for (int i=0; i < headerOptions.length;i++) if (headerOptions[i]
		 * instanceof MolPropertiesIOSetting) ((MolPropertiesIOSetting)
		 * headerOptions[i]).getProperties() .assign(nextMolecule);
		 */
		return nextMolecule;
	}
	...
	/*
	 * protected IOSetting[] setHeaderOptions() { Vector commandOptions = new
	 * Vector(); IOSetting[] options = new IOSetting[header.size()]; for (int
	 * i=0; i < IColumnTypeSelection._columnTypesS.length;i++)
	 * commandOptions.add(IColumnTypeSelection._columnTypesS[i]);
	 * 
	 * for (int i=0; i < header.size();i++) { options[i] = new
	 * OptionIOSetting("Select column type", IOSetting.HIGH,
	 * header.get(i).toString(), commandOptions,
	 * IColumnTypeSelection._columnTypesS[IColumnTypeSelection._ctX]); } return
	 * options; }
	 */
	...
	protected void processHeader(BufferedReader in) {
		try {
			...
			for (int i = 0; i < getNumberOfColumns(); i++) {
				...
				if (smiHeader != null)
					smiHeader = smiHeader.toUpperCase();
				if (smiHeader != null && smiHeader.equals(defaultSMILESHeader)) {
					smilesIndex = i;
					break;
				}
				if ((smilesIndex < 0)
	...
	public void extractRowKeyAndData(String line) {
		...
		while (st.hasMoreTokens()) {
			...
			if (next != null) {
				...
				if (next.startsWith("InChI="))
					inchiIndex = fieldIndex;
			} else
				values[fieldIndex] = "";
