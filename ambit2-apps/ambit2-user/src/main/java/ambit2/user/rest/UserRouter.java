...
public class UserRouter extends MyRouter {
	...
	public UserRouter(Context context, OrganisationRouter orgRouter, ProjectRouter projectRouter) {
		...
		/*
		 * attach(String.format("/{%s}%s",UserDBResource.resourceKey,Resources.
		 * alert), alertRouter);
		 */
	}
	...
	public Restlet attachLocalDBAuthNZ(Router router, Context context, String secret, long sessionLength) {
		 ...
		 * auth.setNext(authz); authz.setNext(setCookieUserRouter);
		 */
		...
		router.attach(String.format("%s%s", Resources.register, Resources.notify),
	...
	public static Filter createBundlePolicyAuthorizer(Context context, String datadbname, String default_userdb,
			String config, int HOMEPAGE_DEPTH) throws Exception {
		String usersdbname = context.getParameters().getFirstValue(AMBITConfig.users_dbname.name());
		if (usersdbname == null)
			usersdbname = "ambit_users";
