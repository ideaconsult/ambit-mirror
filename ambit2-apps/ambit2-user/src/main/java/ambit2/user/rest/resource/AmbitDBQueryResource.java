...
public abstract class AmbitDBQueryResource<Q extends IQueryRetrieval<T>, T extends Serializable>
	...
	public static final MediaType GPML_WIKIPATHWAYS = new MediaType(
			"application/wikipath+xml");
	...
	protected ITask<Reference, Object> addTask(ICallableTask callable, T item,
		...
		return ((ITaskApplication) getApplication()).addTask(String.format(
				"%s %s %s", callable.toString(), getItemName(item),
				reference == null ? "" : " "), callable, getRequest()
				.getRootRef(), getToken());
	...
	protected Representation getHTMLByTemplate(Variant variant)
			throws ResourceException {
		getHTMLBeauty();
		return toRepresentation(getMap(variant), getTemplateName(),
				MediaType.TEXT_PLAIN);
	}

	protected void execUpdate(Object object, IQueryUpdate query)
			throws ResourceException {
