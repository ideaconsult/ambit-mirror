...
public class CreateUsersPolicyTest extends DbUnitTest {
	...
	protected IQueryUpdate<RESTPolicy, RESTPolicyUsers> createQuery()
			throws Exception {
	...
	protected void createVerify(IQueryUpdate<RESTPolicy, RESTPolicyUsers> query)
		...
		table = c.createQueryTable("EXPECTED_USER",
				"SELECT role_name FROM roles where role_name regexp '^B.*.R$'");
		Assert.assertEquals(1, table.getRowCount());
		table = c
				.createQueryTable(
						"EXPECTED_USER",
						"SELECT user_name,role_name FROM user_roles where user_name in ('test','admin') and role_name regexp '^B.*.R$'");
		Assert.assertEquals(2, table.getRowCount());
		c.close();
	...
	protected IQueryUpdate<RESTPolicy, RESTPolicyUsers> updateQuery()
			throws Exception {
		DBRole role = null;
		List<DBUser> users = new ArrayList<>();
		DBUser user = new DBUser();
		...
		policy.setUri("http://localhost/ambit2"
				+ getResource(bundletest.toString()));
	...
	protected void updateVerify(
			IQueryUpdate<RESTPolicy, RESTPolicyUsers> query, int users)
			throws Exception {
		IDatabaseConnection c = getConnection();
		...
		table = c.createQueryTable("EXPECTED_USER",
				"SELECT role_name FROM roles where role_name regexp '^B.*.W$'");
		Assert.assertEquals(1, table.getRowCount());
		table = c
				.createQueryTable(
						"EXPECTED_USER",
						"SELECT user_name,role_name FROM user_roles where user_name in ('test','admin') and role_name regexp '^B.*.W$'");
