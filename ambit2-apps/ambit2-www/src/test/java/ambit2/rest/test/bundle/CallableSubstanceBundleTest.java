...
public class CallableSubstanceBundleTest extends DbUnitTest {
	...
	try {
	    ...
	    SubstanceURIReporter<IQueryRetrieval<SubstanceRecord>> reporter = new SubstanceURIReporter<IQueryRetrieval<SubstanceRecord>>(
		    new Reference("http://localhost:8081/ambit2"));
	    CallableSubstanceBundle callable = new CallableSubstanceBundle(bundle, reporter, Method.POST, form,
	...
	try {
	    ...
	    SubstanceURIReporter<IQueryRetrieval<SubstanceRecord>> reporter = new SubstanceURIReporter<IQueryRetrieval<SubstanceRecord>>(
		    new Reference("http://localhost:8081/ambit2"));
	    CallableSubstanceBundle callable = new CallableSubstanceBundle(bundle, reporter, Method.PUT, form,
		    c.getConnection(), null);
	    TaskResult task = callable.call();
	    ITable table = c1.createQueryTable("EXPECTED",
		    String.format("SELECT idbundle,idsubstance from bundle_substance"));
