...
public class DatasetsResourceTest extends ProtectedResourceTest {
	...
	public void testAddCompoundsToDataset() throws Exception {
		...
		Reference reference = upload(new Reference(dataset),
				new FileRepresentation(url.getFile(), ChemicalMediaType.CHEMICAL_MDLSDF),
				new Variant(MediaType.TEXT_URI_LIST), false);
	...
	protected Reference upload(Reference datasetURI, Representation entity, Variant variant, boolean newDataset)
		...
		try {
			...
			OTRemoteTask task = new OTRemoteTask(datasetURI, variant.getMediaType(), entity,
					newDataset ? Method.POST : Method.PUT);
			...
			if (Status.SUCCESS_OK.equals(task.getStatus())) {
				...
				throw new ResourceException(task.getStatus(),
						task.getResult() == null ? task.getStatus().getDescription() : task.getResult().toString());
