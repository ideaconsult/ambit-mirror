...
public class PropertyResourceTest extends ResourceTest {
	...
	public void testQueryName() throws Exception {
		RDFPropertyIterator iterator = new RDFPropertyIterator(new Reference(
				String.format("http://localhost:%d%s?%s=%s", port,
						PropertyResource.featuredef,
						QueryResource.search_param, "Property")), "test");
		iterator.setCloseModel(true);
		iterator.setBaseReference(new Reference(String.format(
	...
	public void testQuerySameAS() throws Exception {
		RDFPropertyIterator iterator = new RDFPropertyIterator(new Reference(
				String.format("http://localhost:%d%s?%s=%s", port,
						PropertyResource.featuredef,
						OpenTox.params.sameas.toString(), "CASRN")), "test");
		iterator.setCloseModel(true);
		iterator.setBaseReference(new Reference(String.format(
				"http://localhost:%d", port)));
		while (iterator.hasNext()) {
	...
	public void testRDFXMLForeignURI() throws Exception {
		try {
			RDFPropertyIterator iterator = new RDFPropertyIterator(
					new Reference("http://google.com"),"test");
			iterator.setBaseReference(new Reference(String.format(
					"http://localhost:%d", port)));
			while (iterator.hasNext()) {
				Property p = iterator.next();
	 ...
	 * in) throws Exception {
	 * 
	 * Document doc = createDOM(in); PropertyDOMParser parser = new
	 * PropertyDOMParser() {
	 * 
	 * @Override public void handleItem(Property entry) throws AmbitException {
	...
	public boolean verifyResponseURI(String uri, MediaType media, InputStream in)
		...
		int count = 0;
		while ((line = r.readLine()) != null) {
			Assert.assertEquals(String.format("http://localhost:%d%s/1", port,
					PropertyResource.featuredef), line);
			count++;
		}
		return count == 1;
	}
	...
	public void testDeleteEntry() throws Exception {
		...
		ITable table = c.createQueryTable("EXPECTED",
				"SELECT * FROM properties where idproperty=1");
	...
	public void testCopyEntry() throws Exception {
		...
		Form form = new Form();
		form.add(OpenTox.params.feature_uris.toString(), String.format(
				"http://localhost:%d%s/1", port, PropertyResource.featuredef));

		Response response = testPost(String.format("http://localhost:%d%s",
				port, PropertyResource.featuredef),
				MediaType.APPLICATION_RDF_XML, form.getWebRepresentation());
		Assert.assertEquals(Status.SUCCESS_OK, response.getStatus());
		Assert.assertEquals("http://localhost:8181/feature/7", response
		...
		ITable table = c
				.createQueryTable(
						"EXPECTED",
						"SELECT name,comments,title,url,type FROM properties join catalog_references using(idreference) where idproperty in (1,7) order by idproperty");
		Assert.assertEquals(2, table.getRowCount());
		Assert.assertEquals(table.getValue(0, "name"),
				table.getValue(1, "name"));
	...
	public void testCopyEntry1() throws Exception {
		...
		PropertyRDFReporter.addToModel(model, p, reporter,
	...
	public void testCreateEntry2() throws Exception {
		...
		BufferedReader br = new BufferedReader(new InputStreamReader(getClass()
				.getClassLoader().getResourceAsStream("feature.rdf")));
		String line;
	...
	public void testCreateEntry3() throws Exception {
		...
		StringWriter writer = new StringWriter();
		BufferedReader br = new BufferedReader(new InputStreamReader(getClass()
				.getClassLoader().getResourceAsStream("feature1.rdf")));
		String line;
		while ((line = br.readLine()) != null) {
			writer.append(line);
		}
	...
	public void testCreateEntry() throws Exception {
		...
		p.setNominal(true);
		PropertyRDFReporter.addToModel(
				model,
				p,
				new PropertyURIReporter(new Reference(String.format(
						"http://localhost:%d%s", port,
						PropertyResource.featuredef))),
				new ReferenceURIReporter());
		StringWriter writer = new StringWriter();
		model.write(writer, "RDF/XML");
	...
	public void testUpdateExistingEntry1() throws Exception {
		...
		Property p = new Property("Property 1", new LiteratureEntry("Dummy 1",
				"NA"));
		p.setLabel("Test");
		p.setId(1);
		OntModel model = OT.createModel();
		PropertyRDFReporter.addToModel(model, p, new PropertyURIReporter(
				new Reference(String.format("http://localhost:%d%s", port))),
				new ReferenceURIReporter());
		StringWriter writer = new StringWriter();
		model.write(writer, "RDF/XML");

		Response response = testPost(String.format("http://localhost:%d%s/2",
				port, PropertyResource.featuredef),
				MediaType.APPLICATION_RDF_XML, writer.toString());
		Assert.assertEquals(Status.SUCCESS_OK, response.getStatus());
		...
		ITable table = c
				.createQueryTable(
						"EXPECTED",
						String.format(
								"SELECT * FROM properties join catalog_references using(idreference) where name='Property 1' and comments='%s' and title='Dummy' and url='NA'",
								"http://www.opentox.org/api/1.1#Test"));
		Assert.assertEquals(1, table.getRowCount());
		c.close();
		Assert.assertEquals("http://localhost:8181/feature/2", response
				.getLocationRef().toString());
	}
	...
	public void testUpdateExistingEntry() throws Exception {
		...
		Property p = new Property("Property 1", new LiteratureEntry("Dummy",
				"NA"));
		p.setLabel("Test");
		p.setNominal(true);
		OntModel model = OT.createModel();
		...
		Response response = testPut(String.format("http://localhost:%d%s/1",
				port, PropertyResource.featuredef), MediaType.TEXT_URI_LIST,
				new StringRepresentation(writer.toString(),
		...
		c.close();
		Assert.assertEquals("http://localhost:8181/feature/1", response
				.getLocationRef().toString());
	}
	...
	public void testUpdateSameAs() throws Exception {
		...
		ITable table = c
				.createQueryTable(
						"EXPECTED",
						String.format(
								"SELECT * FROM properties where idproperty=1 and comments='%s'",
								sameAs));
		Assert.assertEquals(0, table.getRowCount());
		OTFeature feature = OTFeature.feature(String.format(
				"http://localhost:%d%s/1", port, PropertyResource.featuredef),"test");
	...
	public void testCreateExistingEntry() throws Exception {
		...
		ITable table = c
				.createQueryTable(
						"EXPECTED",
						String.format(
								"SELECT * FROM properties join catalog_references using(idreference) where name='CAS' and comments='%s' and title='Dummy' and url='NA'",
								"http://www.opentox.org/api/1.1#CASRN"));
		Assert.assertEquals(1, table.getRowCount());
		c.close();
		Assert.assertEquals("http://localhost:8181/feature/3", response
				.getLocationRef().toString());
	}
	...
	public void testCreateEntry4() throws Exception {
		...
		Individual f = model.createIndividual(OT.OTClass.Feature
				.getOntClass(model));
		f.addProperty(model.createAnnotationProperty(DC.creator.getURI()),
				model.createTypedLiteral("http://opentox.ntua.gr"));

		f.setSameAs(model.createResource("http://other.com/feature/200",
				OT.OTClass.Feature.getOntClass(model)));
		...
		ITable table = c
				.createQueryTable(
						"EXPECTED",
						"SELECT * FROM properties join catalog_references using(idreference) where name='http://other.com/feature/200' and comments='http://other.com/feature/200' and title='http://opentox.ntua.gr' and url='http://opentox.ntua.gr'");
		// ITable table =
		// c.createQueryTable("EXPECTED","SELECT * FROM properties join catalog_references using(idreference) where name='http://other.com/feature/200' and comments='http://other.com/feature/200' and title='http://opentox.ntua.gr' and url='http://localhost:8181'");
		Assert.assertEquals(1, table.getRowCount());
		c.close();
	}
	...
	public void testCreateForeignEntry() throws Exception {
		...
		form.add(OpenTox.params.feature_uris.toString(),
				"http://my.new.property.org");

		Response response = testPost(String.format("http://localhost:%d%s",
				port, PropertyResource.featuredef), MediaType.TEXT_RDF_N3,
		...
		ITable table = c
				.createQueryTable(
						"EXPECTED",
						"SELECT * FROM properties join catalog_references using(idreference) where name='http://my.new.property.org' and comments='http://my.new.property.org' and title='http://my.new.property.org' and url='http://my.new.property.org'");
		Assert.assertEquals(1, table.getRowCount());
	...
	public void testGetJavaObject() throws Exception {
		testGetJavaObject(String.format("http://localhost:%d%s?%s=%s", port,
				PropertyResource.featuredef, OpenTox.params.sameas.toString(),
