...
public class PropertyResourceTest extends ResourceTest {
	...
	RDFPropertyIterator iterator = new RDFPropertyIterator(new Reference(String.format(
		"http://localhost:%d%s?%s=%s", port, PropertyResource.featuredef, QueryResource.search_param,
		"Property")));
	...
	RDFPropertyIterator iterator = new RDFPropertyIterator(new Reference(String.format(
		"http://localhost:%d%s?%s=%s", port, PropertyResource.featuredef, OpenTox.params.sameas.toString(),
		"CASRN")));
	iterator.setCloseModel(true);
	iterator.setBaseReference(new Reference(String.format("http://localhost:%d", port)));
	while (iterator.hasNext()) {
	...
	int count = 0;
	while ((line = r.readLine()) != null) {
	    Assert.assertEquals(String.format("http://localhost:%d%s/1", port, PropertyResource.featuredef), line);
	    count++;
	}
	return count == 1;
    }
	...
	Assert.assertEquals("http://localhost:8181/feature/7", response.getLocationRef().toString());
	...
	Assert.assertEquals(table.getValue(0, "name"), table.getValue(1, "name"));
	...
	PropertyRDFReporter.addToModel(model, p, reporter, new ReferenceURIReporter());
	...
	StringWriter writer = new StringWriter();
	BufferedReader br = new BufferedReader(new InputStreamReader(getClass().getClassLoader().getResourceAsStream(
		"feature.rdf")));
	String line;
	...
	Response response = testPost(String.format("http://localhost:%d%s", port, PropertyResource.featuredef),
		MediaType.APPLICATION_RDF_XML, writer.toString());
	Assert.assertEquals(Status.SUCCESS_OK, response.getStatus());
	Assert.assertEquals(response.getLocationRef().toString(), "http://localhost:8181/feature/7");
	...
	StringWriter writer = new StringWriter();
	InputStream in = getClass().getClassLoader().getResourceAsStream("featureMNA2.rdf");
	...
	InputStream in = getClass().getClassLoader().getResourceAsStream("featureMNA3.rdf");
	Assert.assertNotNull(in);
	StringWriter writer = new StringWriter();
	...
	Response response = testPost(String.format("http://localhost:%d%s/2", port, PropertyResource.featuredef),
		MediaType.APPLICATION_RDF_XML, writer.toString());
	Assert.assertEquals(Status.SUCCESS_OK, response.getStatus());
	...
	OntModel model = OT.createModel();
	PropertyRDFReporter.addToModel(model, p,
		new PropertyURIReporter(new Reference(String.format("http://localhost:%d%s", port,PropertyResource.featuredef))),
		new ReferenceURIReporter());
	StringWriter writer = new StringWriter();
	model.write(writer, "RDF/XML");
	...
	ITable table = c.createQueryTable("EXPECTED",
		String.format("SELECT * FROM properties where idproperty=1 and comments='%s'", sameAs));
	Assert.assertEquals(0, table.getRowCount());
	OTFeature feature = OTFeature.feature(String.format("http://localhost:%d%s/1", port,
		PropertyResource.featuredef));
	...
	Response response = testPost(String.format("http://localhost:%d%s", port, PropertyResource.featuredef),
		MediaType.TEXT_RDF_N3, form.getWebRepresentation());
	Assert.assertEquals(Status.SUCCESS_OK, response.getStatus());
