...
var jToxBundle = {
	...
	settings : {
		...
		maxStars : 10,
		matrixIdentifiers : [ "http://www.opentox.org/api/1.1#CASRN",
				"#SubstanceName", "#SubstanceUUID",
				"http://www.opentox.org/api/1.1#SubstanceDataSource", ],
		matrixMultiRows : [ "#Tag", "http://www.opentox.org/api/1.1#Diagram",
				"#ConstituentName", "#ConstituentContent",
				"#ConstituentContainedAs" ]
	},

	parseFeatureId : function(featureId) {
		var parse = featureId
				.match(/https?\:\/\/(.*)\/property\/([^\/]+)\/([^\/]+)\/.+/);
		if (parse == null)
			return null;
		else
			return {
	...
	init : function(root, settings) {
		...
		var loadAction = function() {
			...
			if (!method)
				return;
			ccLib.fireCallback(self[method], self, this.id, $(this).closest(
					'.ui-tabs-panel')[0], false);
		};
		...
		var loadPanel = function(panel) {
			if (panel) {
				...
				if (subs.length > 0)
					...
					ccLib.fireCallback(self[$(panel).data('action')], self,
							panel.id, panel, true);
			}
		};
		...
		if (self.settings.bundleUri === undefined) {
			$('.jtox-users-select', root).tokenInput(
					"../myaccount/users?media=application/json", {
						"hintText" : "Type in an user name",
						minChars: 2
					});
		} else {
			
			$.ajax({
				url : "../myaccount/users?media=application/json&bundle_uri="+encodeURIComponent(self.settings.bundleUri)+"&mode=R",
				context : document.body
			}).done(function(json) {
				// console.log(json);
				$('#canRead', root).tokenInput(
						"../myaccount/users?media=application/json", {
							"hintText" : "Type in an user name",
							"prePopulate" : json,
							 minChars: 2
			...
			$.ajax({
				url : "../myaccount/users?media=application/json&bundle_uri="+encodeURIComponent(self.settings.bundleUri)+"&mode=W",
				context : document.body
			}).done(function(json) {
				// console.log(json);
				$('#canWrite', root).tokenInput(
						"../myaccount/users?media=application/json", {
							"hintText" : "Type in an user name",
							"prePopulate" : json,
							minChars: 2
	...
	modifyUri : function(uri) {
		return ccLib.addParameter(uri, "bundle_uri="
				+ encodeURIComponent(this.bundleUri));
	},
		...
		if (!$(panel).hasClass('initialized')) {
			...
			self.createForm.assStart.onclick = function(e) {
				...
				if (ccLib.validateForm(self.createForm, checkForm)) {
					jT.service(self, '/bundle', {
						method : 'POST',
						data : ccLib.serializeForm(self.createForm)
					}, function(bundleUri, jhr) {
						if (!!bundleUri)
							self.load(bundleUri);
						else
							// TODO: report an error
							console.log("Error on creating bundle ["
									+ jhr.status + ": " + jhr.statusText);
					});
				}
			};
			...
			self.createForm.assNewVersion.onclick = function(e) {
				...
				if (!self.bundleUri)
					return;
				jT.service(self, self.bundleUri + '/version', {
					method : 'POST'
				}, function(bundleUri, jhr) {
					if (!!bundleUri)
						self.load(bundleUri);
						...
						// TODO: report an error
						console.log("Error on creating bundle [" + jhr.status
								+ ": " + jhr.statusText);
				});
			};
			...
			$('span.ui-icon-star', starsEl)
					...
					.on(
							...
							'mouseout',
							function(e) {
								self.starHighlight(this,
										parseInt(self.createForm.stars.value));
							});

			// install change handlers so that we can update the values
			$('input, select, textarea', self.createForm).on('change',
					function(e) {
						e.preventDefault();
						e.stopPropagation();
						if (!self.bundleUri)
						...
						if (ccLib.fireCallback(checkForm, el, e)) {
							...
							$(el).addClass('loading');
							jT.service(self, self.bundleUri, {
								method : 'PUT',
								data : data
							}, function(result) {
								$(el).removeClass('loading');
								if (!result) // i.e. on error - request the
												// old data
	...
	onMatrix : function(panId, panel) {
		...
		if (!$(panel).hasClass('initialized')) {
			jTConfig.matrix.groups = function(miniset, kit) {
				var groups = {
					"Identifiers" : self.settings.matrixIdentifiers
							.concat(self.settings.matrixMultiRows)
				}, groupids = [], endpoints = {};
					...
					return function(data, type, full) {
						...
						for ( var fId in self.matrixKit.dataset.feature) {
							...
							if (f.sameAs != feat.sameAs
									|| full.values[fId] == null)
								continue;

							var catId = self.parseFeatureId(fId).category, config = jT.$
									.extend(
											true,
											{},
											kit.settings.configuration.columns["_"],
											kit.settings.configuration.columns[catId]);

							var theData = full.values[fId];
							var preVal = (ccLib.getJsonValue(config,
									'effects.endpoint.bVisible') !== false) ? f.title
									: null;
							preVal = [ f.creator, preVal ].filter(
									function(value) {
										return value !== null
									}).join(' ');
							...
							for (var i = 0, vl = theData.length; i < vl; ++i) {
								...
								html += '<a class="info-popup" data-index="'
										+ i
										+ '" data-feature="'
										+ fId
										+ '" href="#">'
										+ jT.ui.renderObjValue(d, f.units,
												'display', preVal) + '</a>';
								html += jT.ui.putInfo(full.compound.URI
										+ '/study?property_uri='
						...
						if (self.edit.matrixEditable)
							html += '<span class="ui-icon ui-icon-circle-plus edit-popup" data-feature="'
									+ theId + '"></span>';
						return html;
					};
				};
				...
				for ( var fId in miniset.feature) {
					...
					if (feat.sameAs == null
							|| feat.sameAs.indexOf("echaEndpoints.owl#") < 0)
						continue;
					...
					if (endpoints[feat.sameAs] == null) {
						...
						if (!feat.title)
							feat.title = feat.sameAs.substr(feat.sameAs
									.indexOf('#') + 1);
				...
				/*
				 * Sort columns alphabetically, in this case - by the category
				 * number.
				 */
					...
					if (grp != 'Identifiers') {
						...
						group
								.sort(function(a, b) {
									...
									return (miniset.feature[a].title < miniset.feature[b].title) ? -1
											: 1;
								});
					}
				}
				...
				/*
				 * Sort groups by the columns titles / category number. Since we
				 * are trying to sort an Object keys, which is itself nonsense
				 * in JavaScript, this may fail at any time.
				 */
			...
			conf.baseFeatures['#Tag'] = {
				...
				render : function(data, type, full) {
					...
					if (!!bInfo.tag) {
						html += '<button class="jt-toggle active" disabled="true"'
								+ (!bInfo.remarks ? '' : 'title="'
										+ bInfo.remarks + '"')
								+ '>'
								+ (bInfo.tag == 'source' ? 'S' : 'T')
								+ '</button><br />';
					}
			...
			var dressButton = function() {
				if (self.edit.study.length < 1) {
					saveButton.disabled = true;
					$(saveButton).removeClass('jt-alert').addClass(
							'jt-disabled');
					saveButton.innerHTML = "Saved";
				} else {
					saveButton.disabled = false;
					$(saveButton).addClass('jt-alert').removeClass(
							'jt-disabled');
					...
					function() {
						if (self.edit.study.length > 0) {
							var toAdd = JSON.stringify({
								study : self.edit.study
							});

							// make two nested calls - for adding and for
							// deleting
							$(saveButton).addClass('loading');
							jT.service(self, self.bundleUri + '/matrix', {
								method : 'PUT',
								headers : {
								...
								if (!!result)
									jT.service(self, self.bundleUri
										...
										if (!!result) {
											self.edit.study = [];
											self.matrixKit.query(self.bundleUri
													+ '/matrix');
											dressButton();
										}
									});
			...
			var onEditClick = function(data) {
				...
				if (!jel.hasClass('edit-popup')) {
					...
					ccLib.fillTree(infoDiv, {
						...
						value : jT.ui.renderObjValue(val[valueIdx],
								feature.units, 'display'),
					// source: '<a target="_blank" href="' + feature.source.URI
					// + '">' + feature.source.type + '</a>'
					});
					...
					// we're taking the original jToxEndpoint editor here and
					// glue our part after it.
					boxOptions.content = jT.getTemplate('#jtox-endeditor').innerHTML
							+ editDiv.innerHTML;
					...
					boxOptions.onOpen = function() {
						var box = this;
						var content = this.content[0];
						jToxEndpoint.linkEditors(self.matrixKit, content, {
							category : parse.category,
							top : parse.topcategory,
							onchange : endSetValue,
							conditions : true
						});
						$('input[type=button]', content).on(
								'click',
								function() {
									addFeature(data, featureId, featureJson,
											jel[0]);
									box.close();
								});
					};
			...
			var addFeature = function(data, fId, value, element) {
				...
				var catId = self.parseFeatureId(fId).category, config = jT.$
						.extend(
								true,
								{},
								self.matrixKit.settings.configuration.columns["_"],
								self.matrixKit.settings.configuration.columns[catId]), f = null;

				self.matrixKit.dataset.feature[fId] = f = {};
				f.sameAs = "http://www.opentox.org/echaEndpoints.owl#" + catId;
				f.title = value.effects[0].endpoint
				...
				for ( var cId in value.effects[0].conditions) {
					f.annotation
							.push({
								'p' : cId,
								'o' : jT.ui
										.renderObjValue(value.effects[0].conditions[cId])
							});
				}
				...
				var preVal = (ccLib.getJsonValue(config,
						'effects.endpoint.bVisible') !== false) ? f.title
						: null;
				preVal = [ f.creator, preVal ].filter(function(value) {
					return value !== null
				}).join(' ');

				var html = '<span class="ui-icon ui-icon-circle-minus delete-popup" data-index="'
						+ (self.edit.study.length - 1) + '"></span>&nbsp;';
				html += '<a class="info-popup unsaved-study" data-index="0" data-feature="'
						+ fId
						+ '" href="#">'
						+ jT.ui.renderObjValue(value.effects[0].result, null,
								'display', preVal) + '</a>';
				...
				$('.info-popup', span).on('click', function(e) {
			...
			var deleteFeature = function(data, featureId, valueIdx, reason,
					element) {
				self.edit.study
						.push({
							owner : {
				...
				$('span', element.parentNode).removeClass(
						'ui-icon-circle-minus').addClass('ui-icon-circle-plus')
						.data('index', self.edit.study.length - 1).on(
								'click.undodelete',
								function() {
									var idx = $(this).data('index');
									$(this).addClass('ui-icon-circle-minus')
											.removeClass('ui-icon-circle-plus')
											.off('click.undodelete').data(
													'index', null);
			...
			self.matrixKit = new jToxCompound(
					$('.jtox-toolkit', panel)[0],
					{
						crossDomain : true,
						rememberChecks : true,
						tabsFolded : true,
						...
						onPrepared : function(miniset, kit) {
							if (featuresInitialized)
								return;
							// this is when we have the features combined, so we
							// can make the multi stuff
															...
															function(_data,
																...
																var dt = ccLib
																		.getJsonValue(
																				_data,
																				(fId
																						.indexOf('#Diagram') > 0 ? 'component.'
																						: '')
																						+ oldData);
																return (typeof oldRender == 'function' ? oldRender(
																		dt,
																		_type,
																		fId
																				.indexOf('#Diagram') > 0 ? _data.component
																				: _data)
																		: dt);
															});
								};
						...
						onLoaded : function(dataset) {
							jToxCompound.processFeatures(dataset.feature,
									this.feature);
							...
							for (var i = 0, dl = dataset.dataEntry.length; i < dl; ++i) {
								...
								if (data.composition != null)
									for (var j = 0; j < data.composition.length; ++j)
										jToxCompound.processEntry(
												data.composition[j].component,
												dataset.feature);
							}
						},
						...
						onRow : function(row, data, index) {
							...
							jT
									.$(
											'td.jtox-multi .jtox-diagram span.ui-icon',
											row)
									.on(
											'click',
											function() {
												setTimeout(
														function() {
															ccLib.equalizeHeights
																	.apply(
																			window,
																			jT
																					.$(
																							'td.jtox-multi table tbody',
																							row)
																					.toArray());
														}, 50);
											});

							$('.info-popup, .edit-popup, .delete-popup', row)
									.on(
											'click',
											function(e) {
												if (!$(this).hasClass(
							...
							$('button.jtox-up', row)
									.on(
											'click',
											function() {
												var i = $(self.fixTable).find(
														'> tbody > tr').index(
														row);
												var varRow = $(self.varTable)
														.find('> tbody > tr')[i];
												$(row)
														.insertBefore(
												...
												$(varRow)
														.insertBefore(
																$(varRow.previousElementSibling));
											});
							$('button.jtox-down', row).on(
									'click',
									function() {
										var i = $(self.fixTable).find(
												'> tbody > tr').index(row);
										var varRow = $(self.varTable).find(
												'> tbody > tr')[i];
										$(row).insertAfter(
												$(row.nextElementSibling));
										$(varRow).insertAfter(
												$(varRow.nextElementSibling));
									});
		...
		$('.create-button', panel).on('click', function() {
			var el = this;
			$(el).addClass('loading');
			jT.service(self, self.bundleUri + '/matrix/working', {
				method : 'POST',
				data : {
					deletematrix : false
				}
		...
		$('.create-final-button', panel).on('click', function() {
			var el = this;
			$(el).addClass('loading');
			jT.service(self, self.bundleUri + '/matrix/final', {
				method : 'POST',
				data : {
					deletematrix : false
				}
		...
		if (panId == 'xinitial') {
			...
			var queryPath = (panId == 'xfinal') ? '/matrix/final'
					: '/matrix/working';
	...
	onEndpoint : function(id, panel) {
		...
		if (id == "endsubstance") {
			...
			if (!self.substancesQueryKit) {
				...
				// Modify the #IdRow not to show tag buttons and add #Tag column
				// that show the selected tag.
				self.substancesQueryKit.kit().settings.configuration.baseFeatures['#IdRow'] = {
					used : true,
					basic : true,
					data : "number",
					column : {
				...
				self.substancesQueryKit.kit().settings.configuration.baseFeatures['#Tag'] = {
					title : 'Tag',
					used : false,
					basic : true,
					visibility : "main",
				...
				self.substancesQueryKit.kit().settings.configuration.groups.Identifiers
						.push('#Tag');

				// provid onRow function so the buttons can be set properly...
				self.substancesQueryKit.kit().settings.onRow = function(row,
						data, index) {
					if (!data.bundles) {
						return;
					}
					...
					if (!!bundleInfo.tag) {
						$('button.jt-toggle.' + bundleInfo.tag.toLowerCase(),
								row).addClass('active');
				...
				$('#structures-expand-all')
						.on(
								'click',
								function() {
									$(
											'#jtox-substance-query .jtox-details-open.ui-icon-folder-collapsed')
											.each(function() {
												this.click();
											});
								});
				$('#structures-collapse-all')
						.on(
								...
								function() {
									$(
											'#jtox-substance-query .jtox-details-open.ui-icon-folder-open')
											.each(function() {
												this.click();
											});
								});
			...
			self.substancesQueryKit.kit().queryDataset(
					self.bundleUri + '/compound');
				...
				self.endpointKit = new jToxEndpoint(root, {
					...
					onRow : function(row, data, index) {
						...
						if (!!bundleInfo && bundleInfo.tag == "selected")
							$('input.jtox-handler', row).attr('checked',
									'checked');
					}
				});
	...
	onStructures : function(id, panel) {
		...
		if (!self.queryKit) {
			...
			self.queryKit.kit().settings.onRow = function(row, data, index) {
				...
				var bundleInfo = data.bundles[self.bundleUri] || {};
				// we need to setup remarks field regardless of bundleInfo
				// presence
				var noteEl = $('textarea.remark', row).on('change',
						function(e) {
				...
				if (!!bundleInfo.tag) {
					$('button.jt-toggle.' + bundleInfo.tag.toLowerCase(), row)
							.addClass('active');
					noteEl.val(bundleInfo.remarks);
				} else {
					noteEl.prop('disabled', true).val(' ');
				...
				.call(
						...
						function(bundle) {
							if (!!bundle) {
								...
								self.starHighlight($('.data-stars-field div',
										self.createForm)[0], bundle.stars);
								self.createForm.stars.value = bundle.stars;

								// now take care for enabling proper buttons on
								// the Indetifiers page
	...
	progressTabs : function() {
		$(this.rootElement).tabs(
				this.bundleSummary.compound > 0 ? 'enable' : 'disable', 2);
		$(this.rootElement).tabs(
				this.bundleSummary.substance > 0
						&& this.bundleSummary.property > 0 ? 'enable'
						: 'disable', 3);
		if (this.bundleSummary.matrix > 0
				|| this.bundleSummary['matrix/final'] > 0) {
			$('#xfinal').button('enable');
		} else {
	...
	selectSubstance : function(uri, el) {
		var self = this;
		$(el).addClass('loading');
		jT.service(self, self.bundleUri + '/substance', {
			method : 'PUT',
			data : {
				substance_uri : uri,
...
// Now some handlers - they should be outside, because they are called within
// windows' context.
function onSelectStructure(e) {
	jToxBundle.selectStructure($(this).data('data'),
			$(this).hasClass('target') ? 'target' : 'source', this);
}
...
function preDetailedRow(index, cell) {
	...
	var uri = this.settings.baseUrl + '/substance?type=related&compound_uri='
			+ encodeURIComponent(data.compound.URI) + '&filterbybundle='
			+ encodeURIComponent(this.settings.bUri) + '&bundle_uri='
			+ encodeURIComponent(this.settings.bUri);

	var $row = $(cell.parentNode), $idcell = $row.find('td:first-child'), $button = $row
			.find('td:first-child > .jtox-details-open');
	...
	if (!!$(cell).data('details')) {
		if ($(cell).data('details').hasClass('jtox-hidden')) {
			...
			$button.removeClass('ui-icon-folder-collapsed').addClass(
					'ui-icon-folder-open');
		} else {
			$(cell).data('details').addClass('jtox-hidden');
			$idcell.removeAttr('rowspan');
			$button.addClass('ui-icon-folder-collapsed').removeClass(
					'ui-icon-folder-open');
		}
	} else {

		var $cell = $('<td class="paddingless"></td>'), $newRow = $('<tr></tr>')
				.append($cell).insertAfter($row).addClass($row[0].className);

		$idcell.attr('rowspan', '2');

		$cell.attr('colspan', $row.find('td:visible').length - 1).removeClass(
				'jtox-hidden');
...
function onDetailedRow(row, data, event) {
	...
	if (!el)
		return;
	var uri = this.settings.baseUrl + '/substance?type=related&compound_uri='
			+ encodeURIComponent(data.compound.URI);
	...
	var div = document.createElement('div');
	el.appendChild(div);
	new jToxSubstance(div, $.extend(true, {}, this.settings, {
		crossDomain : true,
		selectionHandler : null,
		substanceUri : uri,
		showControls : false,
		onLoaded : null,
		onDetails : function(root, data, element) {
			new jToxStudy(root, $.extend({}, this.settings, {
