...
public class BundleChemicalsResource<Q extends IQueryRetrieval<IStructureRecord>>
		extends AmbitDBResource<Q, IStructureRecord> {
	...
	protected void doInit() throws ResourceException {
		...
		customizeVariants(new MediaType[] { ChemicalMediaType.CHEMICAL_MDLSDF,
				...
				ChemicalMediaType.IMAGE_JSON, MediaType.TEXT_CSV,
				ChemicalMediaType.APPLICATION_JSONLD,
				MediaType.APPLICATION_RDF_XML, MediaType.TEXT_RDF_N3 });
	...
	protected CallableProtectedTask<String> createCallable(Method method,
			Form form, IStructureRecord item) throws ResourceException {
		...
		try {
			...
			return new CallableCompoundBundle(bundle, r, method, form, conn,
					getToken());
		} catch (Exception x) {
			try {
				conn.close();
	...
	protected Q createUpdateQuery(Method method, Context context,
		...
		Object idchemical = request.getAttributes().get(
	...
	public IProcessor<Q, Representation> createConvertor(Variant variant)
		...
		if (variant.getMediaType().equals(MediaType.TEXT_URI_LIST)) {
			...
			return new OutputWriterConvertor(new CompoundJSONReporter(
					getTemplate(), getGroupProperties(), folders,
					new SubstanceEndpointsBundle[] { bundle }, getRequest(),
			...
			return new OutputWriterConvertor(new CompoundJSONReporter(
					getTemplate(), getGroupProperties(), folders,
					new SubstanceEndpointsBundle[] { bundle }, getRequest(),
					getRequest().getRootRef().toString(), includeMol,
			...
			return new OutputWriterConvertor<IStructureRecord, Q>(
					new SDFReporter<Q>(template, getGroupProperties(),
							changeLineSeparators),
					ChemicalMediaType.CHEMICAL_MDLSDF, filenamePrefix);
		} else if (variant.getMediaType().equals(
				ChemicalMediaType.CHEMICAL_MDLMOL)) {
			return new OutputWriterConvertor<IStructureRecord, Q>(
					new SDFReporter<Q>(new Template(), getGroupProperties(),
							true, changeLineSeparators),
					ChemicalMediaType.CHEMICAL_MDLMOL, filenamePrefix);
		} else if (variant.getMediaType()
				.equals(ChemicalMediaType.CHEMICAL_CML)) {
			return new OutputWriterConvertor<IStructureRecord, Q>(
					new CMLReporter<Q>(), ChemicalMediaType.CHEMICAL_CML,
					filenamePrefix);
		} else if (variant.getMediaType().equals(
				ChemicalMediaType.CHEMICAL_SMILES)) {
			return new OutputWriterConvertor<IStructureRecord, Q>(
					new SmilesReporter<Q>(true, getTemplate()),
					ChemicalMediaType.CHEMICAL_SMILES, filenamePrefix);
		} else if (variant.getMediaType().equals(
				ChemicalMediaType.CHEMICAL_INCHI)) {
			return new OutputWriterConvertor<IStructureRecord, Q>(
					new SmilesReporter<Q>(false, Mode.InChI, getTemplate()),
			...
			return new OutputWriterConvertor<IStructureRecord, Q>(
					new CSVReporter(getRequest().getRootRef().toString(),
							getTemplate(), groupProperties, String.format(
									"%s%s", getRequest().getRootRef(), "")),
					MediaType.TEXT_CSV, filenamePrefix);
		} else if (variant.getMediaType().equals(MediaType.APPLICATION_RDF_XML)) {
			switch (rdfwriter) {
			case stax: {
				return new RDFStaXConvertor<IStructureRecord, Q>(
						new DatasetRDFStaxReporter("", getRequest(),
								getTemplate(), getGroupProperties()),
						filenamePrefix);
			}
			default: { // jena
				return new RDFJenaConvertor<IStructureRecord, Q>(
						new DatasetRDFReporter("", getRequest(),
								variant.getMediaType(), getTemplate(),
								getGroupProperties()), variant.getMediaType(),
						filenamePrefix);
	...
	protected void setGroupProperties(Context context, Request request,
		...
		if (gp == null || gp.length == 0)
			gp = new String[] { "http://www.opentox.org/api/1.1#ChemicalName",
