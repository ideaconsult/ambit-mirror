...
public class BundleChemicalsResource<Q extends IQueryRetrieval<IStructureRecord>> extends AmbitDBQueryResource<Q,IStructureRecord> {
	...
	protected CallableProtectedTask<String> createCallable(Method method,
		...
		try {
			CompoundURIReporter<IQueryRetrieval<IStructureRecord>> r = new CompoundURIReporter<IQueryRetrieval<IStructureRecord>>(getRequest());
	...
	protected Q createUpdateQuery(Method method, Context context,
		...
		if (Method.POST.equals(method) || Method.PUT.equals(method) ) {
			if (idchemical==null) return null;//post/put allowed only on /bundle/{id}/compound level, not on /bundle/id/compound/{idcompound}
		} else {
			if (idchemical!=null) {
				try {
						Integer idnum = new Integer(Reference.decode(idbundle.toString()));
	...
	public IProcessor<Q, Representation> createConvertor(Variant variant)  throws AmbitException, ResourceException {
		...
		if (variant.getMediaType().equals(MediaType.TEXT_URI_LIST)) {
			...
			return new OutputWriterConvertor<IStructureRecord, Q>(
					new SDFReporter<Q>(template,getGroupProperties(),changeLineSeparators),ChemicalMediaType.CHEMICAL_MDLSDF,filenamePrefix);
		} else if (variant.getMediaType().equals(ChemicalMediaType.CHEMICAL_MDLMOL)) {
				return new OutputWriterConvertor<IStructureRecord, Q>(
						new SDFReporter<Q>(new Template(),getGroupProperties(),true,changeLineSeparators),ChemicalMediaType.CHEMICAL_MDLMOL,filenamePrefix);			
		} else if (variant.getMediaType().equals(ChemicalMediaType.CHEMICAL_CML)) {
				return new OutputWriterConvertor<IStructureRecord, Q>(
						new CMLReporter<Q>(),ChemicalMediaType.CHEMICAL_CML,filenamePrefix);				
		} else if (variant.getMediaType().equals(ChemicalMediaType.CHEMICAL_SMILES)) {
				return new OutputWriterConvertor<IStructureRecord, Q>(
						new SmilesReporter<Q>(true,getTemplate()),ChemicalMediaType.CHEMICAL_SMILES,filenamePrefix);
		} else if (variant.getMediaType().equals(ChemicalMediaType.CHEMICAL_INCHI)) {
			return new OutputWriterConvertor<IStructureRecord, Q>(
					new SmilesReporter<Q>(false,Mode.InChI,getTemplate()),ChemicalMediaType.CHEMICAL_INCHI,filenamePrefix);
