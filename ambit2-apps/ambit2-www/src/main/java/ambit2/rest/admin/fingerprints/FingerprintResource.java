...
public class FingerprintResource extends
		QueryResource<QueryFingerprints, IFingerprint<FPTable, String>> {
	...
	protected void doInit() throws ResourceException {
		...
		customizeVariants(new MediaType[] { MediaType.TEXT_HTML,
				MediaType.TEXT_URI_LIST, MediaType.TEXT_CSV,
	...
	@Override
	public IProcessor<QueryFingerprints, Representation> createConvertor(
			Variant variant) throws AmbitException, ResourceException {
		if (variant.getMediaType().equals(MediaType.TEXT_URI_LIST)) {
			FingerprintURIReporter reporter = new FingerprintURIReporter(
					getRequest());
			return new OutputWriterConvertor(reporter, MediaType.TEXT_URI_LIST);
		} else if (variant.getMediaType().equals(MediaType.TEXT_CSV)) {
			return new OutputWriterConvertor(new FingerprintsCSVReporter(
					getRequest()), MediaType.TEXT_CSV);
		} else {
			return new OutputWriterConvertor(new FingerprintsCSVReporter(
					getRequest()), MediaType.TEXT_CSV);
		}
	...
	@Override
	protected QueryFingerprints createQuery(Context context, Request request,
		...
		try {
			if (fptype == null)
				...
				fp.setType(FPTable.valueOf(fptype.toString()));
		} catch (Exception x) {
			fp.setType(FPTable.fp1024);
		}
		q.setValue(fp);
		try {
			Object order = getResourceRef(request).getQueryAsForm()
					.getFirstValue("order");
			q.setOrder(order == null ? _order.frequency : _order.valueOf(order
					.toString()));
		} catch (Exception x) {
		}
		return q;
