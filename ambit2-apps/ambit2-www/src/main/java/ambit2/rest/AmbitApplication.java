...
public class AmbitApplication extends FreeMarkerApplication<String> {
	...
	if (attachSubstanceRouter()) {
	    ...
	    router.attach(String.format("%s/{%s}/{%s}", SubstancePropertyResource.substanceproperty,
		    SubstancePropertyResource.topcategory, SubstancePropertyResource.endpointcategory),
	     ...
	     */
	    router.attach(String.format("%s%s", SubstanceResource.substanceID, SubstanceStructuresResource.structure),
		    SubstanceStructuresResource.class);
	    router.attach(String.format("%s%s/{%s}", SubstanceResource.substanceID,
		    SubstanceStructuresResource.structure, SubstanceStructuresResource.compositionType),
		    SubstanceStructuresResource.class);
	    /**
	     * /substance/{id}/studysummary
	     */
	    router.attach(String.format("%s/%s", SubstanceResource.substanceID, SubstanceStudyFacetResource.resource),
		    SubstanceStudyFacetResource.class);

	    /**
	     * /substance/{id}/study
	     */
	    router.attach(String.format("%s%s", SubstanceResource.substanceID, SubstanceStudyResource.study),
		    SubstanceStudyResource.class);

	    /**
	     * /substance/{id}/composition/{type}
	     */
	    router.attach(
		    String.format("%s%s", SubstanceResource.substanceID, SubstanceCompositionResource.composition),
		    SubstanceCompositionResource.class);
	    router.attach(
		    String.format("%s%s", SubstanceResource.substanceID, SubstanceCompositionResource.compositionID),
		    SubstanceCompositionResource.class);
	    ...
	    router.attach(String.format("%s%s", OwnerSubstanceFacetResource.ownerID, SubstanceResource.substance),
		    SubstanceByOwnerResource.class);
	    router.attach(String.format("%s%s", OwnerSubstanceFacetResource.ownerID, DatasetResource.dataset),
		    SubstanceDatasetResource.class);

	    router.attach(
		    String.format("%s%s", OwnerSubstanceFacetResource.ownerID, SubstanceStructuresResource.structure),
		    OwnerStructuresResource.class);

	    router.attach(String.format("%s%s/{%s}", OwnerSubstanceFacetResource.ownerID,
		    SubstanceStructuresResource.structure, SubstanceStructuresResource.compositionType),
		    OwnerStructuresResource.class);
	}
	...
	if (openToxAAEnabled) {
	    /** /algorithm */
	    router.attach(AllAlgorithmsResource.algorithm, createAuthenticatedOpenResource(new AlgorithmRouter(
		    getContext())));
	    /** /model */
	 ...
	 * Directory editorDir = new Directory(getContext(),
	 * "war:///editor/png"); Directory styleDir = new
	 * Directory(getContext(), "war:///style"); Directory jquery = new
	 * Directory(getContext(), "war:///jquery"); Directory scriptsDir = new
	 * Directory(getContext(), "war:///scripts");
	 * 
	 * router.attach("/meta/", metaDir); router.attach("/images/", imgDir);
	 * router.attach("/jmol/", jmolDir); router.attach("/jme/", jmeDir);
	 * router.attach("/ui/png", editorDir); router.attach("/jquery/",
	 * jquery); router.attach("/style/", styleDir);
	...
	if ((identifier == null) || "".equals(identifier) || identifier.indexOf("${") > -1)
	    throw new Exception(String.format("Property %s not set. The web application will be READ ONLY!",
		    identifierKey));
	...
	try {
	    FreeMarkerStatusService.REPORT_LEVEL aa = FreeMarkerStatusService.REPORT_LEVEL.valueOf(getProperty(
		    FreeMarkerStatusService.report_level, ambitProperties));
		...
		aa = FreeMarkerStatusService.REPORT_LEVEL.valueOf(getContext().getParameters().getFirstValue(
