...
public class QMapJSONReporter<Q extends IQueryRetrieval<QMap>> extends
		QueryReporter<QMap, Q, Writer> {
	/**
     * 
     */
	...
	protected List<String> cacheProperty = new ArrayList<String>();
	protected List<String> cacheDataset = new ArrayList<String>();
	...
	public Object processItem(QMap item) throws AmbitException {
		try {
			if (item.getProperty() instanceof SubstanceProperty) {
				SubstanceProperty sproperty = (SubstanceProperty) item
						.getProperty();
				if (cacheProperty.indexOf(sproperty.getRelativeURI()) < 0) {
					cacheProperty.add(sproperty.getRelativeURI());
					propertyReporter.processItem(item.getProperty());
				}
			} else if (cacheProperty.indexOf(Integer.toString(item
					.getProperty().getId())) < 0) {
				cacheProperty.add(Integer.toString(item.getProperty().getId()));
				propertyReporter.processItem(item.getProperty());
			}
			if (item.getDataset().getID() <= 0) {
				if (cacheDataset.indexOf(item.getDataset().getName()) < 0) {
					cacheDataset.add(item.getDataset().getName());
					datasetReporter.processItem(item.getDataset());
				}
			} else if (cacheDataset.indexOf(item.getDataset().getID()) < 0) {
				cacheDataset.add(Integer.toString(item.getDataset().getID()));
				datasetReporter.processItem(item.getDataset());
			}
			...
			getOutput()
					.write(String.format(
							...
							qmapJSON.dataset.jsonname(),
							qmapJSON.URI.jsonname(), dataseturi,
							qmapJSON.featureURI.jsonname(), propertyuri,
							qmapJSON.threshold, item.getActivityThreshold(),
							qmapJSON.threshold, item.getSimilarityThreshold()));
			comma = ",";
		} catch (Exception x) {
			x.printStackTrace();
		}
