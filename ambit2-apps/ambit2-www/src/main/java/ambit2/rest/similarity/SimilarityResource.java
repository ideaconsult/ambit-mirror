...
public class SimilarityResource<Q extends IQueryRetrieval<IStructureRecord>> extends StructureQueryResource<Q> {
	...
	return (dataset_id == null) ? null : String.format("%s%s/%s%s", getRequest().getRootRef(),
		OpenTox.URI.dataset.getURI(), dataset_id, PropertyResource.featuredef);
	...
	csvReporter.setSimilarityColumn(Property.getInstance("metric",
		queryObject == null ? "" : queryObject.toString(), "http://ambit.sourceforge.net"));
	return csvReporter;
    }
	...
	try {
		...
		try {
		    ...
		    this.dataset_id = Reference.decode(getRequest().getAttributes().get(DatasetResource.datasetKey)
			    .toString());
