...
public class QMapSpaceJSONReporter<Q extends IQueryRetrieval<QMapSpace>>
		extends QueryReporter<QMapSpace, Q, Writer> {
	/**
     * 
     */
			...
			getOutput().write(
					String.format(
							"\n{"
									...
									+ "\n\t\"%s\":%f," + "\n\t\"%s\":%f,"
									+ "\n\t\"%s\":%f" + "\n}",
							QMapJSONReporter.qmapJSON.URI.jsonname(),
							cmpuri,
							QMapJSONReporter.qmapJSON.qmap.jsonname(),
							uri,
							QMapJSONReporter.qmapJSON.activity.jsonname(),
							item.getRecord().getRecordProperty(
									item.getQmap().getProperty()),
							QMapJSONReporter.qmapJSON.g2, item.getG2(),
							QMapJSONReporter.qmapJSON.fisher, item.getFisher(),
							QMapJSONReporter.qmapJSON.a, item.getA(),
							QMapJSONReporter.qmapJSON.b, item.getB(),
							QMapJSONReporter.qmapJSON.c, item.getC(),
							QMapJSONReporter.qmapJSON.d, item.getD()));
