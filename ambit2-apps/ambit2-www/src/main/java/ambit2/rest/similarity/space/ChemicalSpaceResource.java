...
public class ChemicalSpaceResource<Q extends IQueryRetrieval<ChemSpaceCell>>
		extends AbstractPairwiseResource<ChemSpaceCell, Q> {
	...
	protected QueryAbstractReporter createHTMLReporter(Dimension d) {
		return null;// new
					// ChemicalSpaceHTMLReporter(getRequest(),DisplayMode.table,(ChemicalSpaceJSONReporter)createJSONReporter());
	}

	@Override
	protected CSVReporter createCSVReporter() {
		CSVReporter csvReporter = new CSVReporter(getRequest().getRootRef()
				.toString(), null, null, String.format("%s%s", getRequest()
				.getRootRef(), getCompoundInDatasetPrefix()));
	...
	protected QueryAbstractReporter createJSONReporter(String callback) {
		return new ChemicalSpaceJSONReporter<IQueryRetrieval<ChemSpaceCell>>(
				getRequest(), getProperty());
	}
	...
	protected Q getQueryById(Integer key) throws ResourceException {
		...
		try {
			q.setThreshold_similarity(Double.parseDouble(form
					.getFirstValue("similarity")));
		} catch (Exception x) {
			// default
		}
		try {
			q.setThreshold_dactivity(Double.parseDouble(form
					.getFirstValue("dactivity")));
		} catch (Exception x) {
			// default
		}
		setPaging(form, q);
	...
	protected Q getQueryById(String key) throws ResourceException {
		if (key.startsWith(DatasetStructuresResource.QR_PREFIX)) {
			...
			Form form = getResourceRef(getRequest()).getQueryAsForm();
			try {
				q.setMaxPoints(Integer.parseInt(form
						.getFirstValue("resolution")));
			} catch (Exception x) {
				// default
			}
			setPaging(form, q);
