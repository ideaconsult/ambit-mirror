...
public class SubstanceCategoryProperty extends
		CatalogResource<EffectRecord<String, IParams, String>> {
	...
	protected Iterator<EffectRecord<String, IParams, String>> createQuery(
		...
		try {
			Object topcategory = Reference.decode(request.getAttributes()
					.get(SubstancePropertyResource.topcategory).toString());
			Object endpointcategory = Reference
					.decode(request.getAttributes()
							.get(SubstancePropertyResource.endpointcategory)
							.toString());
			if (topcategory == null || endpointcategory == null)
				throw new ResourceException(Status.CLIENT_ERROR_BAD_REQUEST);
			I5_ROOT_OBJECTS category;
			try {
				category = I5_ROOT_OBJECTS.valueOf(endpointcategory.toString()
						.replace("_SECTION", ""));
			} catch (Exception x) {
				throw new ResourceException(Status.CLIENT_ERROR_BAD_REQUEST,
						x.getMessage());
			...
			if (!category.getTopCategory().equals(topcategory.toString())) {
				throw new ResourceException(Status.CLIENT_ERROR_BAD_REQUEST,
						String.format("Expected /property/%s/%s",
								category.getTopCategory(), category.name()));
			}
			effects = new ArrayList<EffectRecord<String, IParams, String>>();

			if (category.getEndpoints() != null
					&& category.getEndpoints().length > 0)
	...
	public IProcessor<Iterator<EffectRecord<String, IParams, String>>, Representation> createJSONConvertor(
		...
		if (variant.getMediaType().equals(MediaType.APPLICATION_JAVASCRIPT)) {
			...
			EffectRecordJSONReporter cmpreporter = new EffectRecordJSONReporter(
					getRequest(), jsonpcallback);
			return new StringConvertor(cmpreporter,
					MediaType.APPLICATION_JAVASCRIPT, filenamePrefix);
		} else { // json by default
			EffectRecordJSONReporter cmpreporter = new EffectRecordJSONReporter(
					getRequest(), null);
			return new StringConvertor(cmpreporter, MediaType.APPLICATION_JSON,
					filenamePrefix);
		}
	}
