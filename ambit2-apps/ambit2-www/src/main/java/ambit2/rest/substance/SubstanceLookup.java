...
public class SubstanceLookup<Q extends IQueryRetrieval<SubstanceRecord>, T extends SubstanceRecord>
	...
	protected enum _query_subtype {
		protocol, experiment, owner, uuid
	};
	...
	protected Q createQuery(Context context, Request request, Response response) throws ResourceException {
		...
		try {
			Object prm = request.getAttributes().get("type");
			type = _query_type.valueOf(prm.toString());
		} catch (Exception x) {
		...
		case related: {
			 ...
			 */
			if (idchemical != null) {
				CompositionRelation composition = new CompositionRelation(null,
						new StructureRecord(idchemical, -1, null, null), null);
		...
		case study: {
			...
			try {
				Object prm = request.getAttributes().get("subtype");
				subtype = _query_subtype.valueOf(prm.toString());
			} catch (Exception x) {
				throw new ResourceException(Status.CLIENT_ERROR_BAD_REQUEST);
			}

			Object subsubtype = request.getAttributes().get("subsubtype");
			switch (subtype) {
			case protocol: {
				/**
