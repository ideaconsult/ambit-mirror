...
public class UIResource extends FreeMarkerResource {
	...
	public void configureTemplateMap(Map<String, Object> map, Request request,
		...
		try {
			Object bundleURI = OpenTox.params.bundle_uri.getFirstValue(request
		...
		try {
			map.put(AMBITConfig.custom_structurequery.name(),
					((AmbitApplication) getApplication()).getCustomStructureQuery());
		} catch (Exception x) {}
		...
		try {
			map.put(AMBITConfig.custom_logo.name(),
					((AmbitApplication) getApplication()).getCustomLogo());
		} catch (Exception x) {}		
		try {
	...
	protected Representation getRepresentation(Variant variant)
		...
		case layout: { // ketcher specifics
			String smiles = getRequest().getResourceRef().getQueryAsForm()
					.getFirstValue("smiles");
			if ((smiles != null) && !"".equals(smiles.trim())) {
				IStructureRecord record = new StructureRecord();
				record.setContent(smiles);
				record.setFormat(MOL_TYPE.CSV.name());
				StructureEditorProcessor processor = new StructureEditorProcessor(
						page.name());
				try {
