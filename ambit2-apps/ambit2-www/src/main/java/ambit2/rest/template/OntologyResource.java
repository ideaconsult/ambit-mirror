...
public class OntologyResource extends QueryResource<IQueryRetrieval<Property>, Property> {
    ...
    @Override
    public IProcessor<IQueryRetrieval<Property>, Representation> createConvertor(Variant variant)
	...
	if (variant.getMediaType().equals(MediaType.APPLICATION_RDF_XML)
	    ...
	    return new RDFJenaConvertor<Property, IQueryRetrieval<Property>>(
		    new TemplateRDFReporter<IQueryRetrieval<Property>>(getRequest(), variant.getMediaType(),
			    isRecursive()), variant.getMediaType(), filenamePrefix);
	    ...
	    return new OutputWriterConvertor(new OntologyHTMLReporter(getRequest(), !isRecursive() ? DisplayMode.table
		    : DisplayMode.singleitem), MediaType.TEXT_HTML);
    }
    ...
    @Override
    protected Property createObjectFromHeaders(Form requestHeaders, Representation entity) throws ResourceException {
	...
	if (entry instanceof Dictionary) {
	    ...
	    throw new ResourceException(Status.CLIENT_ERROR_BAD_REQUEST, String.format("Expected %s instead of %s",
		    Dictionary.class.getName(), entry.getClass().getName()));
    };
