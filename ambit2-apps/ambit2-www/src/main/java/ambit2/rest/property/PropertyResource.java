...
public class PropertyResource extends
		QueryResource<IQueryRetrieval<Property>, Property> {
	/**
	...
	public final static String featuredefID = String.format("/{%s}",
			idfeaturedef);
	...
	protected IQueryRetrieval<Property> createQuery(Context context,
		...
		Object key = request.getAttributes().get(
		...
		if (key != null)
			try {
				if (record == null)
					record = new StructureRecord();
				record.setIdstructure(Integer.parseInt(Reference.decode(key
						.toString())));
				chemicalsOnly = false;
			} catch (Exception x) {
			}
		...
		try {
			...
			if (o == null) {
				IQueryRetrieval<Property> qf = getFreeTextQuery(getContext(),
				...
				if (key != null) {
					...
					try {
						q.setCondition(condition == null ? StringCondition
								.getInstance(StringCondition.C_EQ)
								: StringCondition.getInstance(condition));
					} catch (Exception x) {
						q.setCondition(StringCondition
								.getInstance(StringCondition.C_EQ));
					}
					return q;
				}
				...
				if (key != null) {
					...
					try {
						q.setCondition(condition == null ? StringCondition
								.getInstance(StringCondition.C_REGEXP)
								: StringCondition.getInstance(condition));
					} catch (Exception x) {
						q.setCondition(StringCondition
								.getInstance(StringCondition.C_REGEXP));
	...
	protected IQueryRetrieval<Property> getFreeTextQuery(Context context,
			Request request, Response response) throws ResourceException {
		Form form = request.getResourceRef().getQueryAsForm();
	...
	protected RDFObjectIterator<Property> createObjectIterator(
		...
		try {
			RDFPropertyIterator iterator = new RDFPropertyIterator(reference,
					mediaType, getRequest().getResourceRef().toString()) {
				@Override
	...
	protected RDFObjectIterator<Property> createObjectIterator(
		...
		RDFPropertyIterator iterator = new RDFPropertyIterator(entity,
				entity.getMediaType()) {
			@Override
			protected boolean skip(Resource newEntry) {
				return super.skip(newEntry) || isFeatureSource(newEntry);
	...
	protected AbstractUpdate createUpdateObject(Property entry)
		...
		if (getRequest().getAttributes().get(idfeaturedef) != null)
			try {
				entry.setId(Integer.parseInt(getRequest().getAttributes()
						.get(idfeaturedef).toString()));
			} catch (Exception x) {
				entry.setId(-1);
			}
