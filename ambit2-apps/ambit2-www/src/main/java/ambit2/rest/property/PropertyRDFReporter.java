 ...
 * @param <Q>
 */
public class PropertyRDFReporter<Q extends IQueryRetrieval<Property>> extends
		QueryRDFReporter<Property, Q> {
	...
	public static Individual addToModel(OntModel jenaModel, Property item,
		...
		if ((uriReporter == null) || (uriReporter.getBaseReference() == null)
			...
			if (item.getClazz() == Dictionary.class) {
				feature = jenaModel.createIndividual(id,
						featureType.getOntClass(jenaModel));
			} else
				feature = jenaModel.createIndividual(featureType
		...
		if (uri.indexOf("/model/") > 0)
			feature.addOntClass(OT.OTClass.ModelPredictionFeature
					.getOntClass(jenaModel));
		...
		if ((uri.indexOf("http://") < 0) && (uri.indexOf("https://") < 0)) {
			...
			if (_type.Algorithm.equals(item.getReference().getType())) {
				...
				source = jenaModel.createIndividual(uri,
						OT.OTClass.Algorithm.createOntClass(jenaModel));
				feature.addProperty(
						OT.OTProperty.hasSource.createProperty(jenaModel),
						source);
				...
				uri = String.format("%s/dataset/%s",
						uriReporter.getBaseReference(), Reference.encode(uri));
				Resource src = jenaModel.createResource(uri);
				feature.addProperty(
						OT.OTProperty.hasSource.createProperty(jenaModel), src);
			} else {
				feature.addProperty(
						OT.OTProperty.hasSource.createProperty(jenaModel), uri);
			}
			...
			for (PropertyAnnotation a : item.getAnnotations())
				try {
					PropertyAnnotationRDFReporter.annotation2RDF(a, jenaModel,
							feature, uriReporter.getBaseReference().toString());
				} catch (Exception x) {
					x.printStackTrace();
				}
	...
	public static String hasSourceURI(Property item,
			QueryURIReporter<Property, IQueryRetrieval<Property>> uriReporter) {
		...
		if ((uri.indexOf("http://") < 0) && (uri.indexOf("https://") < 0)) {
			...
			if (_type.Algorithm.equals(item.getReference().getType())) {
				return String.format("%s/algorithm/%s",
						uriReporter.getBaseReference(), Reference.encode(uri));
			} else if (_type.Model.equals(item.getReference().getType())) {
				return String.format("%s/model/%s",
						uriReporter.getBaseReference(), Reference.encode(uri));
			} else if (_type.Feature.equals(item.getReference().getType())) {
				return String.format("%s/feature/%s",
						uriReporter.getBaseReference(), Reference.encode(uri));

			} else if (_type.Dataset.equals(item.getReference().getType())) {
				// this seems to confuse everybody's else parsers ...
				return String.format("%s/dataset/%s",
						uriReporter.getBaseReference(), Reference.encode(uri));
