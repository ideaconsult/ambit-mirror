...
public class AlgorithmJSONReporter extends AlgorithmURIReporter {
    ...
    public AlgorithmJSONReporter(Request request) {
	this(request, null);
    }

    public AlgorithmJSONReporter(Request request, String jsonpcallback) {
	super(request);
	...
	try {
	    ...
	    output.write(String.format(format, uri, algorithm.getId(),
		    algorithm.getName() == null ? "" : algorithm.getName(), algorithm.getContent() == null ? ""
			    : algorithm.getContent(), algorithm.getEndpoint() == null ? "" : algorithm.getEndpoint(),
		    algorithm.getDescription() == null ? "" : algorithm.getDescription(),
		    algorithm.getFormat() == null ? "" : algorithm.getFormat(),
		    algorithm.getImplementationOf() == null ? "" : algorithm.getImplementationOf(), algorithm
			    .isDataProcessing(), algorithm.isRequiresDataset(), algorithm.isSupervised(), algorithm
			    .getRequirement() == null ? "" : algorithm.getRequirement(), b.toString()));
	    comma = ",";
	} catch (IOException x) {
