...
public class ChartResource extends ProtectedResource {
	...
	protected void doInit() throws ResourceException {
		...
		thumbnail = false;
		try {
			thumbnail = Boolean.parseBoolean(getParams().getFirstValue("thumbnail"));
		} catch (Exception x) {
			thumbnail = false;
		}
		...
		h = thumbnail ? 100 : 400;
		try {
			w = Integer.parseInt(getParams().getFirstValue("w"));
		} catch (Exception x) {
		}
		try {
			h = Integer.parseInt(getParams().getFirstValue("h"));
		} catch (Exception x) {
		}
		...
		logY = false;
		try {
			logX = Boolean.parseBoolean(getParams().getFirstValue("logX"));
		} catch (Exception x) {
		}
		try {
			logY = Boolean.parseBoolean(getParams().getFirstValue("logY"));
		} catch (Exception x) {
		}
		...
		maxX = null;
		try {
			minX = Double.parseDouble(getParams().getFirstValue("minX"));
		} catch (Exception x) {
			minX = null;
		}
		try {
			maxX = Double.parseDouble(getParams().getFirstValue("maxX"));
		} catch (Exception x) {
			maxX = null;
		}
		...
		legend = false;
		try {
			legend = Boolean.parseBoolean(getParams().getFirstValue("legend"));
		} catch (Exception x) {
			legend = false;
		}
		...
		param = null;
		try {
			param = getParams().getFirstValue("param");
		} catch (Exception x) {
			param = null;
		}
	...
	protected BufferedImage createImage() throws ResourceException {
		...
		try {
			...
			for (String p : property)
				reader.process(new Reference(p));
