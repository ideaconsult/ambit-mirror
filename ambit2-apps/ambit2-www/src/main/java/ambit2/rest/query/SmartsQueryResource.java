 ...
 * 
 */
public class SmartsQueryResource extends
	...
	protected String getDefaultTemplateURI(Context context, Request request,
		...
		// getRequest().getRootRef(),PropertyResource.featuredef,Reference.encode(textSearch))
				...
				String.format("%s%s/%s%s", getRequest().getRootRef(),
						OpenTox.URI.dataset.getURI(), dataset_id,
						PropertyResource.featuredef);
		// String.format("riap://application/dataset/%s%s",dataset_id,PropertyResource.featuredef);
	}

	protected IQueryRetrieval<IStructureRecord> getScopeQuery(Context context,
	...
	protected IQueryRetrieval<IStructureRecord> createQuery(Context context,
		...
		try {
			...
			IQueryRetrieval<IStructureRecord> freetextQuery = getScopeQuery(
			...
			try {
				...
				if (filter != null) {
					...
					filterBySubstance = "yes".equals(filter)
							|| "on".equals(filter) || "true".equals(filter);
				}
			} catch (Exception x) {
				filterBySubstance = false;
			}
				...
				if (key == null) {
					if (freetextQuery == null)
						throw new ResourceException(
								Status.CLIENT_ERROR_BAD_REQUEST, "Empty smarts");
					else {
			...
			if (form.getFirstValue("fp1024") != null)
				try {
					fp1024_screenining = Boolean.parseBoolean(form
							.getFirstValue("fp1024"));
				} catch (Exception x) {
			...
			if (form.getFirstValue("sk1024") != null)
				try {
					sk1024_screenining = Boolean.parseBoolean(form
							.getFirstValue("sk1024"));
				} catch (Exception x) {
			...
			if (form.getFirstValue("atomproperties") != null)
				try {
					read_atomprops = Boolean.parseBoolean(form
							.getFirstValue("atomproperties"));
				} catch (Exception x) {
					read_atomprops = true;
				}
			...
			try {
				...
				Object datasetid = request.getAttributes().get(
						DatasetResource.datasetKey);
				if (datasetid != null) {
					QueryDatasetByID scope = new QueryDatasetByID();
				...
				if ((folders != null) && (folders.length > 0)) {
					ChemicalByQueryFolder qa = new ChemicalByQueryFolder(
							folders);
					QueryCombinedStructure qc = new QueryCombinedStructure();
					qc.add(query);
					qc.setChemicalsOnly(true);
	...
	protected String inchi2smiles(String inchi) {
		try {
			...
			if ((c == null) || (c.getAtomContainer() == null)
				...
				return null;
			return SmilesGenerator.generic().aromatic()
					.create(c.getAtomContainer());
		} catch (Exception x) {
			return null;
		}
	}
	...
	protected String name2smiles(String name) {
		try {
			Logger.getLogger("uk.ac.cam.ch.wwmm.opsin").setLevel(Level.ERROR);
