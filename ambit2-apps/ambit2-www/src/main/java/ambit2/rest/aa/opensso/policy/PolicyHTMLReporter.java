...
public class PolicyHTMLReporter extends PolicyURIReporter {
    ...
    public PolicyHTMLReporter(Request ref, boolean collapsed) {
	super(ref);
	...
	try {
		...
		for (CallablePolicyCreator._method matcher : CallablePolicyCreator._method.values())
		    output.write(String.format("<input type=CHECKBOX title='%s' name='%s' %s>%s",
			    matcher.getDescription(), matcher.name(), matcher.name(), matcher.name()));
		output.write("</td>");
		output.write("</tr>");
	...
	try {
	    ...
	    output.write(String.format("<a href='%s/%s/%s'>Back</a>", getRequest().getRootRef(),
		    AdminResource.resource, OpenSSOPoliciesResource.resource));
	    output.write("</p>");
	    output.write("</p>");
