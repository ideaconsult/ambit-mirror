 ...
 * http://cactus.nci.nih .gov/chemical/structure/"structure
...
public class CompoundLookup extends StructureQueryResource<IQueryRetrieval<IStructureRecord>> {
	...
	protected IQueryRetrieval<IStructureRecord> createQuery(Context context, Request request, Response response)
		...
		try {
			...
			throw new ResourceException(Status.CLIENT_ERROR_BAD_REQUEST,
					String.format("Invalid structure identifier", id));
		}
		id = null;
		try {
		...
		if (isInchiKey(text))
			...
			try {
				...
				Object q = CallableQueryProcessor.getQueryObject(new Reference(url), getRequest().getRootRef(),
						getApplication().getContext(), getCookies(), getAgent(),
	...
	protected QueryField getTextQuery(Property property, boolean caseSensitive, boolean retrieveProperties,
			String value) {
	...
	protected QueryFieldMultiple getMultiTextQuery(Property property, boolean caseSensitive, boolean retrieveProperties,
			String[] value) {
		 ...
		 * property = Property.getEINECSInstance(); q_by_name.setCaseSensitive(false);}
		 * else if((cas+einecs)==value.length) q_by_name.setCaseSensitive(false); else
		 * if (inchi>0) q_by_name.setCaseSensitive(false); else
	...
	protected Template createTemplate(Context context, Request request, Response response) throws ResourceException {
		 ...
		 * 
		 * String[] featuresURI = new String[r.length]; for (int i=0; i < r.length;i++)
		 * featuresURI[i] = String.format("%s/%s?sameas=%s",getRequest
	...
	/*
	 * public IAtomContainer isInChI(String inchi) throws Exception { if ((inchi!=
	 * null) && inchi.startsWith(AmbitCONSTANTS.INCHI)) { InChIGeneratorFactory f =
	 * InChIGeneratorFactory.getInstance(); InChIToStructure c
	 * =f.getInChIToStructure(inchi, SilentChemObjectBuilder.getInstance()); if
	 * ((c==null) || (c.getAtomContainer()==null) ||
	 * (c.getAtomContainer().getAtomCount()==0)) throw new
	 * Exception("Invalid InChI"); searchType = _searchtype.inchi; return
	 * c.getAtomContainer(); } else return null; }
	 */
