 ...
 * @param <Q>
 */
public class CompoundJSONReporter<Q extends IQueryRetrieval<IStructureRecord>>
		extends CSVReporter<Q> {
	/**
	 * 
	 */
	...
	public CompoundJSONReporter(Template template, Request request,
			String urlPrefix, Boolean includeMol, String jsonpCallback) {
		this(template, null, null, null, request, urlPrefix, includeMol,
				jsonpCallback);
	}

	public CompoundJSONReporter(Template template, Profile groupedProperties,
			String[] folders, SubstanceEndpointsBundle[] bundles,
			Request request, String urlPrefix, Boolean includeMol,
			String jsonpCallback) {
		super(request.getRootRef().toString(), template, groupedProperties,
				folders, bundles, urlPrefix, includeMol);
		this.jsonpCallback = JSONUtils.jsonSanitizeCallback(jsonpCallback);

		propertyJSONReporter = new PropertyJSONReporter(request);
		hilightPredictions = request.getResourceRef().getQueryAsForm()
				.getFirstValue("model_uri");
	}
	...
	protected void configureCollectionProcessors(String baseURI) {
		if (folders != null && folders.length > 0) {
			 ...
			 */
				...
				@Override
				protected IStructureRecord processDetail(
						IStructureRecord master, IFacet<String> detail)
						throws Exception {
					master.clearFacets();
		...
		if (bundles != null && bundles.length > 0) {
			 ...
			 */
				...
				@Override
				protected IStructureRecord processDetail(
						IStructureRecord master, BundleRoleFacet detail)
						throws Exception {
					master.clearFacets();
					master.addFacet(detail);
					return master;
	...
	protected List<Property> template2Header(Template template,
		...
		Collections.sort(h, new Comparator<Property>() {
			public int compare(Property o1, Property o2) {
				return Integer.toString(o1.getId()).compareTo(
						Integer.toString(o2.getId()));
	...
	public Object processItem(IStructureRecord item) throws AmbitException {
		try {
			...
			if (item.getInchi() != null)
				builder.append(String.format(",\t\t\"%s\":\"%s\"\n", "inchi",
						JSONUtils.jsonEscape(item.getInchi())));
			if (item.getFormula() != null)
				builder.append(String.format(",\t\t\"%s\":\"%s\"", "formula",
						JSONUtils.jsonEscape(item.getFormula())));
			...
			for (int j = 0; j < header.size(); j++) {
				...
				if (value instanceof Double) {
					if (((Double) value).isNaN())
						...
						builder.append(String.format(Locale.ENGLISH,
								"\t\t\"%s\":%6.3f", key, (Double) value));
				} else if (value instanceof Integer)
					builder.append(String.format("\t\t\"%s\":%d", key,
							(Integer) value));
				else if (value instanceof Long)
					builder.append(String.format("\t\t\"%s\":%l", key,
							(Long) value));
			...
			if (facets != null)
				for (IFacet facet : facets) {
					...
					builder.append(String.format("\t\t{\"%s\":%d}",
							facet.getValue() == null ? "" : facet.getValue(),
							facet.getCount()));
					delimiter = ",";
				}
			builder.append("\n\t\t],");
			...
			if ((item instanceof SubstanceRecord)
					&& ((SubstanceRecord) item).getRelatedStructures() != null) {
				SubstanceRecord substance = (SubstanceRecord) item;
				builder.append(String.format("\n\t%s:[\n", JSONUtils
						.jsonQuote(jsonCompound.composition.jsonname())));
				List<CompositionRelation> composition = substance
				...
				for (int j = 0; j < composition.size(); j++) {
					...
					if (j > 0)
						builder.append(",\n");
					String component = "{}";
					if (cr.getSecondStructure() != null
							&& cr.getSecondStructure().getIdchemical() > 0) {
		...
		if (facets != null)
			for (IFacet facet : facets)
				if (facet instanceof BundleRoleFacet) {
					builder.append(delimiter);
					builder.append("\n\t\t");
					builder.append(facet.toJSON(propertyJSONReporter
							.getRequest().getRootRef().toString(), null));
					delimiter = ",";
				}
		builder.append("\n\t\t}");
	}
