...
public class DatasetResource<Q extends IQueryRetrieval<IStructureRecord>> extends DatasetStructuresResource<Q> {
		...
		if ((datasetsURI != null) && (datasetsURI.length > 0)) {
			...
			try {
				ChemicalByDataset cd = new ChemicalByDataset(
						new Integer(getRequest().getAttributes().get(datasetKey).toString()));
				qc.setScope(cd);
			} catch (Exception x) {
			}
			...
			for (String datasetURI : datasetsURI) {
				...
				try {
					qc.add(new ChemicalByDataset(
							new Integer(vars.get(DatasetStructuresResource.datasetKey).toString())));
				} catch (Exception x) {
	...
	protected Representation copyDatasetToQueryResultsTable(Form form, boolean clearPreviousContent)
		...
		if ((queryResultsID != null) && (queryResultsID > 0)) {
			callable = new CallableQueryResultsCreator(form, getRequest().getRootRef(), getContext(),
		...
		try {
			...
			if (form.size() != 0) {
				if ((compounds == null) || (compounds.length == 0))
					throw new ResourceException(Status.CLIENT_ERROR_BAD_REQUEST,
							String.format("Missing %s parameters", OpenTox.params.compound_uris));
				try {
	...
	public void onUpdateSuccess() throws Exception {
		if (isAAEnabled())
			deletePolicy(null, getToken() == null ? null : getToken().toString());
	}
