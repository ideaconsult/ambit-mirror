...
public class CliOptions {
	...
	protected void parseCommands() throws Exception {
		...
		if (in == null) throw new IOException("Error loading " + apiConfig);
		try {
			    			...
			    			while (i.hasNext()) {
		    				    ...
		    				    for (_commandmode subcommand : _commandmode.values()) {
				    				...
				    				if (params!=null) {
				   					...
				   					while (n.hasNext()) {
				   						 String param = n.next();
				   						 prm.append(" -d "+param.replace(":","")+"="+params.get(param).get("value").asText()+"");
				   						 prmrest.append(d+param.replace(":","")+"="+Reference.encode(params.get(param).get("value").asText()));
				   						 d = "&";
				   					 }
	...
	protected void setConfig(String config) {
		if (config!=null) {
			...
			if (!this.configFile.exists()) config = null;
		}
		if (config==null) {
	...
	public void setCommand(String command) throws Exception {
		...
		if (node==null) throw new InvalidCommand(command==null?"(Not specified)":command);
		else if (node instanceof ObjectNode) {
			this.command = (ObjectNode)node;
		} else throw new InvalidCommand(command);
	}
	...
	protected void printCommand(String cmd, ObjectNode cmdDef, boolean json,Writer out) throws IOException, JsonMappingException , JsonGenerationException{
			 ...
			 for (_commandmode subcommand : _commandmode.values()) {
				 ...
				 if (params!=null) {
					 ...
					 while (i.hasNext()) {
						 String key = i.next();
						 out.write(d+key.replace(":","")+"={"+params.get(key).get("type").asText()+"}");
						 example.append(d+key.replace(":","")+"="+Reference.encode(params.get(key).get("value").asText()));
						 d = "&";
					 }
					 out.write("\n\tExample\n");
					 out.write(example.toString());
	...
	protected void printCommand2html(String cmd, ObjectNode cmdDef, boolean json,Writer out, boolean runExample) throws IOException, JsonMappingException , JsonGenerationException{
		 //ObjectWriter writer = mapper.defaultPrettyPrintingWriter();
		    // ***IMPORTANT!!!*** for Jackson 2.x use the line below instead of the one above:
		 if (json) {
			 ObjectWriter writer = mapper.writer().withDefaultPrettyPrinter();
				 ...
				 if (runExample) {
					 ...
					 if (node!=null && node.asBoolean()) {
						 ...
						 try {
							 ...
							 out.write("</pre>");
							 out.write("</div>");
							 out.write("\n<h6>Retrieved in <a id='time_"+cmd+"_"+subcommand+"'>"+ timeelapsed +"</a> msec</h6>");	 
						 }
					 }
				 }
