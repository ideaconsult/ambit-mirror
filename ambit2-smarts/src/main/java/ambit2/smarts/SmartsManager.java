...
/**
 * Implements utilities needed for handling searching with SMARTS utilizing JDK
 * library. Some complex manipulations are encapsulated here (such as recursive
 * smarts)
 */

public class SmartsManager {
	int recursiveStrategy = 1; // 0 - subqueries, 1 - recursive substructure
								// search (default one)
	...
	// Pointers to the new atoms that substitute the original recursive atoms in
	// the
	...
	// The atoms connected to the recursive atoms and the corresponding bond
	// orders.
	// The bonds between two recursive atoms are not put in topLayers.
	// The atom indexes for such bonds are stored separately in bondRecAt1 and
	// bondRecAt2
	TopLayer topLayers[];
	List<Integer> bondRecAt1 = new ArrayList<Integer>();
	List<Integer> bondRecAt2 = new ArrayList<Integer>();
	List<IBond> bondRecBo = new ArrayList<IBond>(); // It contains actually a
													// bond expression
	List<Integer> compFrags = new ArrayList<Integer>(); // The indexes of the
														// query frags which are
														// in a component
	boolean fragMaps[][];
	int components[];
	...
	public void setQuery(String smQuery) {
		...
		if (parser.hasRecursiveSmarts) {
			...
			if (recursiveStrategy == 0) // This strategy for recursive atoms is
		 ...
		 * SmartsHelper sh = new SmartsHelper(); System.out.println("container "
		 * + sh.toSmarts(parser.container)); System.out.println("Fragments: ");
		 * 
		 * for (int i = 0; i < parser.fragments.size(); i++) { String frag =
		 * sh.toSmarts(parser.fragments.get(i)); System.out.println( "  Comp = "
		 * + parser.fragmentComponents.get(i).intValue() + "  NA = " +
		 * parser.fragments.get(i).getAtomCount() + "  NB = " +
		 * parser.fragments.get(i).getBondCount() + "  " + frag ); }
		 */
	...
	boolean mappingIn(IAtomContainer target) throws Exception {
		if (FlagUseCDKIsomorphismTester) {
			boolean res = uit.isSubgraph(target, query);
			return (res);
	...
	boolean fragmentSearchIn(IAtomContainer target) throws Exception {
		...
		if (noComponentsSpecified) // There are no zero level brackets i.e. each
									// fragments maps anywhere
		{
		...
		for (int i = 0; i < parser.fragments.size(); i++) {
			if (parser.fragmentComponents.get(i).intValue() == 0) {
				...
				if (!res)
					return (false); // This non-component fragment is not found
									// in the target
			} else
		...
		for (int i = 0; i < compFrags.size(); i++) {
			components[i] = parser.fragmentComponents.get(
					compFrags.get(i).intValue()).intValue();
	...
	void printComponentFrags() // A test helper procedure
	{
		...
		for (int i = 0; i < compFrags.size(); i++) {
			...
			String frag = sh.toSmarts(parser.fragments.get(compFrags.get(i)
					.intValue()));
	...
	boolean checkComponentMapings() {
		...
		// where all elements are different from -1 (i.e. each fragment is
		// mapped onto a target component)
	...
	boolean addComponentToCombination(int a[], Stack<int[]> comb) {
		...
		if (n == -1)
			return (true); // No elements equal to -1 were found i.e. all
							// fragments are mapped
	...
	int[] setComponentInCombination(int n, int a[]) {
		...
		for (int i = n + 1; i < components.length; i++) {
			if (components[i] == components[n]) {
				if (fragMaps[i][a[n]]) // i-th fragment maps target component
										// a[n]
					a[i] = a[n];
				else
					return (null);
			} else
				break; // It is applied because the fragments for a given
						// component are sequential
		}
	...
	void getRecursiveAtoms() {
		...
		for (int i = 0; i < query.getAtomCount(); i++) {
			if (query.getAtom(i) instanceof SmartsAtomExpression) {
				SmartsAtomExpression sa = (SmartsAtomExpression) query
						.getAtom(i);
	...
	void analyseRecursiveAtoms() {
		// Checks whether:
		// (1) all recursive expressions are separated only with OR logical
		// operations and
		// (2) all recursive expressions are after the last ';' operand
		// (3) all expression after last ';' are recursive smarts
		//
		// For example [...;$(C),$(CC),$(CCO)] will be equivalent to the
		// combination
		...
		for (int i = 0; i < recAtoms.size(); i++) {
			...
			for (int j = lastLoAnd + 1; j < sa.tokens.size(); j++) {
				SmartsExpressionToken token = (SmartsExpressionToken) sa.tokens
						.get(j);
	...
	void genSubQueries() {
		...
		// Base structure is obtaned from the original structure by removing
		// recAtoms
		gatBaseStructure();
	...
	void expandBaseStruct(IQueryAtomContainer struct, int pos) {
		...
		IQueryAtomContainer fragment = (IQueryAtomContainer) sa.recSmartsContainers
				.get(curComb[pos]);
		...
		for (int i = 0; i < tl.atoms.size(); i++)
			addBond(struct, fragment.getFirstAtom(), (IAtom) tl.atoms.get(i),
					(IBond) tl.bonds.get(i));
	...
	void addBond(IQueryAtomContainer container, IAtom atom0, IAtom atom1,
			IBond bond) {
		IBond newBond = null;
	...
	boolean recursiveSearchIn0(IAtomContainer target) throws Exception {
		...
		// The recursive smarts is replaced with equivalent list of ordinary
		// smarts (sub queries)
		// obtained from all possible combinations by varing of recursive
		// expressions
		//
		// IMPORTANT NOTE !!!
		// It is not allways possible to substitute the original recursive
		// SMARTS with
		// a list of ordinary smarts.
		// Success is always guaranteed when only OR operations (',') are used
		// with the
		// recursive conditions.
		for (int i = 0; i < subQueryList.size(); i++) {
			IQueryAtomContainer subQuery = subQueryList.get(i);
			boolean res = uit.isSubgraph(target, subQuery);
			if (res)
				return (true);
		}
	...
	public IAtomContainerSet getAllIsomorphismMappings(IAtomContainer target)
			throws Exception {
		IAtomContainerSet s = MoleculeTools
				.newAtomContainerSet(SilentChemObjectBuilder.getInstance());
		if (query == null)
			return (s);
		...
		if (FlagUseCDKIsomorphismTester) {
			...
			if (query.getAtomCount() == 1) {
				...
				for (int i = 0; i < v.size(); i++) {
					IAtomContainer c = MoleculeTools
			...
			for (List<RMap> bondMap : maps) {
				IAtomContainer c = generateFullIsomorphismMapping(bondMap,
						target, query);
				s.addAtomContainer(c);
			}
			return (s);
	...
	public List<IAtom> getFirstPosAtomMappings(IAtomContainer target)
			throws Exception {
		if (query == null)
			return (null);
		if (FlagSetSmartsDataForTarget)
	...
	public List getBondMappings(IAtomContainer target) throws Exception {
		...
		List bondList;
		try {
			bondList = uit.getSubgraphMaps(target, query);
		...
		for (int i = 0; i < recAtoms.size(); i++) {
			...
			for (int j = 0; j < vRecCon.size(); j++) {
				...
				if (FlagUseCDKIsomorphismTester)
					...
					v = getFirstPosAtomMappings_CurrentIsoTester(target,
							vRecCon.get(j));
			...
			/*
			 * for (int j = 0; j < vRecCon.size(); j++)
			 * System.out.println("recusive match: " +
			 * recAtoms.get(i).recSmartsStrings.get(j) + "  "+
	...
	List<IAtom> getFirstPosAtomMappings(IAtomContainer target,
			IAtomContainer recQuery) throws Exception {
		List bondMaps;

		if (recQuery.getAtomCount() == 1)
			return (getAtomMappingsFor1AtomQuery(target, recQuery));
		else if (recQuery.getAtomCount() == 2)
			return (getFirstPosAtomMappingsFor2AtomQuery(target, recQuery));
		else {
			bondMaps = uit.getSubgraphMaps(target, recQuery);
			return (getAtomMapsFromBondMaps(bondMaps, target, recQuery));
		}

	}

	List<IAtom> getAtomMappingsFor1AtomQuery(IAtomContainer target,
			IAtomContainer recQuery) {
	...
	List<IAtom> getFirstPosAtomMappingsFor2AtomQuery(IAtomContainer target,
			IAtomContainer recQuery) {
		List<IAtom> atomMaps = new ArrayList<IAtom>();
		if (recQuery.getBondCount() == 0)
			return (atomMaps); // The two atoms must be connected otherwise
				...
				for (int j = 0; j < ca.size(); j++)
					if (qAtom1.matches((IAtom) ca.get(j))) {
						...
						if (bo != null)
							if (qBond.matches(bo)) {
								atomMaps.add(at);
								// It is not needed to register all maps
	...
	IAtomContainerSet getAllIsomorphismMappingsFor2AtomQuery(
			IAtomContainer target, IAtomContainer recQuery) {
		IAtomContainerSet s = MoleculeTools
				.newAtomContainerSet(SilentChemObjectBuilder.getInstance());

		if (recQuery.getBondCount() == 0)
			return (s); // The two atoms must be connected otherwise substr.
				...
				for (int j = 0; j < ca.size(); j++)
					if (qAtom1.matches((IAtom) ca.get(j))) {
						...
						if (bo != null)
							if (qBond.matches(bo)) {
								IAtomContainer c = MoleculeTools
										.newMolecule(SilentChemObjectBuilder
												.getInstance());
								c.addAtom(at);
	...
	List<IAtom> getAtomMapsFromBondMaps(List bondMapping,
			IAtomContainer target, IAtomContainer recQuery) {
	 ...
	 **/
	public List<IAtom> generateFullAtomMapping(List bondMapList,
			IAtomContainer target, IAtomContainer queryStr) {
		// The query must contain at least 3 atoms and 2 bonds.
		...
		for (int i = 0; i < queryStr.getAtomCount(); i++) {
			// To obtain the partner target atom, two bonds of the query atom
			// are used
			...
			if (conAtList.size() > 1) {
				...
				atMaps[i] = getBondsCommonAtom(target.getBond(tBoNum0),
						target.getBond(tBoNum1));
			}
		}
		...
		for (int i = 0; i < queryStr.getAtomCount(); i++) {
			...
			List<IAtom> conAtList = queryStr.getConnectedAtomsList(qAt);
			if (conAtList.size() == 1) {
				// The neighbor atoms was processed (mapped) in the previous
				// cycle
	...
	public IAtomContainer generateFullIsomorphismMapping(List bondMapList,
			IAtomContainer target, IAtomContainer queryStr) {
		List<IAtom> v = generateFullAtomMapping(bondMapList, target, queryStr);
		IAtomContainer ac = MoleculeTools.newMolecule(SilentChemObjectBuilder
				.getInstance());
		for (int i = 0; i < v.size(); i++)
			ac.addAtom(v.get(i));
	...
	IAtom getBondsCommonAtom(IBond b1, IBond b2) {
		if (b1.contains(b2.getAtom(0)))
			return (b2.getAtom(0));
		else if (b1.contains(b2.getAtom(1)))
			return (b2.getAtom(1));
		return (null);
	}
	...
	public List<List<IAtom>> getMappingsForGroup(IAtomContainer target, IQueryAtomContainer groupQuery, SmartsConst.SSM_MODE FlagSSMode)
	{
		switch (FlagSSMode)
		{
		//TODO	
	...
	public void testTopLayers() {
		for (int i = 0; i < topLayers.length; i++) {
			...
			for (int j = 0; j < tl.atoms.size(); j++)
				System.out.print(SmartsHelper
						.atomToString((SMARTSAtom) tl.atoms.get(j))
						+ " "
