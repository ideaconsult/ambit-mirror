...
public class SmartsToChemObject extends
		DefaultAmbitProcessor<IQueryAtomContainer, IAtomContainer> {

	private static final long serialVersionUID = -5893878673124511317L;

	public static final String markProperty = "MARKED_AB";

	// When this flag is true aromatic bonds are forced between any two aromatic
	// atoms
	public boolean forceAromaticBondsAlways = false;

	// When this flag is true aromatic bonds are forced between atoms which are
	// not part of a ring
	// e.g. the SMARTS Query describes a part of aromatic cycle "cccc"
	// But this flag will not force the bond aromaticity for the single bond in
	// the biphenyl.
	...
	// Work variables which are set by functions analyzeSubExpressionsFromLowAnd
	// and getExpressionAtomType
	int mSubAtomType, mSubAromaticity, mCurSubArom, mRecCurSubArom,
			mSubAtomCharge, mCurSubAtCharge;
	...
	/**
	 * Maximal possible atom container from this query is generated. This
	 * container may be fragmented since some original atoms or bonds could be
	 * removed.
	 * 
	 * Following rule is applied: each query atom/bond which is a simple
	 * expression is converted to a normal IAtom/IBond object Also some
	 * heuristics are applied in order to get information from more complicated
	 * atoms expressions (when possible).
	 * 
	 * @param query
		...
		for (int i = 0; i < query.getBondCount(); i++) {
			...
			if (b != null) {
				...
				if ((mFlagConfirmAromaticBond)
						&& (ats[0].getFlag(CDKConstants.ISAROMATIC))
						&& (ats[1].getFlag(CDKConstants.ISAROMATIC))) {
					if (forceAromaticBondsAlways) {
						b.setFlag(CDKConstants.ISAROMATIC, true);
					} else {
						// Ring info is used to determine whether this bond must
						// have aromatic flag
						if (ringSet == null) {
	 ...
	 * container is always fully connected as the original query. The "removed"
	 * atoms and bonds are marked with a set property This object is useful for
	 * visualization purposes.
	 */
	public IAtomContainer extractAtomContainerFullyConnected(
			IQueryAtomContainer query, IRingSet ringSet) {
		if (query == null)
			return null;
		// Converting the atoms
		List<IAtom> atoms = new ArrayList<IAtom>();
	...
	public IQueryAtomContainer convertKekuleSmartsToAromatic(
			IQueryAtomContainer query, IRingSet ringSet) throws Exception {
		List<IRingSet> rs = getMaxCondensedRingSystems(ringSet);
		if (rs.size() == 0)
			return (query);

		for (int i = 0; i < rs.size(); i++) {
			IQueryAtomContainer qac = getCondensedFragmentFromRingSets(query,
					rs.get(i));
	 ...
	 * defined atom type. When this is impossible null object is returned
	 * 
	 **/
	public IAtom smartsExpressionToAtom(SmartsAtomExpression a) {
		...
		// 3. If only one expression defines an atom type then this is assigned
		// as the
		// result atom type
		...
		for (int i = 0; i < subs.size(); i++) {
			...
			// System.out.println("    mSubAtomType = " + mSubAtomType +
			// "  mSubAromaticity = " + mSubAromaticity + "  mSubAtomCharge = "
			// + mSubAtomCharge);
			if (mSubAtomType != -1) {
				if (atType == -1)
					atType = mSubAtomType;
	...
	public void analyzeSubExpressionsFromLowAnd(SmartsAtomExpression atExp,
	...
	public int getExpressionAtomType(SmartsAtomExpression atExp,
			SmartsAtomExpression sub) {
		...
		for (int i = 1; i <= n; i++) {
			...
			for (int k = pos[i - 1] + 1; k < pos[i]; k++) {
				...
				// When given primitive defines an atom type it must not be
				// negated
				switch (seTok.type) {
				...
				case SmartsConst.AP_Recursive:
					int recExpAtType = getRecursiveExpressionAtomType(atExp,
							seTok.param);
					if (recExpAtType > 0)
						if (!FlagNot) {
	...
	public int getRecursiveExpressionAtomType(SmartsAtomExpression atExp, int n) {
		// Recursive call is performed. That is way the values of the global
	...
	public IBond smartsExpressionToBond(SmartsBondExpression b) {
		...
		List<SmartsBondExpression> subs = getSubExpressions(b, SmartsConst.LO
				+ SmartsConst.LO_ANDLO);
		int boType = -1;
		int isArom = -1;
		...
		if (boType != -1) {
			...
			IBond bond = SilentChemObjectBuilder.getInstance().newInstance(
					IBond.class);
	...
	public List<SmartsBondExpression> getSubExpressions(
			SmartsBondExpression b, int separator) {
		List<SmartsBondExpression> v = new ArrayList<SmartsBondExpression>();
	...
	public void analyzeSubExpressionsFromLowAnd(SmartsBondExpression boExp,
		...
		List<SmartsBondExpression> sub_subs = getSubExpressions(sub,
				SmartsConst.LO + SmartsConst.LO_OR);
	...
	public int getExpressionBondType(SmartsBondExpression boExp,
			SmartsBondExpression sub) {
		// 'sub' expression is represented only by HI_AND and NOT operations
		mCurSubBoArom = -1;
		...
		int n = 0;
		for (int i = 0; i < sub.tokens.size(); i++) {
			if (sub.tokens.get(i).intValue() == SmartsConst.LO
					+ SmartsConst.LO_AND) {
				n++;
				pos[n] = i;
			}
		...
		for (int i = 1; i <= n; i++) {
			...
			for (int k = pos[i - 1] + 1; k < pos[i]; k++) {
				...
				// When given primitive defines a bond type it must not be
				// negated
				switch (seTok) {
	...
	/**
	 * This function finds all condensed ring systems and returns a vector where
	 * each element of is a maximal condensed system
	 */
	List<IRingSet> getMaxCondensedRingSystems(IRingSet ringSet) {
		List<IRingSet> v = new ArrayList<IRingSet>();
		int n = ringSet.getAtomContainerCount();
		if (n == 0)
			return (v); // empty container is returned

		IRingSet workRS = MoleculeTools.newRingSet(SilentChemObjectBuilder
				.getInstance());
		workRS.add(ringSet);
	...
	IRingSet getCondenzedRingsTo(IAtomContainer startAC, IRingSet ringSet) {
		...
		while (curRing < condRS.getAtomContainerCount()) {
			// Adding all neighbors of the currentRing which are not in rs
			// container
			IRingSet rsConnected = ringSet.getConnectedRings((IRing) condRS
					.getAtomContainer(curRing));
			...
			curRing++; // The cycle will stop if curRing is the last one and no
						// other rings are added
		}

		return (condRS);
	}

	IQueryAtomContainer getCondensedFragmentFromRingSets(
			IQueryAtomContainer query, IRingSet rs) {
		// All atoms and bonds from each ring are added
		// Check for duplication of atoms and bonds is done

		IQueryAtomContainer qac = new QueryAtomContainer(query.getBuilder());
		for (int i = 0; i < rs.getAtomContainerCount(); i++) {
