...
import ambit2.stats.transforms.densityestimation.KDensity1D;
import ambit2.stats.transforms.densityestimation.KNormal;
import ambit2.stats.transforms.densityestimation.Kernel;
...
public class DataCoverageDensity extends DataCoverageDescriptors {
	...
	public DataCoverageDensity() {
		super();
		kdeList = null;
		appDomainMethodType.setDensityMode();
	}

	/**
	 * @param mode
	 */
	public DataCoverageDensity(int mode) {
		//TODO bandwidth mode
		super(mode);
		kdeList = null;
		appDomainMethodType.setDensityMode();		
	...
	protected void doEstimation(Vector points, int np, int nd) {
	    ...
	    KDensity1D akde;
	    for (int d = 0; d< nd; d++) {
	        for (int p = 0; p < np; p++) A[p] = ((double[])points.get(p))[d];
	        akde = null;
	        double datarange[] = new double[2];
	        if (Tools.ExtendedInterval(A,np, datarange))  {
	        	mindata[d] = datarange[0];
