...
import ambit2.io.IAmbitResultWriter;
...
/**
 * Default implementation of {@link ambit2.io.batch.IBatch}. See examples for {@link ambit2.database.search.DbSimilarityByFingerprintsReader},
 * {@link ambit2.database.search.DbSimilarityByAtomenvironmentsReader}, {@link ambit2.database.search.DbExactSearchReader},
 * {@link ambit2.ui.actions.BatchAction} descendants. See exapmle at {@link ambit2.database.writers.DbSubstanceWriter}.
...
public class DefaultBatchProcessing implements IBatch {
	...
	/* (non-Javadoc)
     * @see ambit2.io.batch.IBatch#processRecord(java.lang.Object)
     */
    ...
    /* (non-Javadoc)
     * @see ambit2.io.batch.IBatch#getBatchStatistics()
     */
    public IBatchStatistics getBatchStatistics() {
        return statistics;
    }
    /* (non-Javadoc)
     * @see ambit2.io.batch.IBatch#readRecord()
     */
    ...
    /* (non-Javadoc)
     * @see ambit2.io.batch.IBatch#writeRecord(java.lang.Object)
     */
        ...
        if (object instanceof IChemObject)
	        try {
	        	if (writer instanceof MDLWriter) {
	        		...
	        		Iterator p =((IChemObject)object).getProperties().keySet().iterator();
	        		while (p.hasNext()) {
	        			Object key = p.next();
	        			if (CDKConstants.ALL_RINGS.equals(key)) continue;
	        			if (AmbitCONSTANTS.AMBIT_IDSTRUCTURE.equals(key)) continue;
	        			if (CDKConstants.SMALLEST_RINGS.equals(key)) continue;
	        			if (AmbitCONSTANTS.AMBIT_IDSUBSTANCE.equals(key)) continue;
	        			props.put(key,((IChemObject)object).getProperties().get(key));
    ...
    /* (non-Javadoc)
     * @see ambit2.io.batch.IBatch#getConfig()
     */
    public IBatchConfig getConfig() {
        return configFile;
    }
    /* (non-Javadoc)
     * @see ambit2.io.batch.IBatch#setConfig(ambit2.io.batch.IBatchConfig)
     */
	...
	/* (non-Javadoc)
     * @see ambit2.io.batch.IBatch#closeProcessor()
     */
