 ...
 * A property "LC50" would be transformed to a {@link ambit2.data.experiment.TemplateField} with name "LC50" from a template {@link ambit2.data.experiment.DSSToxLC50Template}
 * and the experimental Hashtable will store the pair ("LC50",template.getField("LC50")).<br>
 * The template can be acessed by {@link #getStudy()} {@link ambit2.data.experiment.Study#getTemplate()}.<br>
 * The class also has a slot to store information of a QSAR model {@link ambit2.data.model.Model} in case
 * the data imported is a QSAR model.<br>
 * All the processing is done automatically (asking the user if necessary) if a {@link ambit2.io.AmbitSettingsListener} is assigned to {@link org.openscience.cdk.io.IChemObjectReader}.<br>
 * Used to hold data for {@link ambit2.io.MolPropertiesIOSetting}. <br>
 * See {@link ambit2.data.molecule.MolPropertiesFactory} for examples. 
...
public class MolProperties extends PropertyTranslator implements Serializable {
	...
	public MolProperties(Map properties) {
		super();
