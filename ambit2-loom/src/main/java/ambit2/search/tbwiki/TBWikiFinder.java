...
public class TBWikiFinder extends AbstractFinder<String, IStructureRecord> implements  ICallBack<String, QuerySolution> {
	...
	public void process(String term, QuerySolution row) {
		...
		if ("Has_CAS".equals(key))  label = Property.opentox_CAS;
		else if ("Has_InChI".equals(key))    label = Property.opentox_InChI; 
		else if ("Has_InChIKey".equals(key))  label = Property.opentox_InChIKey;
		else if ("Has_Smiles".equals(key))  label = Property.opentox_SMILES;
		else if ("Has_PubChem_CID".equals(key))  label = Property.opentox_Pubchem;
		else if ("Has_ChEBI_Id".equals(key))  label = Property.opentox_ChEBI;
		else if ("Has_ChEMBL_Id".equals(key))  label = Property.opentox_ChEMBL;
		else if ("Has_ChemSpider_Id".equals(key))  label = Property.opentox_ChemSpider;
		else if ("Has_DrugBank_Id".equals(key))  label = Property.opentox_DrugBank;
		else if ("Has_KEGG_Id".equals(key))  label = Property.opentox_KEGG;
		else if ("Has_SigmaAldrich_Id".equals(key))  label = Property.opentox_SigmaAldrich;
		else if ("Has_Leadscope_Id".equals(key))  label = Property.opentox_LS;
		...
		if ("http://www.w3.org/2000/01/rdf-schema#label".equals(key)) {
			...
			pname = object.asResource().getLocalName();
			if ("Subject".equals(pname)) return;
			label = node.asResource().getURI();
			value = "Yes";
