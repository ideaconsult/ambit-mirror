 ...
 * and allows the user to edit structure query. See example for {@link ambit.database.data.AmbitDatabaseToolsData}.
...
public class StructureQueryPanel extends JPanel implements Observer {
	...
	public void setMolecule() {
		try {
		    ...
		    ISetOfMolecules m = chemModel.getSetOfMolecules();
			...
			chemModel.setSetOfMolecules(molecules);
...
//         fire a change so that the view gets updated
	...
	protected ISetOfMolecules getMoleculeForEdit(IAtomContainer atomContainer) throws Exception {
		...
		if (molecules.length == 0) {
			molecules = new org.openscience.cdk.Molecule[1];
		...
		ISetOfMolecules m =  new org.openscience.cdk.SetOfMolecules();
	...
	public void updateMolecule() {
    	...
    	if (molecules == null) molecules = DefaultChemObjectBuilder.getInstance().newSetOfMolecules();
