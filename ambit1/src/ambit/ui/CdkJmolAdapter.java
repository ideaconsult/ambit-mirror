...
public class CdkJmolAdapter extends JmolAdapter {
	...
	public Object openBufferedReader(String name, BufferedReader bufferedReader) {
		...
		if (chemFile == null)
			...
			Enumeration it= atomContainer.atoms();
			while(it.hasMoreElements()) {
				IAtom atom=(IAtom)it.nextElement();
				try {
	...
	public int getAtomSetCount(Object clientFile) {
		...
		if (clientFile instanceof IAtomContainer) {
			...
			IChemModel[] models = ChemFileManipulator.getAllChemModels((IChemFile)clientFile);
			bcLogger.debug("Found a IChemFile... good");
			bcLogger.debug("Found #sets: " + models.length);
	...
	class AtomIterator extends JmolAdapter.AtomIterator {
		...
		AtomIterator(IChemFile chemFile) {
			...
			if (modelCount > 0) {
				containers = ChemModelManipulator.getAllAtomContainers(models[0]);
				if (containers.length>0) 
					atomCount = ((IAtomContainer)containers[0]).getAtomCount();
			}
			iatom = 0;
			imodel = 0;
			icontainer = 0;
		...
		public boolean hasNext() {
			...
			if (iatom == atomCount) {
				icontainer++;
				if (icontainer == containers.length) {
					imodel++;
				...
				try{
					atomCount = containers[icontainer].getAtomCount();
			...
			atom = containers[icontainer].getAtomAt(iatom++);
...
//			      bcLogger.debug("insertion code: " + getInsertionCode());
	...
	class BondIterator extends JmolAdapter.BondIterator {
		...
		BondIterator(IChemFile chemFile) {
			...
			modelCount = models.length;
			if (modelCount > 0) {
				containers = ChemModelManipulator.getAllAtomContainers((IChemModel)models[0]);
				if (containerCount > 0) 
					bondCount = containers[0].getBondCount();
			}
			ibond = 0;
			imodel = 0;
			icontainer = 0;
		...
		public boolean hasNext() {
			if (ibond == bondCount) {
				...
				if (icontainer >= containerCount) {
					imodel++;
					if (imodel >= models.length) return false;
					containers = ChemModelManipulator.getAllAtomContainers(models[0]);
					containerCount = containers.length;
					if (containerCount == 0) return false;
					icontainer = 0;
				}
				bondCount = containers[icontainer].getBondCount();
				ibond = 0;
				return hasNext();
			}
			// FIXME: skip all bonds which are messy, but this should really be examined more closely
			bond = containers[icontainer].getBondAt(ibond++);
			boolean isOK = bond.getAtomCount() == 2 && (bond.getAtomAt(0) != null && bond.getAtomAt(1) != null);
			if (!isOK) {
		...
		public Object getAtomUniqueID1() {
			return (bond.getAtomCount() == 2) ? bond.getAtomAt(0) : null;
		}

		public Object getAtomUniqueID2() {
			return (bond.getAtomCount() == 2) ? bond.getAtomAt(1) : null;
		}
	...
	public class StructureIterator extends JmolAdapter.StructureIterator {
		...
		StructureIterator(IChemFile chemFile) {
			...
			IChemModel[] models = ChemFileManipulator.getAllChemModels(chemFile);
			if (models.length == 0) return;
			IAtomContainer[] containers = ChemModelManipulator.getAllAtomContainers(models[0]);
			if (containers.length == 0) return;
			if (!(containers[0] instanceof PDBPolymer)) return;
			PDBPolymer polymer = (PDBPolymer)containers[0];
