...
protected IMolecule getMolecule() {
	...
	public void roundtrip(boolean checkAromaticity) {
		...
		try {
			 ...
			 */
			IAtomContainer[] molecules = ChemFileManipulator.getAllAtomContainers(chemFile);
			assertEquals(1,molecules.length);
			
			assertTrue(UniversalIsomorphismTester.isIsomorph(m,molecules[0]));
			
			Fingerprinter fp = new Fingerprinter();
			assertEquals(fp.getFingerprint(m),fp.getFingerprint(molecules[0]));
			...
			*/
			...
			List<IAtomType> atnew = getAtomTypes(atm, (IMolecule) molecules[0]);
	...
	protected List<IAtomType> getAtomTypes(IAtomTypeMatcher atm, IMolecule m) throws CDKException {
		...
		Enumeration e = m.atoms();
		while (e.hasMoreElements()) {
			IAtomType atomType = atm.findMatchingAtomType(m,(IAtom)e.nextElement());
