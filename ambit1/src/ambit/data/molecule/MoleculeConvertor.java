	...
	private void atomOrder(IAtomContainer ac) {
		...
		for (int i = 0; i < bonds.length; i++) {
			for (int j = 0; j < bonds[i].getAtomCount(); j++) {
				int n = ac.getAtomNumber(bonds[i].getAtomAt(j));
				aorder[n] ++;
			}	
		}
...
//	    	System.out.print("["+(i+1)+"] "+atomsOrder[i]+"\t");
	...
	public void createGamut() {
		...
		Hashtable ht = mfa.getFormulaHashtable();
		ht.remove("H");
		Integer c;
		String element;
		...
		for (int i = 0; i < ac.getBondCount(); i++) {
			order = bonds[i].getOrder();
			if (order == 1) bondsTypeNo[0]++;
			else if (order == 1.5) bondsTypeNo[1]++;
			else if (order == 2) bondsTypeNo[2]++;
			else if (order == 3) bondsTypeNo[3]++;
			bondsTypeNo[4]++;
		}
		if (spanningTree == null)
			spanningTree = new SpanningTree();
		spanningTree.buildSpanningTree((org.openscience.cdk.AtomContainer)ac);
	...
	public String createSMILES() {
		...
		if (gen == null)
			gen = new SmilesGenerator(DefaultChemObjectBuilder.getInstance());
	...
	public void setMolecule(IAtomContainer mol) {
		this.molecule = mol;
		if (spanningTree != null) spanningTree.clear();
		disconnected = false;
		rings = DefaultChemObjectBuilder.getInstance().newRingSet();
	...
	public static int getCyclicBondsBySSSR(IMolecule mol) {
	    ...
	    for (int i =0; i < mol.getBondCount(); i++)  
	        mol.getBondAt(i).setFlag(CDKConstants.VISITED,false);
	    ...
	    for (int i=0; i < rs.getAtomContainerCount(); i++) {
	        ...
	        for (int j=0; j < ring.getBondCount(); j++) {
	            ...
	            ring.getBondAt(j).setFlag(CDKConstants.VISITED,true);
	        }    
	    }
	    int cb = 0; //cyclic bonds count
	    for (int i =0; i < mol.getBondCount(); i++)  
	        if (mol.getBondAt(i).getFlag(CDKConstants.VISITED)) cb++;
