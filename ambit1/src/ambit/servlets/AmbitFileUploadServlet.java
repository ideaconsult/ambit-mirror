...
/**
 * An {@link ambit.servlets.AmbitServlet} descendant to perform online batch processing.
 *  The users uploads a file, which is processed by various {@link ambit.processors.IAmbitProcessor}s. <br>
 *  The result is sent back to the user as a file of specified type.
 *  Valid parameters are:
 *  caslookup - CAS -> structure lookup  {@link ambit.database.processors.CASSmilesLookup} <br>
 *  smiles - SMILES generation {@link ambit.processors.structure.SmilesGeneratorProcessor }<br>
 *  fingerprints - Fingerprint generation {@link ambit.processors.structure.FingerprintGenerator}<br> 
 *  fingerprints - Fingerprint profile generation {@link ambit.processors.structure.FingerprintProfileGenerator}<br>
 *  verhaar - Classification by Verhaar scheme  {@link ambit.processors.toxtree.ToxTreeProcessor} with {@link verhaar.VerhaarScheme}<br>
 *  cramer - Classification by Cramer rules {@link ambit.processors.toxtree.ToxTreeProcessor} with {@link toxTree.tree.cramer.CramerRules}<br>
 *  XlogP - XlogP calculation {@link ambit.processors.descriptors.CalculateDescriptors} with {@link org.openscience.cdk.qsar.descriptors.molecular.XLogPDescriptor}<br>
 *  readCAS - Reads CAS numbers from database {@link ambit.database.processors.FindUniqueProcessor} and {@link ambit.database.processors.ReadCASProcessor}<br>
 *  readSMILES - Reads SMILES from database {@link ambit.database.processors.FindUniqueProcessor} and {@link ambit.database.processors.ReadSMILESProcessor}<br>
 *  readNAME - Reads Chemical names from database {@link ambit.database.processors.FindUniqueProcessor} and {@link ambit.database.processors.ReadNameProcessor}<br>
 *  readALIAS - Reads other identifiers from database {@link ambit.database.processors.FindUniqueProcessor} and {@link ambit.database.processors.ReadAliasProcessor}<br>
...
public class AmbitFileUploadServlet extends AmbitServlet {
	...
	protected void process(Connection connection, HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
        	...
        	if (calcDescriptors) pc.addProcessor(new CalculateDescriptors(descriptors,null));
     ...
     throws Exception {
			...
			if (reader != null) {
				...
				DefaultBatchProcessing batch = new DefaultBatchProcessing(
					...
					public void start() throws ambit.io.batch.BatchProcessingException {
						if (writer instanceof HTMLTableWriter)
							((HTMLTableWriter) writer).printHeader();
						setRecordsLimit(1000);
						super.start();
					};
					public void finish() throws ambit.io.batch.BatchProcessingException {
						super.finish();
						if (writer instanceof HTMLTableWriter) {
     ...
     throws FileNotFoundException, IOException {
			...
			if (reader != null) {
				...
				SmilesGenerator g = new SmilesGenerator(builder);
