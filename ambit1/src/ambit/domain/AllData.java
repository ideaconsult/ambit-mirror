 ...
 */
//TODO remove IReadData
public class AllData extends CompoundsList {
...
//		if (yresidual != null) yresidual.clear();
	...
	public void setData(Descriptor d, String value) {
		...
		case (IColumnTypeSelection._ctRowID): {
			...
			((Compound)(super.getItem(nPoints-1))).setOrderInModel(o);
			return;
		}
		case (IColumnTypeSelection._ctCAS): {
			((Compound) super.getItem(nPoints-1)).setCAS_RN(value);			
			return;
		}
		case (IColumnTypeSelection._ctSMILES): {
			((Compound) super.getItem(nPoints-1)).setSMILES(value);			
			return;
		}
		case (IColumnTypeSelection._ctChemName): {
			((Compound) super.getItem(nPoints-1)).setName(value);			
			return;
		}
	...
	public AmbitObject getItem(int i) {
		if (p==null) p = new AmbitPoint();
		p.setCompound(((Compound) super.getItem(i)));
	...
	/* (non-Javadoc)
	 * @see ambit.data.AmbitList#addItem(ambit.data.AmbitObject)
	 */
	public int addItem(AmbitObject entry) {
		try {
			if (entry instanceof AmbitPoint) {
				...
				int r = addRow(point.getCompound(),
	...
	*/
	...
	/* (non-Javadoc)
	 * @see ambit.io.IReadData#addRow(ambit.data.descriptors.DescriptorsList, java.util.List)
	 */
	...
	public int addRow(Compound c, DescriptorsList dlist,Hashtable values, boolean removeMatched) {
		int r = addRow(c);
		Descriptor d;
		int n = dlist.size();
		if (n > values.size()) n = values.size();
...
//				if (equation == null) equation = new Vector();
	...
	/* (non-Javadoc)
	 * @see ambit.data.molecule.CompoundsList#createNewItem()
	 */
	public AmbitObject createNewItem() {
		return new AmbitPoint(new Compound());
	}
