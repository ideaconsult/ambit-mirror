...
public class TransformFilter implements Serializable {
     ...
     * 
     */
    private static final long serialVersionUID = 373435065193751882L;
    // protected
	...
	return FMinVector[index];
    }

    public double getMaxValue(int index) {
	return FMaxVector[index];
    }

    public double getDescriptorsRankThreshold() {
	return FDescriptorsRankThreshold;
    }
    ...
    public boolean InitializeFilter(Matrix points, int inputNo, int outputNo, int[] rowsToProcess) throws Exception {

	if (rowsToProcess == null)
	    return InitializeFilter(points, inputNo, outputNo);
	else {
    ...
    /**
     * Transforms points from original to transformed space only nPoints with
     * indices stored in pointsToProcess are evaluated
     * 
     * @param points
     * @param pointsToProcess
     * @return a new Matrix with transformed points
     */
    public Matrix TransformPoints(Matrix points, int[] pointsToProcess) {
	if (pointsToProcess == null)
     ...
     * 
     * @return indices of most important descriptors ( those with rank >
     *         FDescriptorsRankThreshold )
     */
    ...
    int getInputDescriptorNumber() {
	return FInputDescriptorNumber;
    }

    int getOutputDescriptorNumber() {
	return FInputDescriptorNumber;
    }
    ...
    /*
     * property InputDescriptorNumber : int read FInputDescriptorNumber write
     * setInputDescriptorNumber; property OutputDescriptorNumber : int read
     * FOutputDescriptorNumber write setOutputDescriptorNumber; property
     * MinValue[index : integer] : double read getMinValue; property
     * MaxValue[index : integer] : double read getMaxValue; property
     * TransformError : int[] read FTransformError;
     */
}
