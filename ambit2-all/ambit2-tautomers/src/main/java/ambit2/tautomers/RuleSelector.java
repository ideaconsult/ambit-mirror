...
public class RuleSelector {
	protected RSM selectionMode = RSM.RANDOM;
	protected int ruleNumberLimit = 10;
	protected int limitFuzziness = 1; // This is used for fuzzines in the limit
										// handling
	...
	public RSM getSelectionMode() {
		return selectionMode;
	}

	public void setSelectionMode(RSM selectionMode) {
	...
	public void setLimitDiffForSwitchToCombinatorial(
			int limitDiffForSwitchToCombinatorial) {
	...
	public List<IRuleInstance> selectRules(TautomerManager tman,
	...
	private List<IRuleInstance> selectRandom(TautomerManager tman,
			List<IRuleInstance> ruleList) {
		List<IRuleInstance> selected = new ArrayList<IRuleInstance>();

		if ((ruleNumberLimit + limitDiffForSwitchToCombinatorial) <= ruleList
				.size())
		...
		} else // selectionOrder = TautomerConst.RULE_ORDER_BY_SIZE
		{
			...
			if (rlist.size() < ruleNumberLimit) {
				// 1-3 rules are not enough thus some 1-5 rules will be added as
				// well
				selected.addAll(rlist);
				rlist = getRules(ruleList, 5);

				if (selected.size() + rlist.size() < ruleNumberLimit) {
					// 1-3 and 1-5 rules together are not enough thus some 1-7
					// rules will be added as well
					selected.addAll(rlist);
	...
	public static RuleSelector getDefaultSelectorAll() {
		...
		selector.setSelectionMode(RSM.ALL);
		return selector;
	}

	public static RuleSelector getDefaultSelectorRandom() {
		RuleSelector selector = new RuleSelector();
		selector.setSelectionMode(RSM.RANDOM);
		return selector;
	}
