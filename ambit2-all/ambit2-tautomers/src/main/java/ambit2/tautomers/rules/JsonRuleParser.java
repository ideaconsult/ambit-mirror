...
public class JsonRuleParser {
	private String error = null; // This is a work variable used by the
									// functions:
									// extractInt/Double/StringKeyword
	...
	/*
	 * import java.io.File; import java.io.RandomAccessFile; import
	 * java.net.URL;
	 * 
	 * public static void main(String agrs[]) throws Exception {
	 * System.out.println("*****************"); JsonRuleParser jrp = new
	 * JsonRuleParser(); URL resource =
	 * jrp.getClass().getClassLoader().getResource(
	 * "ambit2/tautomers/energy-rules.json"); File file = new
	 * File(resource.getFile());
	 * 
	 * RandomAccessFile f = new RandomAccessFile(file,"r"); long length =
	 * f.length(); int n = 0; while (f.getFilePointer() < length) { n++; String
	 * line = f.readLine(); System.out.println(line); } f.close(); }
	 */
	...
	public EnergyCorrection parseEnergyCorrection(JsonNode node) {
		...
		for (int i = 1; i <= 7; i++) {
			...
			if (!acNode.isMissingNode()) {
				...
				if (ac.errors == null)
					enCorr.atomConditions.put(new Integer(i - 1), ac); // 1-base
																		// -->
																		// 0-base
																		// indexing
				else {
				...
				for (Entry<Integer, AtomCondition> entry : eCorrection.atomConditions.entrySet()) {
					...
					if (cond.errors != null)
						for (String err : cond.errors)
							addError("ENERGY_RULES[" + (i + 1) + "], " + "ENERGY_CORRECTION[" + (k + 1) + "], " + "ATOM"
									+ (atomIndex + 1) + "_CONDITIONS error: " + err);
				}
			}
