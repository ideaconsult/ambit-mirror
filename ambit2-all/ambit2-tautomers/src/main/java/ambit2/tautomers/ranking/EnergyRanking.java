...
public class EnergyRanking 
{
	...
	public double calculateRank(TautomerIncrementStep incStep , IAtomContainer tautomer) throws Exception
	{
		...
		for (int i = 0; i < incStep.usedRuleInstances.size(); i++)
		{
			...
			List<EnergyRule> eRules = getEnergyRuleByName(ri.rule.name);
			if (eRules == null)
				continue;
			
			//System.out.println("Rule: " + ri.rule.name);
			
			for (EnergyRule eRule : eRules)  //It is possible to have several energy rules for a given tautomer rule
			{	
	...
	List<EnergyRule> getEnergyRuleByName(String rname)
	{
		List<EnergyRule> found_rules = new ArrayList<EnergyRule>();
		for (EnergyRule r : rules)
			if (r.ruleName.equals(rname))
			{	
				found_rules.add(r);
			}
		if (found_rules.isEmpty())
			return null;
		else 
			return found_rules;
	}
