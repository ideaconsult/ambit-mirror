...
public class StoredQueryTableModel extends ResultSetTableModel
		implements ISelectableRecords, ISortableColumns, IFilteredColumns {
	/**
	 * 
	 */
	...
	protected IQueryRetrieval[] queryFields = new IQueryRetrieval[] { FIELDS.CONSENSUSLABEL.getQuery(),
			FIELDS.DATASET.getQuery(), FIELDS.STRUCTURELABEL.getQuery() };
	...
	public StoredQueryTableModel(boolean chemicalsOnly) {
		...
		queryMolecule.setUseCache(true);
		queryFieldExecutor = new QueryExecutor<RetrieveField>();
		queryFieldExecutor.setUseCache(true);
	...
	public IStructureRecord getRecord(int rowIndex, int columnIndex) {
		try {
			if (workingRecord == null)
				workingRecord = new StructureRecord(-1, -1, null, null);
	...
	public Object getValueAt(int rowIndex, int columnIndex) {
		try {
			...
			// selected
			case 0:
				return records.getBoolean(AbstractStructureQuery.FIELD_NAMES.selected.toString());
			case 1:
				return getAtomContainer(record);
			case 2:
				return records.getFloat(AbstractStructureQuery.FIELD_NAMES.metric.toString());
			default: {
				int offset = getColumnCount() - FIELDS.values().length;
				if (columnIndex >= offset) {
					return getLabel(record, columnIndex - offset);
				} else if (columnIndex < (getColumnCount() - 1)) {
					return (fields == null) ? "" : getField(record, ((Property) fields.getElementAt(columnIndex - 3)));
				} else
					return getStatsAt(record);
	...
	/*
	 * public Object getValueAt(int rowIndex, int column) { int columnIndex = column
	 * +1; if (records == null) return "N/A"; else try { int type =
	 * records.getMetaData().getColumnType(columnIndex); records.first();
	 * records.relative(rowIndex);
	 * 
	 * if (records.getMetaData().getColumnName(columnIndex).equals(
	 * "uncompress(structure)")) { if (type == java.sql.Types.VARBINARY) {
	 * InputStream s = records.getBinaryStream(columnIndex); IAtomContainer m =
	 * null; if ("SDF".equals(records.getString("format"))) m =
	 * MoleculeTools.readMolfile(new InputStreamReader(s)); else m =
	 * MoleculeTools.readCMLMolecule(new InputStreamReader(s)); s = null; return m;
	 * } else {
	 * 
	 * return MoleculeTools.readCMLMolecule(records.getString(columnIndex)); }
	 * 
	 * } else switch (type) { case java.sql.Types.INTEGER: return
	 * records.getInt(columnIndex); case java.sql.Types.BOOLEAN: return
	 * records.getBoolean(columnIndex); case java.sql.Types.TINYINT: return
	 * records.getBoolean(columnIndex); case java.sql.Types.BIT: return
	 * records.getBoolean(columnIndex); case java.sql.Types.CHAR: return
	 * records.getString(columnIndex); case java.sql.Types.TIMESTAMP: return
	 * records.getDate(columnIndex); case java.sql.Types.VARCHAR: return
	 * records.getString(columnIndex); default: { System.out.println(type); return
	 * records.getString(columnIndex); } } } catch (CDKException x) {
	 * x.printStackTrace(); return null; } catch (SQLException x) {
	 * x.printStackTrace(); return rowIndex + " " + x.getMessage(); } catch
	 * (Exception x) { x.printStackTrace(); return null; } }
	 */
	@Override
	...
	public String getColumnName(int column) {
		...
		default: {
			...
			if (column >= offset)
				...
				return (fields == null) ? Integer.toString(column) : fields.getElementAt(column - 3).getName();
		}
		}
	}
	...
	public void sort(int column, boolean ascending) throws UnsupportedOperationException {
		if (storedResults.getFieldname() == null)
			return;
		String col = getColumnName(column);
		if (!AbstractStructureQuery.FIELD_NAMES.metric.toString().equals(col.toLowerCase()))
			throw new UnsupportedOperationException("Column " + getColumnName(column) + " is not sortable!");
