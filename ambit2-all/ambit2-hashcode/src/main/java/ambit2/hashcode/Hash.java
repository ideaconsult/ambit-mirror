...
{   private long hash_code;
    ...
    // Utility Methods for Hash Values
    //
    ...
    //  Constructors for Hash values;
    //  invoked as: new Hash( some-key-to-be-hashed )
    //  they operate on the key and return a Hash value.

    public Hash()               // The Hash of nothing
    {   this.hash_code = initial_hash; }

    public Hash ( Hash hc )     // The next extension of a Hash value
    {   this.hash_code = lhv( hc.hash_code ); }

    public Hash (boolean arg)   // Make Hash from boolean
    {   this.hash_code = ( arg ? hash_true : hash_false ) ; }

    public Hash (byte arg)      // Make Hash from byte
    {   this.hash_code = mix_master[ (((int)(arg))+183) & 0xff ]; }

    public Hash (short arg)     // Make Hash from short
    {   this.hash_code = lhv( (long) arg ); }

    public Hash (char arg)      // Make Hash from char
    {   this.hash_code = lhv( (long) arg ); }

    public Hash (int arg)       // Make Hash from int
    {   this.hash_code = lhv( (long) arg ); }

    public Hash (long arg)      // Make Hash from long
    {   this.hash_code = lhv( arg ); }

    //   These are commented out because they require a C routine
    //   to give back the floating point bits back unconverted
    //   as a long integer.
    //
    // public Hash (float arg)      // Make Hash from float
    // {   this.hash_code = lhv( bytes8( (double) arg ) ); }
    //
    // public Hash (double arg)     // Make Hash from double
    // {   this.hash_code = lhv( bytes8( (double) arg ) ); }

    public Hash (String arg)        // Make Hash from String
    {   this.hash_code = buzhash( new StringBuffer(arg) ); }

    public Hash (StringBuffer arg)  // Make Hash from StringBuffer
    {   this.hash_code = buzhash( arg ); }

    // These are all the constructors supported here, one could think
    // of others, there should be one for every type for which an
    // an equal comparison works.
    ...
    // Hash Abstract Type Methods for manipulating Hash Values

    // Methods to Compare Hash Values and Return Boolean.

    public boolean equal ( Hash b )  // Test Equivalence of two Hashes
    {   return ( this.hash_code == b.hash_code ); }

    public boolean not_equal ( Hash b )  // Test Equivalence of two Hashes
    {   return ( this.hash_code != b.hash_code ); }

    public boolean more ( Hash b )  // Test Hash Value a bigger than b
    {   if ( (this.hash_code>>>1) > (b.hash_code>>>1) )
             return true;
        else if ( (this.hash_code>>>1) < (b.hash_code>>>1) )
             return false;
        else return (this.hash_code&0x1) > (b.hash_code&0x1);
        }

    public boolean less ( Hash b )  // Test Hash Value a less than b
    {   return b.more(this); }

    // Methods for Combining Two Hash Values.

    public Hash wag ( Hash b )  // Weak Agglomeration of two Hashes
    {   Hash result = new Hash();
        result.hash_code = this.hash_code ^ b.hash_code;
        return result;
        }

    public Hash sag ( Hash b )  // Strong Agglomeration of two Hashes
    {   StringBuffer tmp = new StringBuffer(8);
        Hash result = new Hash();
        long aval   = this.hash_code;
        long bval   = b.hash_code;

        tmp.setLength(8);
        for( int i=0; i<4; i+=2 )
        {   tmp.setCharAt( i,   (char) aval );
            aval >>>= 16;
            tmp.setCharAt( i+1, (char) bval );
            bval >>>= 16;
            }

        result.hash_code = buzhash( tmp );
        return result;
        }

    // Methods which Convert Hash Values to Integers.

    private static long NOSIGN = 0x7fffffffffffffffL;

    public byte  bag ( byte b )   // Convert Hash to byte
    {   return (byte) ((this.hash_code & NOSIGN ) % (b<0? -b : b )); }

    public short bag ( short b )  // Convert Hash to short
    {   return (short)((this.hash_code & NOSIGN ) % (b<0? -b : b )); }

    public char  bag ( char b )   // Convert Hash to char
    {   return (char) ((this.hash_code & NOSIGN ) % b); }

    public int   bag ( int b )    // Convert Hash to int
    {   return (int)  ((this.hash_code & NOSIGN ) % (b<0? -b : b )); }

    public long  bag ( long b )   // Convert Hash to long
    {   return         (this.hash_code & NOSIGN ) % (b<0? -b : b ); }
