...
public class OPSCountsByCompound implements IMolecularDescriptor {
	...
	public DescriptorValue calculate(IAtomContainer molecule) {
		try {
			Object inchikey = molecule
					.getProperty(Property.opentox_InChIKey_std);
			if (inchikey == null) {
				if (processor == null)
			...
			if (inchikey != null) {
				...
				return new DescriptorValue(getSpecification(),
						getParameterNames(), getParameters(), result,
						getDescriptorNames());
			}
	...
	public DescriptorSpecification getSpecification() {
		return new DescriptorSpecification(
				String.format("http://ambit.sf.net/api/#%s",
						"OPSCountsByCompound"),
