...
public class SimilarityDepict extends AbstractDepict {
	...
	for (int i = 0; i < (n - 1); i++) {
		...
		b.append(AmbitResource.printWidget("Similarity", String.format("Tanimoto = %4.3f", similarity), style));
		b.append("</td>");
		b.append("</tr>");
	    }
    ...
    protected String getSmilesWidget(String smiles, String uri, String style, int id) {

	return AmbitResource.printWidget(String.format(
		"<a href='%s?search=%s%s%s'>%s</a>&nbsp;<span style='float:right;'>%s</span>", uri, smiles == null ? ""
			: Reference.encode(smiles), smarts == null ? "" : "&smarts=",
		smarts == null ? "" : Reference.encode(smarts), "CDK depiction", String.format(AbstractDepict.gplus,
			uri)), String.format(
		"<img id='cdk_%d' src='%s/any?search=%s&smarts=%s' alt='%s' title='%s' onError=\"hideDiv('cdk_%d')\">",
		id, uri, smiles == null ? "" : Reference.encode(smiles),
		smarts == null ? "" : Reference.encode(smarts), smiles == null ? "" : smiles, smiles == null ? ""
			: smiles, id), style);
    }
	...
	for (int i = 0; i < n; i++) {
	    ...
	    w.write(String.format("<input name='%s' size='70' value='%s'>\n", QueryResource.search_param,
		    query_smiles[i] == null ? "" : query_smiles[i]));
	    w.write("</td>");
	    if (i == 0)
	...
	for (int i = 0; i < n; i++)
		...
		bitset[i] = fingerprinter.getFingerprint(molecule);
	    } catch (Exception x) {
		bitset[i] = null;
	    }
	return bitset;
