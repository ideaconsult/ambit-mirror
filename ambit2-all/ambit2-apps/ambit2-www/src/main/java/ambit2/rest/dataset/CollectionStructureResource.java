...
public class CollectionStructureResource<Q extends IQueryRetrieval<IStructureRecord>>
	...
	protected Representation post(Representation entity, Variant variant) throws ResourceException {
		/*
		 * if ((entity == null) || !entity.isAvailable()) throw new
	...
	protected Representation copyDatasetToQueryResultsTable(Form form, boolean clearPreviousContent)
		...
		if (folder != null) {
			form.removeAll("folder");
			form.add("folder", folder.toString());
		}
		if (datasetid != null) {
			form.removeAll("title");
			form.add("title", datasetid.toString());
		}

		IStoredQuery sq = new StoredQuery();
		sq.setName(datasetid.toString());
		callable = new CallableQueryResultsCreator(form, getRequest().getRootRef(), getContext(), sq, token,
