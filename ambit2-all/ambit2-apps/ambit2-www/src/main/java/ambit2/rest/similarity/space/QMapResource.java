...
public class QMapResource extends QueryResource<IQueryRetrieval<QMap>, QMap> {
    ...
    @Override
    public IProcessor<IQueryRetrieval<QMap>, Representation> createConvertor(Variant variant) throws AmbitException,
	    ResourceException {
	...
	if (variant.getMediaType().equals(MediaType.TEXT_URI_LIST)) {
	    ...
	    return new OutputWriterConvertor(new QMapJSONReporter<IQueryRetrieval<QMap>>(getRequest()),
    ...
    @Override
    protected IQueryRetrieval<QMap> createQuery(Context context, Request request, Response response)
	...
	if (datasetURI != null)
	    try {
		int id = (Integer) OpenTox.URI.dataset.getId(Reference.decode(datasetURI.toString().trim()),
			request.getRootRef());
		QMap map = new QMap();
	...
	if (propertyURI != null)
	    try {
		int id = (Integer) OpenTox.URI.feature.getId(Reference.decode(propertyURI.toString().trim()),
			request.getRootRef());
