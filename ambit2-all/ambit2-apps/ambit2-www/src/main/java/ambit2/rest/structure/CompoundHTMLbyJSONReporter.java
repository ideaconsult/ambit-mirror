...
public class CompoundHTMLbyJSONReporter<Q extends IQueryRetrieval<IStructureRecord>> extends
	QueryStructureHTMLReporter<Q> {
    /**
	 * 
	 */
    ...
    public CompoundHTMLbyJSONReporter(String prefix, Request request, DisplayMode _dmode, QueryURIReporter urireporter,
	    Template template, Profile groupedProperties, Dimension d, String urlPrefix) {
	super(request, _dmode, false);
	cmp_reporter = new CompoundJSONReporter<IQueryRetrieval<IStructureRecord>>(template, groupedProperties, null,
		null, request, urlPrefix, false, null);
	processors = cmp_reporter.getProcessors();
    }

    @Override
    protected QueryURIReporter createURIReporter(Request request, ResourceDoc doc) {
	return new CompoundURIReporter(request);
    }
	...
	try {
	    ...
	    output.write(String
		    .format("<hr><div class='results'><a href='%s' title='%s'>This dataset</a> | %s&nbsp; "
			    ...
			    /*
			     * showRegistry,registry showSMILES,smiles
			     * showInChI,inchi showProperties,property
			     * showEndpoints,endpoint showCalculated,calculated
			     * showNames,name
			     */
			    ...
			    downloadLinks(),
			    cmp_reporter.getHilightPredictions() == null ? "" : String.format(
				    "Atoms highlighted by the model <a href=%s target=_blank>%s</a></h4></span></div>",
				    cmp_reporter.getHilightPredictions(), cmp_reporter.getHilightPredictions()),
			    String.format(
				    "<form method='GET' action=''>Page&nbsp;<input name='page' type='text' title='Page' size='10' value='%s'>&nbsp;"
				    ...
				    page == null ? "0" : page, pagesize == null ? "100" : pagesize,
	...
	MediaType[] mimes = { ChemicalMediaType.CHEMICAL_MDLSDF, ChemicalMediaType.CHEMICAL_CML,
		...
		ChemicalMediaType.THREECOL_ARFF, MediaType.APPLICATION_RDF_XML, MediaType.APPLICATION_JSON };
	String[] image = { "sdf.jpg", "cml.jpg", "smi.png", "inchi.png", "link.png", "pdf.png", "excel.png",
		"excel.png", "weka.png", "weka.png", "rdf.gif", "json.png"
	...
	w.append(String.format(
		" <a href='%s/ui/_dataset?dataset_uri=%s' title='Test the new dataset browser'>New view mode</a> ",
		uriReporter.getBaseReference().toString(), Reference.encode(uriReporter.getResourceRef().toString())));
	return w.toString();
    }
