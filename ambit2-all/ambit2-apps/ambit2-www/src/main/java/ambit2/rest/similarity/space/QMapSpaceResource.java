...
public class QMapSpaceResource extends
		QueryResource<IQueryRetrieval<QMapSpace>, QMapSpace> {
	...
	public IProcessor<IQueryRetrieval<QMapSpace>, Representation> createConvertor(
		...
		if (variant.getMediaType().equals(MediaType.TEXT_URI_LIST)) {
			...
			return new OutputWriterConvertor(new QMapSpaceJSONReporter(
	...
	protected IQueryRetrieval<QMapSpace> createQuery(Context context,
		...
		try {
			query.setThreshold_a(Integer.parseInt(form
					.getFirstValue("thresholda")));
		} catch (Exception x) {
		...
		if (qmapURI != null)
			try {
				int id = (Integer) OpenTox.URI.qmap.getId(
						Reference.decode(qmapURI.toString().trim()),
						request.getRootRef());
		...
		if (datasetURI != null)
			try {
				int id = (Integer) OpenTox.URI.dataset.getId(
						Reference.decode(datasetURI.toString().trim()),
						request.getRootRef());
				QMap map = new QMap();
		...
		if (propertyURI != null)
			try {
				int id = (Integer) OpenTox.URI.feature.getId(
						Reference.decode(propertyURI.toString().trim()),
						request.getRootRef());
