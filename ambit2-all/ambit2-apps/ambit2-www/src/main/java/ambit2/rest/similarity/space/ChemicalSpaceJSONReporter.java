 ...
 * @param <Q>
 */
public class ChemicalSpaceJSONReporter<Q extends IQueryRetrieval<ChemSpaceCell>> extends QueryReporter<ChemSpaceCell,Q,Writer> {
	/**
	 * 
	 */
		...
		try {
			...
			try {
				if (ranges.get(item.getIndex()[0])==null)
					ranges.put(item.getIndex()[0],item.getLeft()[0]); //to put ranges here
				if (ranges.get(item.getIndex()[1])==null)
					ranges.put(item.getIndex()[1],item.getRight()[0]); //to put ranges here
				if (ranges.get(item.getIndex()[1]+1)==null)
					ranges.put(item.getIndex()[1]+1,item.getRight()[1]); //to put ranges here
				output.write(String.format("%s\n[%d,%d,%6.4E]",comma,item.getIndex()[0],item.getIndex()[1],item.getDistance()));
