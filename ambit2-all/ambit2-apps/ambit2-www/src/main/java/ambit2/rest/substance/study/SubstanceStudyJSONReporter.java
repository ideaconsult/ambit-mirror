 ...
 * @param <Q>
 */
public class SubstanceStudyJSONReporter<Q extends IQueryRetrieval<ProtocolApplication>>
		extends QueryReporter<ProtocolApplication, Q, Writer> {
	/**
	 * 
	 */
	...
	public SubstanceStudyJSONReporter(Request request, String jsonpCallback) {
		super();
		substanceReporter = new SubstanceURIReporter<IQueryRetrieval<SubstanceRecord>>(
				request);
		...
		MasterDetailsProcessor<ProtocolApplication, EffectRecord<String, String, String>, IQueryCondition> effectReader = new MasterDetailsProcessor<ProtocolApplication, EffectRecord<String, String, String>, IQueryCondition>(
			...
			@Override
			protected ProtocolApplication processDetail(
					ProtocolApplication master, EffectRecord detail)
					throws Exception {
				if (detail != null)
		...
		getProcessors()
				.add(new DefaultAmbitProcessor<ProtocolApplication, ProtocolApplication>() {
					/**
		     * 
		     */
					private static final long serialVersionUID = 3463829174956429308L;

					public ProtocolApplication process(
							ProtocolApplication target) throws AmbitException {
						processItem(target);
						return target;
					};
				});
