...
public class AbstractDepict extends ProtectedResource {
	...
	protected static String printGPlusSnippet(String title, String description,
			String image) {
		return String.format(gplus_snippet, title, description,
				image == null ? "" : image);
	}
	...
	protected String getTitle(Reference ref, String... smiles)
		...
		b.append(AmbitResource.printWidget(
				String.format(
						...
						uri, smiles == null ? "" : Reference.encode(smiles[0]),
						smarts == null ? "" : "&smarts=", smarts == null ? ""
								: Reference.encode(smarts), "CDK depiction",
						String.format(gplus, uri)),
				String.format(
						"<img id='cdk' src='%s/any?search=%s&smarts=%s' alt='%s' title='%s' onError=\"hideDiv('cdk')\">",
						uri, Reference.encode(displaySmiles),
						smarts == null ? "" : Reference.encode(smarts),
						displaySmiles, displaySmiles), style));
		b.append("</td>");
		b.append("<td>");
		...
		b.append(AmbitResource.printWidget(
				...
				String.format(
						"<img id='cactvs' src='%s?search=%s' alt='%s' title='%s' onError=\"hideDiv('cactvs')\">",
						uri, Reference.encode(displaySmiles), displaySmiles,
						displaySmiles), style));
		...
		b.append(AmbitResource.printWidget(
				...
				String.format(
						...
						uri, Reference.encode(displaySmiles), h, h,
						displaySmiles, displaySmiles, h, h), style));
		b.append("</td>");

		b.append("<td>");

		String recordTypeOption = recordType == null ? "" : String.format(
				"&record_type=%s", recordType);
		uri = String.format("%s/pubchem", root);
		b.append(AmbitResource.printWidget(
				String.format(
						"<a href='%s?search=%s%s'>%s</a>&nbsp;<span style='float:right;'>%s</span>",
						uri, Reference.encode(displaySmiles), recordTypeOption,
								...
								try {
									if (headless)
										...
										AmbitResource
												.writeTopHeader(
														output,
														smiles[0] == null ? "2D structural diagram"
																: smiles[0],
														getRequest(),
														getResourceRef(getRequest()),
														header_gplus);
										writeSearchForm(output,
												smiles[0] == null ? "N/A"
														: smiles[0],
			...
			if (smiles[0] != null) {
				...
				if (image == null) {
					getResponse().setStatus(Status.CLIENT_ERROR_BAD_REQUEST,
							String.format("Invalid smiles %s", smiles));
					return null;
				}
			} else
	...
	protected Representation post(Representation entity, Variant variant)
		...
		if (MediaType.APPLICATION_WWW_FORM.equals(entity.getMediaType())) {
			...
			throw new ResourceException(Status.CLIENT_ERROR_BAD_REQUEST,
					String.format("%s not supported", entity == null ? ""
							: entity.getMediaType()));
	}
	...
	public void writeSearchForm(Writer w, String title, Request request,
		...
		w.write(String
				.format("<a href=\"%s/ui\"><img itemprop=\"image\" src='%s/images/ambit-logo.png' width='256px' alt='%s' title='%s' border='0'></a>\n",
		...
		w.write("<tr>");
		w.write(String.format("<th><label for='%s'>%s</label>",
		...
		w.write("<td>");
		w.write(String.format(
				"<input name='%s' size='70' value='%s' tabindex='0'>",
