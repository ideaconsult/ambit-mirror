...
public class PropertyJSONReporter extends PropertyURIReporter {
	...
	public Object processItem(Property feature) throws AmbitException {
		try {
			...
			getOutput().write(String.format(
					...
					"\n\t\"%s\":[%s]\n\n}",
					uri,
					jsonFeature.title.jsonname(),JSONUtils.jsonQuote(JSONUtils.jsonEscape(getAggregatedFeatureName(feature))),
					...
					jsonFeature.source.jsonname(),
						...
						JSONUtils.jsonQuote(JSONUtils.jsonEscape(typeSource)),
	...
	private String getAggregatedFeatureName(Property feature) {
		if (feature.getName()==null) return null;
		if (feature.getName().startsWith("http://www.opentox.org/")) {
			if (Property.opentox_CAS.equals(feature.getName())) return Property.CAS;
			if (Property.opentox_IupacName.equals(feature.getName())) return "IUPAC name";
			if (Property.opentox_Name.equals(feature.getName())) return Property.Names;
			if (Property.opentox_InChI.equals(feature.getName())) return "InChI";
			if (Property.opentox_InChI_std.equals(feature.getName())) return "Std. InChI";
			if (Property.opentox_InChIKey.equals(feature.getName())) return "InChI Key";
			if (Property.opentox_InChIKey_std.equals(feature.getName())) return "Std. InChI key";
			...
			if (Property.opentox_ChEBI.equals(feature.getName())) return "CHEBI ID";
			if (Property.opentox_ChEMBL.equals(feature.getName())) return "CHEMBL ID";
			if (Property.opentox_ChemSpider.equals(feature.getName())) return "ChemSpider ID";
			/* IUCLID5 Reference substance UUID */
			if (Property.opentox_IUCLID5_UUID.equals(feature.getName())) return "IUCLID 5 Reference substance UUID";
	
			/* Mostly for usage in ToxBank */
			if (Property.opentox_ToxbankWiki.equals(feature.getName())) return "ToxBank Wiki URL";
			if (Property.opentox_CMS.equals(feature.getName())) return "COSMOS ID";
		}
