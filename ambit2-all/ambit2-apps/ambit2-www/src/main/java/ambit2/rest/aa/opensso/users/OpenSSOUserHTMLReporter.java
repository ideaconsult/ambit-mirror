...
public class OpenSSOUserHTMLReporter extends OpenSSOUsersURIReporter {
    ...
    public OpenSSOUserHTMLReporter(Request ref) {
	super(ref);
    }
	...
	try {
	    ...
	    content.append(String
		    .format("<tr><th align='right' title='OpenTox Authentication and Authorisation is managed by OpenAM http://forgerock.com/openam.html'><a href='http://forgerock.com/openam.html' target=_blank>OpenAM</a> service:&nbsp;</th><td>%s</td></tr>",
		...
		content.append(String
			.format("<tr><th align='right'>%s</th><td align='left'><input type='text' size='40' name='%s' value=''></td></tr>",
		...
		content.append(String.format("<tr><th align='right'>%s</th><td>%s</td></tr>", "User name:&nbsp;",
			item.getUsername()));
		content.append(String.format("<tr><th align='right'>%s</th><td>%s</td></tr>", "Token:&nbsp;", item
			.getToken().toString()));
		...
		content.append(String.format("<a href='%s/bookmark/%s'>My workspace</a>", getRequest().getRootRef(),
			item.getUsername(), item.getUsername()));
		content.append(String.format("&nbsp;&nbsp;<a href='%s/admin/policy'>View/Define access rights</a>",
			getRequest().getRootRef()));
		content.append(String.format("&nbsp;&nbsp;<a href='%s/admin'>Admin</a>", getRequest().getRootRef()));
	    }

	    output.write(AmbitResource
		    .printWidget(
