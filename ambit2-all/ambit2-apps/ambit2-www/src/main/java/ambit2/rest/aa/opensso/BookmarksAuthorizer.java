...
public class BookmarksAuthorizer extends OpenSSOAuthorizer {
	...
	protected boolean authorize(OpenSSOToken ssoToken, Request request) throws Exception {
		...
		Template template1 = new Template(
				String.format("%s/bookmark/{%s}", request.getRootRef(), BookmarkResource.creator));

		Template template2 = OpenTox.URI.bookmark.getTemplate(request.getRootRef());
		Template template3 = new Template(
				String.format("%s/bookmark/{%s}/entries", request.getRootRef(), BookmarkResource.creator));
		Template template4 = new Template(
				String.format("%s/bookmark/{%s}/topics", request.getRootRef(), BookmarkResource.creator));
