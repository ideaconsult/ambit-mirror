...
public class PropertyValueResource<T extends Serializable> extends QueryResource<IQueryRetrieval<T>, T> {
    ...
    public static final String FeatureNameConformer = String.format("%s/{name}%s", featureKey,
	    ConformerResource.conformerID);
    public static final String FeatureNameCompound = String.format("%s/{name}%s", featureKey,
	...
	if (variant.getMediaType().equals(MediaType.TEXT_PLAIN)) {
	    ...
	    return new OutputWriterConvertor(new PropertyValueHTMLReporter(getRequest()), MediaType.TEXT_HTML);
	    ...
	    return new RDFJenaConvertor<T, IQueryRetrieval<T>>(new PropertyValueRDFReporter<T>(getRequest(),
    ...
    @Override
    protected QueryURIReporter<T, IQueryRetrieval<T>> getURUReporter(Request baseReference) throws ResourceException {
    ...
    @Override
    protected IQueryRetrieval<T> createQuery(Context context, Request request, Response response)
	...
	try {
	    record.setIdchemical(Integer.parseInt(Reference.decode(request.getAttributes()
		    .get(CompoundResource.idcompound).toString())));
	} catch (NumberFormatException x) {
	    throw new ResourceException(Status.CLIENT_ERROR_BAD_REQUEST, String.format("Invalid resource id %d",
		    request.getAttributes().get(CompoundResource.idcompound)), x);
	}
	try {
	    record.setIdstructure(Integer.parseInt(Reference.decode(request.getAttributes()
		    .get(ConformerResource.idconformer).toString())));
