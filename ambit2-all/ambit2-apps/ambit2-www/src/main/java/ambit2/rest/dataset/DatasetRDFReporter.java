 ...
 * @param <Q>
 */
public class DatasetRDFReporter<Q extends IQueryRetrieval<IStructureRecord>>
		extends QueryRDFReporter<IStructureRecord, Q> {
	...
	public DatasetRDFReporter(Request request, MediaType mediaType,
			Template template, Profile groupedProperties) {
		this("", request, mediaType, template, groupedProperties);
	}

	public DatasetRDFReporter(String prefix, Request request,
			MediaType mediaType, Template template, Profile groupedProperties) {
	...
	protected QueryURIReporter<IStructureRecord, IQueryRetrieval<IStructureRecord>> createURIReporter(
		...
		try {
			compoundReporter = new CompoundURIReporter<IQueryRetrieval<IStructureRecord>>(
					compoundInDatasetPrefix, req, false);
			dataEntryReporter = new DataEntryURIReporter<IQueryRetrieval<IStructureRecord>>(
					req);
			return new ConformerURIReporter<IQueryRetrieval<IStructureRecord>>(
					compoundInDatasetPrefix, req);
		} catch (Exception x) {
	...
	protected void configurePropertyProcessors() {
		if ((getGroupProperties() != null) && (getGroupProperties().size() > 0))
			getProcessors().add(
					new ProcessorStructureRetrieval(
							new RetrieveGroupedValuesByAlias(
									getGroupProperties())) {
						/**
			     * 
			     */
						...
						public IStructureRecord process(IStructureRecord target)
							...
							((RetrieveGroupedValuesByAlias) getQuery())
									.setRecord(target);
		...
		if ((getTemplate() != null) && (getTemplate().size() > 0))
			getProcessors().add(
					new ProcessorStructureRetrieval(new RetrieveProfileValues(
							SearchMode.idproperty, getTemplate(), true)) {
						/**
			     * 
			     */
						...
						public IStructureRecord process(IStructureRecord target)
								throws AmbitException {
							((RetrieveProfileValues) getQuery())
									.setRecord(target);
							return super.process(target);
						}
					});
	...
	protected void createDatasetIndividual(int datasetID) {
		if (uriReporter.getResourceRef().getQueryAsForm()
				.getFirstValue(OpenTox.params.feature_uris.toString()) != null) {
			dataset = output.createIndividual(OT.OTClass.Dataset
					.getOntClass(output));
		} else {
			if (datasetID <= 0)
				dataset = output.createIndividual(String.format("%s:%s",
						uriReporter.getResourceRef().getScheme(), uriReporter
								.getResourceRef().getHierarchicalPart()),
						OT.OTClass.Dataset.getOntClass(output));
			else
				dataset = output.createIndividual(String.format("%s/%s/%d",
	...
	public Object processItem(IStructureRecord item) throws AmbitException {
		try {
			...
			if (item.getDataEntryID() > 0) {
				dataEntry = getJenaModel().createIndividual(
						dataEntryReporter.getURI(item),
			...
			dataset.addProperty(
					OT.OTProperty.dataEntry.createProperty(getJenaModel()),
					dataEntry);
			int i = 0;

			String uri = item.getType().equals(STRUC_TYPE.NA)
					|| item.getType().equals(STRUC_TYPE.NANO) ? compoundReporter
					.getURI(item) : uriReporter.getURI(item);
			Individual compound = getJenaModel().createIndividual(uri,
					OT.OTClass.Compound.getOntClass(getJenaModel()));

			if (item.getType().equals(STRUC_TYPE.NANO)) {
				compound.addOntClass(OT.OTClass.OTMaterial
						.getOntClass(getJenaModel()));
			}
			// compound.addProperty(DC.identifier, uriReporter.getURI(item));
			dataEntry.addProperty(
					OT.OTProperty.compound.createProperty(getJenaModel()),
					compound);
			...
			for (ambit2.base.data.Property p : header) {
				...
				p.setClazz((value instanceof Number) ? Double.class
						: String.class);
				Individual feature = (Individual) propertyReporter
						.processItem(p);

				Individual featureValue = getJenaModel().createIndividual(
						OT.OTClass.FeatureValue.getOntClass(getJenaModel()));
				featureValue.addProperty(
						OT.OTProperty.feature.createProperty(getJenaModel()),
						feature);

				Literal lValue = null;
				i++;
				dataEntry.addProperty(
						OT.OTProperty.values.createProperty(getJenaModel()),
						featureValue);
				// TODO this is way too inefficient
				if (p.isNominal()) {
					boolean add = true;
					StmtIterator values = getJenaModel().listStatements(
							new SimpleSelector(feature, acceptValue,
									(RDFNode) null));
					...
					if (add) {
						lValue = getJenaModel()
								.createTypedLiteral(
										value,
										(value instanceof Number) ? XSDDatatype.XSDdouble
												: XSDDatatype.XSDstring);
						feature.addProperty(acceptValue, value.toString());
					}
				} else {
					lValue = getJenaModel().createTypedLiteral(
							value,
							(value instanceof Number) ? XSDDatatype.XSDdouble
									: XSDDatatype.XSDstring);
				}

				featureValue.addLiteral(
						OT.DataProperty.value.createProperty(getJenaModel()),
						lValue);
