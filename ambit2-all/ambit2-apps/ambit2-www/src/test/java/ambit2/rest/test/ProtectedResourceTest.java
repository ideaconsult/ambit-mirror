...
public abstract class ProtectedResourceTest extends ResourceTest implements IAuthToken {
	...
	public void setUpAA() throws Exception {
		if (isAAEnabled()) {
			...
			if (ssoToken.login(OpenSSOServicesConfig.getInstance().getTestUser(),
					OpenSSOServicesConfig.getInstance().getTestUserPass())) {
				ClientResourceWrapper.setTokenFactory(this);
			} else
				throw new Exception(
						String.format("Error logging to SSO (%s)", OpenSSOServicesConfig.getInstance().getTestUser()));
		}
	}
	...
	protected String createPartnerReadOnlyPolicy(String uri) throws Exception {
		...
		int httpcode = policy.createGroupPolicy("partner", ssoToken, uri, new String[] { "GET" }, b.toString());
		if (httpcode == 200)
			return b.toString();
		else
			throw new Exception(String.format("Error creating policy %d", httpcode));
	}
	...
	protected String createPartnerRWPolicy(String uri) throws Exception {
		...
		// b.append("_");
		// b.append(UUID.randomUUID());
		int httpcode = policy.createGroupPolicy("partner", ssoToken, uri, new String[] { "GET", "POST", "PUT" },
				b.toString());
		if (httpcode == 200)
			return b.toString();
		else
	...
	protected String createPublicPOSTPolicy(String uri) throws Exception {
		...
		// b.append("_");
		// b.append(UUID.randomUUID());
		int httpcode = policy.createGroupPolicy("member", ssoToken, uri, new String[] { "GET", "POST" }, b.toString());
		if (httpcode == 200)
			return b.toString();
		else
			throw new Exception(String.format("Error creating policy %d", httpcode));
	...
	protected String createPublicROPolicy(String uri) throws Exception {
		...
		// b.append("_");
		// b.append(UUID.randomUUID());
		int httpcode = policy.createGroupPolicy("member", ssoToken, uri, new String[] { "GET" }, b.toString());
		if (httpcode == 200)
			return b.toString();
		else
			throw new Exception(String.format("Error creating policy %d", httpcode));
	...
	protected String createPublicPolicy(String uri) throws Exception {
		...
		// b.append("_");
		// b.append(UUID.randomUUID());
		int httpcode = policy.createGroupPolicy("member", ssoToken, uri,
				new String[] { "GET", "DELETE", "POST", "PUT" }, b.toString());
		if (httpcode == 200)
			return b.toString();
		else
			throw new Exception(String.format("Error creating policy %d", httpcode));
	...
	protected String createPolicy(String uri) throws Exception {
		...
		b.append("_");
		b.append(UUID.randomUUID());
		int httpcode = policy.createUserPolicy(OpenSSOServicesConfig.getInstance().getTestUser(), ssoToken, uri,
				new String[] { "GET", "DELETE", "POST", "PUT" }, b.toString());
		if (httpcode == 200)
			return b.toString();
		else
