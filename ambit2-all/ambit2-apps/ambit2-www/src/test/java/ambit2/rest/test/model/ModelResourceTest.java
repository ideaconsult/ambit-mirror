...
public class ModelResourceTest extends ResourceTest {
	...
	public boolean verifyResponseURI(String uri, MediaType media, InputStream in)
		...
		while ((line = r.readLine()) != null) {
			Assert.assertEquals(
					String.format("http://localhost:%d/model/1", port), line);
			count++;
		}
		return count == 1;
	}
	...
	public void testPostDataset() throws Exception {
		...
		testAsyncTask(getTestURI(), headers, Status.SUCCESS_OK, String.format(
				"%s?%s=%s", dataset, OpenTox.params.feature_uris.toString(),
	...
	public void testPostDataset1() throws Exception {
		Form headers = new Form();
		String dataset = String.format("http://localhost:%d/dataset/1?max=1",
				port);
		headers.add(OpenTox.params.dataset_uri.toString(), dataset);
		testAsyncTask(getTestURI(), headers, Status.SUCCESS_OK, String.format(
				"%s&%s=%s", dataset, OpenTox.params.feature_uris.toString(),
	...
	public void testJ48() throws Exception {
		...
		table = c.createQueryTable("EXPECTED", String.format(
				"SELECT * from properties where comments='%s'",
				Property.opentox_ConfidenceFeature));
		Assert.assertEquals(1, table.getRowCount());
	...
	public void predict(String dataset, String target, String datasetTest,
			String algorithmURI) throws Exception {
		predict(dataset, target, datasetTest, algorithmURI, null);
	}

	public void predict(String dataset, String target, String datasetTest,
			String algorithmURI, String parameter) throws Exception {
		...
		String wekaURI = String.format("http://localhost:%d%s/3", port,
				MLResources.model_resource);
