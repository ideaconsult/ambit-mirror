...
public class AllConformersResourceTest extends ConformerResourceTest {
	...
	while ((line = r.readLine()) != null) {
	    Assert.assertTrue(line.equals(String.format("http://localhost:%d/compound/10/conformer/999", port))
	...
	while (reader.hasNext()) {
	    ...
	    // Assert.assertEquals(0,mol.getProperties().size());test datasets
	    // has properties inside structure table field
	    count++;
	}
	return count == 2;
    }
