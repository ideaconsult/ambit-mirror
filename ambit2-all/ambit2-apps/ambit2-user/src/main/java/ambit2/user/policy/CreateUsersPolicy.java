 ...
 * 
 */
public class CreateUsersPolicy extends
		AbstractUpdate<RESTPolicy, RESTPolicyUsers> implements IDBConfig {
	...
	public CreateUsersPolicy(RESTPolicy existingrole, RESTPolicyUsers policy) {
		super(policy);
		createPolicyQuery = getCreatePolicyQuery();
		createExistingRolePolicy = getCreatePolicyQuery();
	...
	public void setObject(RESTPolicyUsers object) {
		...
		if (createPolicyQuery != null) {
			...
			createPolicyQuery.setGroup(new DBRole(object.getRole(), object
					.getRole()));
		}
		initIndices();
	}
	...
	public List<QueryParam> getParameters(int index) throws AmbitException {
		if (index == i_createrole) {
			...
			params2.add(new QueryParam<String>(String.class, getObject()
					.getRole()));
			return params2;
			...
			List<QueryParam> params2 = new ArrayList<QueryParam>();
			for (DBUser user : getObject().getUsers()) {
				params2.add(new QueryParam<String>(String.class, user
						.getUserName()));
				params2.add(new QueryParam<String>(String.class, getObject()
						.getRole()));
			}
	...
	public String[] getSQL() throws AmbitException {
		...
		sql[i_createrole] = String.format(createRole, databaseName == null ? ""
				: String.format("`%s`.", databaseName));
		sql[i_removeroles] = String.format(
				removeRolefromUsers,
				databaseName == null ? "" : String
						.format("`%s`.", databaseName));

		sql[i_createpolicy] = createPolicyQuery.getSQL()[0];
		if (i_assignroles > 0) {
			sql[i_assignroles] = String.format(
					assignRole2User,
					databaseName == null ? "" : String.format("`%s`.",
							databaseName), users_sql.toString());
		}
		if (i_createpolicy_existingrole > 0) {
			sql[i_createpolicy_existingrole] = createExistingRolePolicy
					.getSQL()[0];
		}
		return sql;
	}
