...
public class RegistrationResource extends CatalogResource<DBUser> {
	...
	protected ICallableTask createCallable(Method method, Form form, DBUser item)
		...
		try {
			...
			try {
				enableEmailVerification = ((IFreeMarkerApplication) getApplication())
			...
			return new AMBITCallableUserCreator(method, item, reporter, form,
					getRequest().getRootRef().toString(), conn, getToken(),
					false, enableEmailVerification,
	...
	protected FactoryTaskConvertor getFactoryTaskConvertor(ITaskStorage storage)
		...
		return new FactoryTaskConvertorRDF<Object>(storage, getHTMLBeauty()) {
			...
			public synchronized Reporter<Iterator<UUID>, Writer> createTaskReporterHTML(
				...
				return new RegistrationTaskHTMLReporter(storage, request, doc,
						htmlbeauty);
			}
			...
			public synchronized Representation createTaskRepresentation(
					...
					throws ResourceException {
				String locationRef = String.format("%s/task/%s", getRequest()
						.getRootRef(), task);
