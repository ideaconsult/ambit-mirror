...
* Toxtree 3.1.0
* groupcontribution package
* synthetic accessibility package (in progress)
* ambit-www : solr JSON updates
...
* grant execute on procedure findByProperty to "guest"@"127.0.0.1"; (replace with the proper mysql user name)
...
* .r3856
* site update
* fixed NPE in facet resource to handle prefixed compound uris , e.g. /dataset/id/compound 
* added subclass to handle dragon descriptors, when dragon home is set via maven config profiles to enable/disable AA on /admin resources
