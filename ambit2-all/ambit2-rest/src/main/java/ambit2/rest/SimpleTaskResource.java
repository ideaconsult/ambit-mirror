 ...
 * running tasks under URL http://host/application/task. Provide (read)access to
 ...
 * If a single task is requested and the task is not completed,returns Status
 * code 202 (accepted, processing has not been completed). <br>
 ...
 * 
 */
public class SimpleTaskResource<USERID> extends
	AbstractResource<Iterator<UUID>, UUID, IProcessor<Iterator<UUID>, Representation>> {
	...
	super.doInit();
	customizeVariants(new MediaType[] { MediaType.TEXT_HTML, MediaType.TEXT_URI_LIST, MediaType.TEXT_PLAIN,
		MediaType.APPLICATION_RDF_XML, MediaType.APPLICATION_RDF_TURTLE, MediaType.TEXT_RDF_N3,
		MediaType.APPLICATION_JSON, MediaType.APPLICATION_JAVASCRIPT, MediaType.TEXT_RDF_NTRIPLES,
	...
	try {
		...
		TaskStatus.valueOf(searchStatus);
	} catch (Exception x) {
	    throw new ResourceException(Status.CLIENT_ERROR_BAD_REQUEST, String.format(
		    "Allowed status values: %s %s, %s, %s", TaskStatus.Running, TaskStatus.Completed,
		    TaskStatus.Cancelled));
	}
    }

    protected boolean filterTask(ITask<ITaskResult, USERID> task, int taskNumber) {
	if ((max > 0) && (taskNumber >= max))
	    return false;
	else
	    return searchStatus == null ? true : searchStatus.equals(task.getStatus().toString());
    }
    ...
    @Override
    protected synchronized Iterator<UUID> createQuery(Context context, Request request, Response response)
	...
	try {
		...
		// turn
		// ((TaskApplication)getApplication()).getTaskStorage().getTasks();
		return getTasks();

	    } else {

		ITask<ITaskResult, USERID> task = ((ITaskApplication<USERID>) getApplication()).findTask(Reference
			.decode(id.toString()));
	    ...
	    throw new ResourceException(Status.CLIENT_ERROR_BAD_REQUEST, String.format("Task id %s", id), x);
	} catch (Throwable x) {
    ...
    @Override
    public synchronized IProcessor<Iterator<UUID>, Representation> createConvertor(Variant variant)
	...
	return tc.createTaskConvertor(variant, getRequest(), null, DisplayMode.table);
