...
/**
 * Implementation of OpenTox API {@link http://opentox.org/dev/apis/api-1.1/Model}
 * @author nina
 *
 */
@Deprecated
public class OTModel extends OTProcessingResource implements IOTModel {
	...
	protected OTFeatures predictedVariables;
	 ...
	 public OTModel(Reference ref) {
		 super(ref);
		 }
	public OTModel(String ref) {
	...
	public OTObject withAlgorithm(String url) {
		...
		try {this.algorithm = url==null?null:OTAlgorithm.algorithm(url); } catch (Exception x) {}
		return this;
	}	
	...
	public OTObject withTrainingData(String url) {
		...
		try {this.trainingData = url==null?null:OTDataset.dataset(url); } catch (Exception x) {}
		return this;
	}	
	...
	public OTModel load() throws Exception  {
	 ...
	 public OTModel load(OTProperty[] varTypes) throws Exception  {
		 ...
		 try {
			 ...
			 model =  (OntModel)OT.createModel(model, new Reference(getUri()),MediaType.APPLICATION_RDF_XML);
			 for (OTProperty varType:varTypes) {
				 ...
				 case dependentVariables: { 
					 if (dependentVariables == null) dependentVariables = OTFeatures.features();
				 ...
				 case model: {
					 sparqlName = "sparql/Model.sparql";
				        ...
				        try {
							...
							Query query = QueryFactory.create(String.format(OTModel.getSparql(sparqlName).toString(),
									getUri(),getUri(),getUri(),getUri(),getUri()));
							...
							while (results.hasNext()) {
								...
								if (var != null)
								withCreator(var.isLiteral()?((Literal)var).getString():((Resource)var).getURI());
								
								var = solution.get(OTProperty.trainingDataset.toString());
								if (var != null)
								withTrainingData(var.isLiteral()?((Literal)var).getString():((Resource)var).getURI());
								
								var = solution.get("algorithm");
								if (var != null)
								withAlgorithm(var.isLiteral()?((Literal)var).getString():((Resource)var).getURI());									
				 ...
				 vars.withDatasetService(dataset_service);
			        ...
			        try {
						...
						Query query = QueryFactory.create(String.format(OTModel.getSparql(sparqlName).toString(),getUri(),getUri()));
						qe = QueryExecutionFactory.create(query,model);
						ResultSet results = qe.execSelect();
					...
					try {
						...
						if (subset!= null) {
							if (subset.isEmpty(false)) { //nothing to calculate
								OTDataset newdataset = OTDataset.dataset(inputDataset.getUri()).withDatasetService(dataset_service).
								addColumns(feature);
								datasets.add(newdataset);
							} else { //run descriptors on the subset
								OTDataset newDataset = subset.copy();
								OTRemoteTask task = feature.getAlgorithm().processAsync(newDataset);
								pool.add(task);								
							}
						} else { //subset == null, run descriptors on entire dataset
							//System.out.println(feature);
							//System.out.println(feature.getAlgorithm());
							OTRemoteTask task = feature.getAlgorithm().processAsync(inputDataset);
	 ...
	 public OTDataset process(OTDataset inputDataset) throws Exception  {
		...
		// System.out.println(getClass().getName() + subsetToCalculate);
		 ...
		 if ((getPredictedVariables()==null) || getPredictedVariables().size()==0) return result;
	 ...
	 public OTDataset process(OTDataset inputDataset) throws Exception  {
		 ...
		 OTDataset subsetToCalculate = subsetWithoutPredictedValues(inputDataset);
		 if (subsetToCalculate == null) { //probably remote dataset
		 ...
		 if (dataset_service.getParentRef().isParent(getUri())) //ours
				 return OTDataset.dataset(inputDataset.getUri()).withDatasetService(dataset_service).
					 		removeColumns().addColumns(OTFeature.feature(String.format("%s/predicted",getUri())));
			 else
				 return OTDataset.dataset(inputDataset.getUri()).withDatasetService(dataset_service).
			 		removeColumns().addColumns(getPredictedVariables());	
		 }
	 ...
	 */
	 ...
	 public OTDataset predict(OTDataset subsetToCalculate) throws Exception  {
			 ...
			 if ((task.getResult()==null) || !task.getResult().toString().startsWith("http"))
				 throw new Exception(String.format("Error when running task at %s Bad Result: %s",task.getUrl(),task.getResult()));
	 ...
	 public OTRemoteTask processAsync(OTDataset inputDataset) throws Exception  { 
		...
		return new OTRemoteTask(new Reference(getUri()),MediaType.TEXT_URI_LIST,params.getWebRepresentation(),Method.POST);
	 ...
	 public OTModel withDatasetService(String uri) throws Exception { 
		...
		public static String getSparql(String name)  throws Exception {
			InputStream in  = OTModel.class.getClassLoader().getResourceAsStream(name);
