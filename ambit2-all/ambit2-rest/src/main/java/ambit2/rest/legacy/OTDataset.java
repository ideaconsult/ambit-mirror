...
/**
 * Implementation of OpenTox API {@link http://opentox.org/dev/apis/api-1.1/dataset}
 * @author nina
 *
 */
@Deprecated
public class OTDataset extends OTObject implements IOTDataset {
	...
	protected String source;
	 ...
	 public static OTDataset createDataset(Representation entity, String dataset_service) throws ResourceException {
			OTRemoteTaskPool pool = new OTRemoteTaskPool();
			try {
				
				OTRemoteTask task = new OTRemoteTask(new Reference(dataset_service),
							MediaType.TEXT_URI_LIST,entity,
							org.restlet.data.Method.POST);
				
				pool.add(task);
				pool.run();
				if (Status.SUCCESS_OK.equals(task.getStatus())) {
					return OTDataset.dataset(task.getResult().toString()).withDatasetService(dataset_service);	
	...
	public static OTDataset createDataset(File file,String mimeType, String dataset_service) throws ResourceException {
			try {
				MediaType media;
				if (mimeType==null) {
					media = ChemicalMediaType.guessMediaByExtension(file.getName().toLowerCase());
				} else media = new MediaType(mimeType);
				if (media == null) throw new ResourceException(Status.CLIENT_ERROR_BAD_REQUEST,"Unsuported file format");
				
				return createDataset(new FileRepresentation(file,media), dataset_service);
			} catch (ResourceException x) {
				throw x;
			} catch (Exception x) {
				throw new ResourceException(Status.SERVER_ERROR_BAD_GATEWAY,x.getMessage());
	 ...
	 public OTDataset copy() throws Exception  { 
		...
		OTRemoteTask task = new OTRemoteTask(new Reference(dataset_service),MediaType.TEXT_URI_LIST,form.getWebRepresentation(),Method.POST);
		task = wait(task,now);
	 ...
	 public synchronized OTDataset put(OTDatasets datasets) throws Exception  { 
			...
			if (getUri()==null)
				task = new OTRemoteTask(dataset_service,MediaType.TEXT_URI_LIST,params.getWebRepresentation(),Method.POST);
			else
				task = new OTRemoteTask(getUri(),MediaType.TEXT_URI_LIST,params.getWebRepresentation(),Method.PUT);
			task = wait(task,now);
			if (task.getError()!=null) throw task.getError();
			Reference ref = task.getResult();
			datasets.clear();
			if (ref.equals(getUri())) return this; else return dataset(getUri()).withDatasetService(dataset_service); 
	 ...
	 public OTDataset filterByFeature(OTFeature feature,boolean withFeatures)  throws Exception  {
		 ...
		 if (dataset_service==null) return null;
		 
		 Reference ref = new Reference(dataset_service.getParentRef());
		 
		 if (!ref.isParent(getUri())) return null;
		 if (!ref.isParent(feature.getUri())) return null;
	 ...
	 public OTDataset filterByFeature(OTFeature feature,boolean withFeatures)  throws Exception  {
		 ...
		 Reference ref = new Reference(dataset_service.getParentRef()).
		 	addSegment("filter").
		 	addQueryParameter(OpenTox.params.dataset_uri.toString(), getUri().toString()).
		 	addQueryParameter(OpenTox.params.condition.toString(), withFeatures?"yes":"no");
	 ...
	 protected OTDataset filterByFeatures(OTFeatures features,boolean withFeatures)  throws Exception  {
		 ...
		 for (OTFeature feature:features.getItems())
			 if (!dataset_service.getParentRef().isParent(feature.getUri())) return null;
	 ...
	 */
	 public OTDataset filteredSubsetWithoutFeatures(OTFeatures features)  throws Exception  {
		 if ((features==null) || (features.size()==0)) return this;
	 ...
	 public OTDataset removeColumns() throws Exception {
		 ...
		 Reference ref  = new Reference(String.format("%s:%s",getUri().getScheme(),getUri().getHierarchicalPart()));
		 ref.setQuery(form.getQueryString());
		 return dataset(ref).withDatasetService(dataset_service);
	 }
/*
	 public OTDataset addColumns(OTFeatures features) throws Exception {
		 if ((features == null) || (features.getItems()==null)) return this;
		 Reference newuri = getUri().clone();
		 String d = newuri.getQuery()==null?"?":"&";
		 StringBuilder b = new StringBuilder();
		 b.append(newuri.toString());
		 int count = 0;
		 for (OTFeature feature:features.getItems()) {
			 if (!feature.isSelected()) continue;
			 b.append(d);
			 b.append(String.format("%s=%s", OpenTox.params.feature_uris.toString(),Reference.encode(feature.getUri().toString())));
			 d="&";
			 count++;
		 }
		 if (count==0) return this;
		 //[] in feature_uris[] is encoded
			 //newuri.addQueryParameter(OpenTox.params.feature_uris.toString(), feature.getUri().toString());
		 return dataset(b.toString()).withDatasetService(dataset_service);
	 }
*/
	 public OTDataset addColumns(OTFeatures features) throws Exception {
	 ...
	 public OTFeatures retrieveFeatures(OTFeatures features,String referer) throws Exception {
		 ...
		 if (getUri().getQuery() != null) {
			 ...
			 Form form = ref.getQueryAsForm();
			 String[] array = form.getValuesArray(OpenTox.params.feature_uris.toString());
