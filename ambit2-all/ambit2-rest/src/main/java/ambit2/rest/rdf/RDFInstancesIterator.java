...
public class RDFInstancesIterator extends RDFDataEntryIterator<Instance, Attribute> {
	...
	StmtIterator dataset = jenaModel.listStatements(new SimpleSelector(null, RDF.type, OT.OTClass.Dataset
		.getOntClass(jenaModel)));
	while (dataset.hasNext()) {
	    instances = new Instances(dataset.next().getSubject().toString(), attributes, 0);
	    break;
	}
	...
	Attribute a = null;
	try {
	    if (((Resource) feature).getProperty(RDF.type).getObject()
		    .equals(OTClass.NumericFeature.getOntClass(jenaModel)))
	...
	if (a == null)
	    a = new Attribute(feature.toString(), (FastVector) null);
	...
	String id = null;
	// get the compound
	StmtIterator compound = jenaModel.listStatements(new SimpleSelector((Resource) newEntry, OT.OTProperty.compound
	...
	while (features.hasNext()) {
	    ...
	    StmtIterator nominalFeatures = jenaModel.listStatements(new SimpleSelector(feature.getSubject(), RDF.type,
		    nominalFeatureClass));
	    ...
	    StmtIterator entries = jenaModel.listStatements(new SimpleSelector(null, featureProperty, feature
		    .getSubject()));
		...
		try {
			...
			if (!isNominal) {
				...
				isNominal = true; // strings become nominals,
