...
public class TestUtilities {
	...
	static SmartsHelper smartsHelper = new SmartsHelper(
			SilentChemObjectBuilder.getInstance());
	static SmartsParser sp = new SmartsParser();
	// static SmilesParser smilesparser = new
	...
	public void preProcessProduct(IAtomContainer mol) throws Exception {
		 ...
		 * CDKConstants.UNSET); atom.setFlag(CDKConstants.ISAROMATIC, false);
		 * atom.setProperty("org.openscience.cdk.renderer.color",
		 * CDKConstants.UNSET); atom.setAtomicNumber((Integer)
		 * CDKConstants.UNSET); atom.setExactMass((Double) CDKConstants.UNSET);
		 */
	...
	public static void printSmartsTokens(String smarts) {
		...
		/*
		 * for (int i = 0; i < qac.getAtomCount(); i++) if (qac.getAtom(i)
		 * instanceof SmartsAtomExpression) System.out.println(
		 * SmartsHelper.getAtomExpressionTokens
		 * ((SmartsAtomExpression)qac.getAtom(i)));
		 */
	...
	public void testSmartsManagerBoolSearchMDL(String smarts, String mdlFile) {
		try {
			...
			IteratingSDFReader reader = new IteratingSDFReader(
					new FileReader(mdlFile), b);

			if (!reader.hasNext()) {
				System.out.println("Could find a molecule in the file: "
						+ mdlFile);
				return;
			}
			...
			if (o instanceof IAtomContainer) {
				...
				boolean res = man.searchIn(mol);
				System.out.println("Man_search " + smarts
						+ " in MDL_FILE   -->  " + res);
			}
		} catch (Exception e) {
		...
		try {
			...
			while (reader.hasNext()) {
				...
				if (o instanceof IAtomContainer) {
					...
					for (int i = 0; i < n; i++)
						ringSizeSum += ringSet.getAtomContainer(i)
								.getAtomCount();
	...
	public void showFullAtomMappingsCDKIsomorphism(String smarts, String smiles)
			throws Exception {
		IAtomContainer mol = SmartsHelper.getMoleculeFromSmiles(smiles);
		...
		System.out.println("Show full atom/bond  mapping for Man_search of  "
		...
		for (Object aBondMapping : bondMapList) {
			...
			System.out.print("  bond mapping: "
					+ bondMappingToString((List) aBondMapping, mol));

			List<IAtom> atomMapping = man.generateFullAtomMapping(
					(List) aBondMapping, mol, man.getQueryContaner());
	...
	String bondMappingToString(List bondMapping, IAtomContainer target) {
		...
		for (int i = 0; i < bondMapping.size(); i++) {
			...
			sb.append(i + "->(" + at0.getSymbol() + at0Index + ","
					+ at1.getSymbol() + at1Index + ")  ");
		}
	...
	public void showFullIsomorphismMappings(String smarts, String smiles)
		...
		for (int k = 0; k < mappingSet.getAtomContainerCount(); k++) {
			...
			System.out
					.print("      " + SmartsHelper.moleculeToSMILES(ac, true));
			// System.out.println();
		}
		System.out.println();
	}
	...
	public void testIsomorphismPositions(String smarts, String smiles)
		...
		System.out.println("Isomorphism Positions: " + smarts + "  in  "
				+ smiles);
	...
	// helper function
	public String getBondMapping(IAtomContainer mol, List<IAtom> amap) {
	...
	public void testIsomorphismMapping(String smarts, String smiles)
		...
		System.out
				.println("Isomorphism Mapping: " + smarts + "  in  " + smiles);
	...
	public void testAtomSequencingFromFile(String fname) {
		...
		try {
			...
			while (f.getFilePointer() < length) {
				...
				String frags[] = SmartsHelper
						.getCarbonSkelletonsFromString(line);
				for (int k = 0; k < frags.length; k++) {
					...
					isoTester.setQuery(query);
					boolean res = checkSequence(query, isoTester.getSequence());
					System.out.println(frags[k].trim() + " -- > "
							+ (res ? "OK" : "FAILED"));
					if (!res)
						nError++;
					nTests++;
	...
	public boolean checkSequence(IQueryAtomContainer query,
			List<QuerySequenceElement> sequence) {
		ChemObjectFactory factory = new ChemObjectFactory(
				SilentChemObjectBuilder.getInstance());
		IAtomContainer skelleton = factory.getCarbonSkelleton(sequence);
		// System.out.println("skelleton = " +
		// SmartsHelper.moleculeToSMILES(skelleton));
		try {
			boolean res = uit.isSubgraph(skelleton, query);
			return (res);
	...
	int testSMARTSBondToIBond(String smarts) {
		...
		System.out.println(smarts
				+ "  --> bond list converted to simple bonds when possible: ");
		for (int i = 0; i < query.getBondCount(); i++) {
			...
			if (bo == null)
				...
				System.out.println("#" + i + "  "
						+ SmartsHelper.bondOrderToIntValue(bo) + "  "
						+ aromatic + "           class = "
						+ query.getBond(i).getClass().getName());
			}
		}
	...
	int testExtractAtomContainer(String smarts) throws Exception {
		...
		System.out.println(smarts);
		IAtomContainer mol2 = SmartsHelper.getMoleculeFromSmiles(smarts);
		...
		try {
			UniversalIsomorphismTester uit = new UniversalIsomorphismTester();
			boolean res = uit.isSubgraph(mol, query);
			if (!res)
				return (3);
		} catch (CDKException e) {
			System.out.println(e.getMessage());
	...
	public void testWithFile(String fname, String testType) {
		...
		if ((!testType.equals("ExtractAtomContainer"))) {
			...
			System.out
					.println("Following ones are allowed: ExtractAtomContainer");
			return;
		}
		int nError = 0;
		int nTests = 0;
	...
	public void testSmartsManagerAtomMapping(String smarts, String smiles)
		...
		System.out.println(smarts + " mapped against " + smiles + " gave "
				+ atoms.size() + " atoms:");
	...
	public void testAtomIndexesForMapping(int nGA, int nTA) {
		...
		for (int i = 0; i < nTA; i++) {
			// if (el.atoms[0].matches(targetAt.get(i)))
			{
				...
				/*
				 * System.out.print("push in "); for(int k = 0; k < t.length-1;
				 * k++) System.out.print(t[k]+" "); System.out.println();
				 */
			}
		}
		...
		while (!st.isEmpty()) {
			...
			for (int i = 0; i < nTA; i++) {
				...
				// if (el.atoms[n].matches(targetAt.get(i)))
				{
					...
					/*
					 * System.out.print("push in "); for(int k = 0; k <
					 * t.length-1; k++) System.out.print(tnew[k]+" ");
					 * System.out.println();
					 */
				}
			}
	...
	public void testCML(String smiles) throws Exception {
		...
		// mol.getAtom(0).getProperty("xy").toString());
		// System.out.println("XXMyProperty2 = " +
		// mol.getAtom(0).getProperty("XXMyProperty2").toString());
		...
		try {
			...
			for (int i = 0; i < mol2.getAtomCount(); i++) {
				System.out.println("AtType" + i + " = "
						+ mol2.getAtom(i).getSymbol());
				System.out.println(SmartsHelper.atomPropertiesToString(mol2
						.getAtom(i)));
			}

			// System.out.println(SmartsHelper.atomPropertiesToString(mol2.getAtom(0)));
			// System.out.println("get - x = " +
			// mol2.getAtom(0).getProperty("x").toString());
			// System.out.println("get - y = " +
			// mol2.getAtom(0).getProperty("y").toString());

			// System.out.println("Properties Map Size = " +
			// mol2.getAtom(0).getProperties().size());
			// System.out.println("MyProperty Type = " + mol2.getAtom(0).
			// getProperty("MyProperty").getClass().getName());
			//

			// System.out.println("RingData2 = " +
			// mol2.getAtom(0).getProperty("RingData2").toString());

			// CMLReader reader = new CMLReader("\\test.cml");
			// CMLReader reader = new CMLReader();
			// IChemFile chemFile =
			// SilentChemObjectBuilder.getInstance().newChemFile();
			// reader.read(chemFile);
			// Molecule m = (Molecule)reader.read(new Molecule());

			/*
			 * FileWriter output = new FileWriter("\\molecule.cml"); CMLWriter
			 * cmlwriter = new CMLWriter(output); cmlwriter.write(mol);
			 * cmlwriter.close();
			 */
		} catch (Exception e) {
			System.out.println(e.toString());
	...
	void testProduceStructuresExhaustively(String smiles, int maxNumSteps)
			throws Exception {
		System.out.println("Producing structs form " + smiles
	...
	void printSequence(String smiles) throws Exception {
		...
		for (int i = 0; i < cof.sequence.size(); i++)
			System.out.println("SeqEl " + i + " : "
					+ cof.sequence.get(i).toString());
	}
	...
	void produceRandomStructures() throws Exception {
		ChemObjectFactory cof = new ChemObjectFactory(
				SilentChemObjectBuilder.getInstance());
		List<StructInfo> vStr = new ArrayList<StructInfo>();
		cof.produceRandomStructsFromMDL(
				"../src/test/resources/einecs/einecs_structures_V13Apr07.sdf",
	...
	void makeStructureStatistics() throws Exception {
		ChemObjectFactory cof = new ChemObjectFactory(
				SilentChemObjectBuilder.getInstance());
		cof.performStructureStatistics(
				"/exhaustive-str-set-ambit-first-900str.txt",
				"../src/test/resources/einecs/einecs_structures_V13Apr07.sdf",
				100, 5000, "/frags_stat_ambit_5000.txt");
	}

	void filterStructsBySize(String inputSmilesFile, String outSmilesFile,
			int maxNumAtoms) {
	...
	int compareIsoTester(String smarts, String mdlFile) {
		...
		try {
			...
			while (reader.hasNext()) {
				...
				if (o instanceof IAtomContainer) {
					...
					CDKRes = uit.isSubgraph(mol, query);
					isoTester.setQuery(query);
		...
		System.out.println("  numDiff = " + numDiff + "  numIso = " + numOfIso
				+ "   " + smarts);
		return (numDiff);
	}
	...
	void testStructureAnalysis(String mdlFile) {
		...
		System.out.println("Loaded " + analyzer.structures.size()
				+ " structures");
	...
	void compareIsoTesterMulti(String inputSmilesFile) {
		try {
			...
			while (f.getFilePointer() < length) {
				...
				if (res > 0)
					nFailedStr++;
				System.out.println("nFailedStr = " + nFailedStr
						+ "    nTest = " + n);
				System.out.println();
			}
			f.close();
	...
	void testScreeningKeys(String target, String queryString) throws Exception {
		...
		IAtomContainer mol = SmartsHelper.getMoleculeFromSmiles(target);
	...
	public void testCheckStereoElements(String targetSmiles) throws Exception
	{
		IAtomContainer target = SmartsHelper
				.getMoleculeFromSmiles(targetSmiles);
		...
		System.out.println(targetSmiles);
	...
	public void testSMIRKS(String smirks, String targetSmiles, SmartsConst.SSM_MODE SSMode)
			throws Exception {
		FlagSSMode = SSMode;
	...
	public void testSMIRKS(String smirks, String targetSmiles,
			ReactionOperation operation) throws Exception {
	...
	public void testSMIRKS_(String smirks, String targetSmiles)
			throws Exception {
		System.out.println("Testing SMIRKS: " + smirks);
		SMIRKSManager smrkMan = new SMIRKSManager(
				SilentChemObjectBuilder.getInstance());
		...
		// System.out.println("reactant SMARTS: " +
		// smartsHelper.toSmarts(reaction.reactant) );
		// System.out.println("product SMARTS: " +
		// smartsHelper.toSmarts(reaction.product) );
		...
		System.out
				.println("    " + SmartsHelper.moleculeToSMILES(target, true));
	...
	public void testSmiles2Smiles(String smiles) throws Exception {
		...
		String smiles3 = smiGen.create(target);
		// String smiles4 = smiGen.createChiralSMILES(target, new boolean[0]);
	...
	public void testSmiles2MOLFile(String smiles, String molFileName)
		...
		// This is specialized Ambit version of the MDL Writer (special hack to
		// code aromatic bonds as bonds of type 4)
		// MDLWriter writer = new MDLWriter(new FileWriter(new
		// File(molFileName)));
		// writer.write((IAtomContainer)target);
		// writer.close();

		MDLV2000Writer writer = new MDLV2000Writer(new FileWriter(new File(
				molFileName)));
	...
	public void testMOL2MOLFile(String inMolFileName, String outMolFileName)
			throws Exception {
		System.out.println("Testing molecule from file: " + inMolFileName);

		IteratingSDFReader reader = new IteratingSDFReader(new FileReader(
				new File(inMolFileName)),
	...
	public void testBondIndexChange(String smarts) throws Exception {
		...
		/*
		 * boolean res = fixAtomIndexes(q); System.out.println("Fixing res = " +
		 * res); IAtomContainer mol1 = smToChemObj.extractAtomContainer(q);
		 * System.out.println(SmartsHelper.moleculeToSMILES(mol1)); for (int i =
		 * 0; i < q.getBondCount(); i++) { IBond b = q.getBond(i); int ind0 =
		 * q.getAtomNumber(b.getAtom(0)); int ind1 =
		 * q.getAtomNumber(b.getAtom(1));
		 * 
		 * System.out.println("bond #" + i + "   --> " +ind0 + "  " + ind1); }
		 */
	}

	public void testPreprocessing(String smiles, boolean FlagConnectAtom)
			throws Exception {
		...
		// SMIRKSManager smrkMan = new
		// SMIRKSManager(SilentChemObjectBuilder.getInstance());
	...
	public void testSMIRKS_JoergTestCase() throws Exception {
		 ...
		 * ValidationReport r = null; r =
		 * validatorEngine.validateAtomContainer(m); String transformedSmiles =
		 * SmartsHelper.moleculeToSMILES(m);
		 * System.out.println(transformedSmiles);
		 * 
		 * if (r == null) System.out.println("ValidationReport is null"); else {
	...
	public void myValidation(IAtomContainer mol) {
		...
		for (int i = 0; i < mol.getAtomCount(); i++) {
			...
			if (a == null)
				...
				System.out.println("atom #" + i + "  " + a.getSymbol() + "  "
						+ a.getImplicitHydrogenCount());
		}
	}

	public void testCOF_FileOperations(String inFile, String outFile)
			throws Exception {
		ChemObjectFactory cof = new ChemObjectFactory(
				SilentChemObjectBuilder.getInstance());
		ArrayList<ArrayList<String>> smiSets = cof
				.loadSmilesTuplesFromFile(inFile);
	...
	public void testBug78() throws Exception {
		SMIRKSManager smrkMan = new SMIRKSManager(
				DefaultChemObjectBuilder.getInstance());
		SMIRKSReaction reaction = smrkMan
				.parse("[H:3][C:1]#[C]>>[H:3][#6:1]=O");
		if (!smrkMan.getErrors().equals("")) {
			System.out.println(smrkMan.getErrors());
			return;
		}
		// IAtomContainer substrate = FormatConverter.fromSmiles("C#C");
		IAtomContainer substrate = SmartsHelper.getMoleculeFromSmiles("C#C",
				true);

		System.out.println(smrkMan.applyTransformationWithSingleCopyForEachPos(
				substrate, null, reaction));
	...
	public static void main(String[] args) throws Exception {
		...
		// tu.testSmartsManagerBoolSearch("(CCC.C1CC12CC2).C=CC#C.CCN.(ClC)",
		// "CCCCC");
		// tu.testSmartsManagerBoolSearch("(CCCC.CC.CCCN).N.C", "CCCCC.CCCN");
		// tu.testSmartsManagerBoolSearch("(Cl.CCCC.CC.CCCCN).N.C",
		// "CCCCC.CCCN");
		// tu.testSmartsManagerBoolSearch("(CCCC.CC).(CCCN).N.C", "CCCCC.CCCN");
		// tu.testSmartsManagerBoolSearch("(CCBr.CCN).(OCC)",
		// "BrCCCCC.CCCN.OCCC");
		// tu.testSmartsManagerBoolSearch("(CCBr).(CCN).(OCC)",
		// "BrCCCCC.CCCN.OCCC");
		...
		// tu.testProduceStructuresExhaustively("c1cc(Br)ccc1CC(CCC)CCNNCC2CCCCCCCC2",
		// 12);
		...
		// Testing SSSR algorithm wether it gives exactly the same numbering of
		// the rings
		...
		// tu.compareIsoTester("C1(C=C)(C(C(CC2C1C(O)CC3(C(C(CO)=O)(O)CCC23)C)F)=CC=O)C",null);
		// //OK
		...
		// tu.testSmartsManagerBoolSearch("[C;]", "CCC"); //----------> gives an
		// Exception at
		// ambit2.smarts.SmartsLogicalExpression.doAND(SmartsLogicalExpression.java:71)
		...
		// tu.testSmartsManagerBoolSearch("[C^1; +]C","CCCC"); //the space
		// within atom expression does not cause parser error !!!
		...
		// tu.testSmartsManagerBoolSearch("CC=C","C1=CC=CC=C1"); //The is false
		// because "CC=C" is interpreted as aliphatic fragment
		// tu.testSmartsManagerBoolSearch("CC=C","C1=CC=CC=C1-C2=CC=CC=C2");
		// //The is false because "CC=C" is interpreted as aliphatic fragment
		// tu.testSmartsManagerBoolSearch("cc=,:c","C1=CC=CC=C1-C2=CC=CC=C2");
		// //The result is true
		...
		// man.supportDoubleBondAromaticityNotSpecified(true);
		...
		// tu.testSmartsManagerBoolSearch("[$([S;H0;D2])!$([S;H0;D2]([$([CX4]),$(C(=O)C)])[$([CX4]),$(C(=O)C)])]",
		// "CCSC=NC");
		// tu.testSmartsManagerBoolSearch("[$([S;H0;D2])!$([S;H0;D2]([CX4])[CX4])!$([S;H0;D2]([CX4])C(=O)C)!$([S;H0;D2](C(=O)C)C(=O)C)]",
		// "CCSC=NC");
		...
		// These examples with a great probability proof that there is a bug in
		...
		// tu.testSmartsManagerBoolSearch("[N,$(C=S)]~[*]~[*]~[N,$(C=S)]","C(=S)N");
		// //[*] matches [H]
		// tu.testSmartsManagerBoolSearch("[N,$(C=S)]~*~*~[N,$(C=S)]","C(=S)N");
		// //false because * does not match [H]
		...
		// tu.testSMIRKS("[H][#6:1]:1:[#6:5]:[#6:6]:[#6:7]:[#8:4]:1>>[OH1]-[#6:1]1:[#6:5]:[#6:6]:[#6:7]:[#8:4]:1",
		// "o1cccc1");
		// tu.testSMIRKS("[H][#6:1]:1:[#6:5]:[#6:6]:[#6:7]:[#8:4]:1>>[OH1]-[#6:1]1:[#6:5]:[#6:6]:[#6:7]:[#8:4]:1",
		// "O1C=CC=C1");
		...
		// tu.testSMIRKS("[N:1][C:2][C:3][C:4]>>[C:4]=[C:3].[C:2]=[N----:1]Cl",
		// "SNCCCN");
		// tu.testSMIRKS("[N:1][C:2]([C:3])>>[N:1][C].[C:2]=[O]", "NCC"); //--->
		// Parser error is produced (Exception is fixed now) !!!!!
		// tu.testSMIRKS("[N:1][C:2]([C:3])>>[N:1][C].[C:2]=[O:4]", "NCC");
		// //---> Parser error is produced (Exception is fixed now) !!!!!
		// tu.testSMIRKS("[N:1][C:2]([C])>>[N:1][C].[C:2]=[O:4]", "NCC"); //--->
		// Parser error is produced !!!!!
		// tu.testSMIRKS("[N:1][C:1]([C])>>[N:1][C].[C:2]=[O]", "NCC"); //--->
		// Should produce duplciation index errors (it is fixed) !!!!!
		// tu.testSMIRKS("[N:1][C:2]([H])>>[N:1][H].[Cl-][C:2]=[O]",
		// "[H]N(C)C[H]");

		// Ambit-SMARTS/SMIRKS parser does not require atom ampping info (":n")
		// to be at the end of the atom expression
		// tu.testSMIRKS("[N;+:1][C:2]([H])>>[N:1;+][H].[Cl-][C:2]=[O]",
		// "[H][N+1](C)C[H]");

		// tu.testSMIRKS("[N;!$(N=O):1][C:2]>>[N:1]Cl.[C:2]", "NCC");
		// tu.testSMIRKS("[N;!$(N=O):1][C:2]>>[N;!$(N=O):1]Cl.[C:2]",
		// "NCCCCCN=O"); //--> No more exception is thrown due to the recursive
		// SMARTS
		// tu.testSMIRKS("[N;$(N-O-Br):1][C:2]>>[N:1]Cl.[C:2]", "ONCC"); //-->
		// No more error is obtained due to the recursive SMARTS
		// tu.testSMIRKS("[N:1][C;!$([C;r5]);!$(CC=C):2]>>[N:1]Cl.[C:2]",
		// "NC1CCCCC1CCCC2CCC=CC2N");

		// tu.testSMIRKS("[O:1]([H:10])[c:2]1[cH:3][cH:4][c:5]([O:6][H:11])[cH:7][c:8]1[$(C),$(Cl),$(OC),$(CC):9]>>[O:1]=[C:2]1[CH:3]=[CH:4][C:5](=[O:6])[CH:7]=[C:8]1[$(C),$(Cl),$(OC),$(CC):9].[H:10][H:11]",
		// "[H]Oc1([H])c([H])(C)c([H])c([H])(O[H])c([H])c1([H])");
		// tu.testSMIRKS("[O:1]([H:10])[c:2]1[cH:3][cH:4][c:5]([O:6][H:11])[cH:7][c:8]1[C:9]>>[O:1]=[C:2]1[CH:3]=[CH:4][C:5](=[O:6])[CH:7]=[C:8]1[C:9].[H:10][H:11]",
		// "[H]Oc1c(C)cc(O[H])cc1");
		// tu.testSMIRKS("[O:1]([H:10])[c:2]1[cH:3][cH:4][c:5]([O:6][H:11])[cH:7][c:8]1[C]>>[O:1]=[C:2]1[CH:3]=[CH:4][C:5](=[O:6])[CH:7]=[C:8]1[C].[H:10][H:11]",
		// "[H]Oc1c(C)cc(O[H])cc1");

		// tu.testSMIRKS("[c:1]1[c:2][c:3][c:4][c:5][c:6]1>>[c:1]1[c:2]([O])[c:3]([O])[c:4][c:5][c:6]1",
		// "c1ccccc1");
		...
		// tu.testSMIRKS("[*;#6:1][C:2]#[C:3][*;#6:4]>>[*:1]/[C:2](/[H])=[*:3](/[H])[C:4]",
		// "COCCCC#CCCC");
		// tu.testSMIRKS("[*:1][C:2]#[C:3][*;#6:4]>>[*:1]/[C:2](/[H])=[C:3](/[H])[C:4]",
		// "COCCCC#CCCC");
		...
		// tu.testSMIRKS("[Cl:3].[N:4][O:1][C:2]>>[N:4].[Cl:3][O:1][C:2]","Cl.NOC");
		// //Multi component reactions do not work ???!!!!
		// tu.testSMIRKS("[N:4][O:1][C:2]>>[N:4].[Cl][O:1][C:2]","NOCF.NOC");
		// //Multi component reaction application ??? to be clarified
		...
		// tu.testSMIRKS("[H:99][O;X2:1]c1ccccc1>>C1CCC([O;X2:1]c2ccccc2)OC1.[H:99]","[H]Oc1ccccc1NN");
		// //benzene atom are not mapped and NN is disconnected
		...
		// tu.testSMIRKS("[#8:2]-[#6:3]>>[#8:2].[#6:3]=S","NCCCOCCCC",
		// ReactionOperation.SingleCopyForEachPos);
		// tu.testSMIRKS("[#8:2]-[#6:3]>>[#8:2].[#6:3]=S","NCCCOCCCCOCCCCOCCC",
		// ReactionOperation.CombinedOverlappedPos);
		// tu.testSMIRKS("[#8:2]-[#6:3]>>[#8:2].[#6:3]=S","NCCCOCCCCOCCCCOCCC",
		// ReactionOperation.SingleCopyForEachPos);
		// tu.testSMIRKS("[#6:1]-[#8:2]-[#6:3]>>[#6:1]-[#8:2].[#6:3]=O","NC1OC(C(=O)C1",
		// ReactionOperation.SingleCopyForEachPos);

		// tu.testSMIRKS("[#6:1]-[#8:2]-[#6:3]>>[#6:1]-[#8:2].[#6:3]=O","NC1OC(C(=O)C1",
		// SmartsConst.SSM_NON_IDENTICAL);
		// tu.testSMIRKS("[#8;$([#8]([#6])[#6]):2]-[#6:3]>>[#8;$([#8]([#6])[#6]):2].[#6:3]=O","NC1OC(C(=O)C1",
		// ReactionOperation.SingleCopyForEachPos);
		...
		// tu.testSMIRKS("[C:1][C:3][C:4][C:2]>>[C:1]1[C:3][C:4][C:2]~1",
		// "CCCC");
		// tu.testSMIRKS("[C:1]C>>[C:1]#&!=,-N", "CC");

		tu.FlagSSModeForSingleCopyForEachPos = SmartsConst.SSM_MODE.SSM_ALL;
		//tu.testSMIRKS(
		...
		//tu.testSMIRKS("[N:1]O>>[N:1]Cl", "C/C=C/CNO");
		...
		// tu.testMOL2MOLFile("D:/test-mol-chiral.mol",
		// "D:/test-mol-chiral-conv.mol");
		...
		// tu.structureStatisticsMDL(5000, "/einecs_structures_V13Apr07.sdf",
		// "/db-5000-str-stat.txt");
		...
		// List<Integer> pos =
		// SmartsHelper.getSmartsPositions("[*;r4,r5,r6,r7,r8](=*)=*",
		...
		*/
		...
		tu.testSmartsManagerBoolSearch("C[C@](C)(C)(N)C","CCCC");
