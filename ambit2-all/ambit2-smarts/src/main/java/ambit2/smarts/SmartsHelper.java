...
public class SmartsHelper {
	...
	static public String getAtomExpressionTokens(SmartsAtomExpression expression) {
		...
		for (int k = 0; k < expression.tokens.size(); k++) {
			sb.append("tok(" + expression.tokens.get(k).type + ","
					+ expression.tokens.get(k).param + ") ");
		}
	...
	static public String getAtomsExpressionTokens(IQueryAtomContainer query) {
		...
		for (int i = 0; i < query.getAtomCount(); i++) {
			if (query.getAtom(i) instanceof SmartsAtomExpression)
				sb.append("Expr["
						+ getAtomExpressionTokens((SmartsAtomExpression) query
								.getAtom(i)) + "] ");
			else
				sb.append(atomToString((SMARTSAtom) query.getAtom(i)) + " ");
		}
	...
	static public String getAtomsAttributes(IAtomContainer container) {
		...
		for (int i = 0; i < container.getAtomCount(); i++) {
			 ...
			 * 20024&atid=120024 sb.append(" NumH=" + (at.getHydrogenCount() +
			 * explHAt));
			 */
	...
	static public String stereoCenterToString(IAtomContainer container,
			ITetrahedralChirality chiral) {
		StringBuffer sb = new StringBuffer();
		...
		for (int i = 0; i < ats.length; i++)
			sb.append(" " + ats[i].getSymbol()
					+ container.getAtomNumber(ats[i]));
	...
	static public String getBondAttributes(IAtomContainer container) {
		...
		for (int i = 0; i < container.getBondCount(); i++) {
			...
			sb.append("  #" + i + " Atoms (" + at0_num + "," + at1_num
					+ ")   Order = " + bondOrderToIntValue(bo));
	...
	static public String bondToStringExhaustive(IQueryAtomContainer query,
			IBond bond) {
		StringBuffer sb = new StringBuffer();
		sb.append(bondToString(bond) + " "
				+ bondAtomNumbersToString(query, bond) + "  "
				+ atomToString(bond.getAtom(0)) + " "
				+ atomToString(bond.getAtom(1)) + "\n");
		return (sb.toString());
	}
	...
	static public IQueryAtomContainer getQueryAtomContainer(IAtomContainer ac,
			boolean HandleAromaticity) {
		IQueryAtomContainer query = new QueryAtomContainer(ac.getBuilder());
			...
			if (b.getOrder() == IBond.Order.TRIPLE)
				...
				if (b.getOrder() == IBond.Order.DOUBLE)
					...
					if (HandleAromaticity) {
						...
						if (isArom)
							newBo = new SingleOrAromaticBond(ac.getBuilder());
						else
							newBo = new SingleNonAromaticBond(ac.getBuilder());
					} else
	...
	static public String atomToString(IAtom a) {
		...
		// Class SymbolQueryAtomAromaticityNotSpecified would be process here as
		// well
		return (a.getSymbol());
	}
	...
	static public String bondAtomNumbersToString(IAtomContainer container,
			IBond b) {
		return (" " + container.getAtomNumber(b.getAtom(0)) + " " + container
				.getAtomNumber(b.getAtom(1)));
	}
	...
	public String toSmarts(IQueryAtomContainer query) {
		...
		traversedAtoms.clear(); // The list of atoms which were traversed
								// (included into Smarts string)
	...
	public static String moleculeToSMILES(IAtomContainer mol,
			boolean useAromaticityFlag) throws Exception {
	...
	static public String atomPropertiesToString(IAtom atom) {
		...
		for (int i = 0; i < keys.length; i++) {
			if (keys[i].toString().toString().equals(CMLUtilities.RingData)
	...
	static public List<Integer> getSmartsPositions(String smartsQuery,
	...
	public static void preProcessStructure(IAtomContainer mol,
			boolean HandleAromaticity, boolean AddImplicitHAtoms)
			throws Exception {
		AtomContainerManipulator.percieveAtomTypesAndConfigureAtoms(mol);
	...
	public static void preProcessStructure(IAtomContainer mol,
			boolean HandleAromaticity, boolean AddImplicitHAtoms,
			boolean ImplicitHToExplicit) throws Exception {
		AtomContainerManipulator.percieveAtomTypesAndConfigureAtoms(mol);
