...
public class IsomorphismTester {
	...
	public void setQuery(IQueryAtomContainer container) throws EmptyMoleculeException {
											...
											if ((k != i) && (k != j))
												if (el.atoms[2].matches(targetAt.get(k)))
													if (matchBond(node, el, 2, targetAt.get(k))) {
														...
														// stack.push(newNode);
														if (newNode.sequenceElNum == sequence.size()) {
															// The node is not added in the stack if the end of the
															// sequence is reached
															boolean FlagOK = true;
															if (FlagCheckStereoElements)
																FlagOK = checkStereoMatching(newNode);
	 ...
	 * if (ambitStereo == SmartsConst.ChC_AntiClock) { if (stereo ==
	 * ITetrahedralChirality.Stereo.ANTI_CLOCKWISE) return true; } else if
	 * (ambitStereo == SmartsConst.ChC_Clock) { if (stereo ==
	 * ITetrahedralChirality.Stereo.CLOCKWISE) return true; }
	 * 
	 * return false; }
	 */
