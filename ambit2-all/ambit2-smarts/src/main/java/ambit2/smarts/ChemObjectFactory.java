...
/**
 * Utilities for generation of different types of chemical objects: molecules,
 * fragments, queries
 */
public class ChemObjectFactory {
	...
	public void setAtomSequence(IAtomContainer target, IAtom startAtom) {
		// This function is implemented analogously to
		// IsomorphismTester.setQueryAtomSequence
		...
		while (!stack.empty()) {
			...
			for (int i = 0; i < curSeqAt.atoms.length; i++) {
				topLayer = (TopLayer) curSeqAt.atoms[i]
						.getProperty(TopLayer.TLProp);
				if (topLayer.atoms.size() == 1)
					continue; // it is terminal atom and no further sequencing
								// should be done
				...
				for (int k = 0; k < a.length; k++) {
					if (a[k] == 0) {
						...
						if (curSeqAt.center == topLayer.atoms.get(k))
							continue;
						// Check whether
						// bond(curSeqAt.atoms[i]-topLayer.atoms.get(k))
						// is already sequenced
						if (getSeqBond(curSeqAt.atoms[i], topLayer.atoms.get(k)) != -1)
							continue;
						// topLayer.atoms.get(k) atom is already sequenced.
						// Therefore sequnce element of 'bond' type is
						// registered.
						// newSeqEl is not added in the stack (this is not
		...
		for (int i = 0; i < sequencedBondAt1.size(); i++) {
			if (sequencedBondAt1.get(i) == at1) {
				if (sequencedBondAt2.get(i) == at2)
					return (i);
			} else if (sequencedBondAt1.get(i) == at2) {
	...
	public IAtomContainer getFragmentFromSequence(int numSteps) {
		...
		HashMap<IAtom, IAtom> m = new HashMap<IAtom, IAtom>(); // Mapping
																// original -->
																// copy
		SequenceElement el;

		// Processing first sequence element (The first center is always
		// different from null
		el = sequence.get(0);
		...
		for (int k = 0; k < el.atoms.length; k++) {
			...
			mol.addAtom(a);
			m.put(el.atoms[k], a);
			addBond(mol, m.get(el.center), a, el.bonds[k].getOrder(),
					el.bonds[k].getFlag(CDKConstants.ISAROMATIC));
		}
		...
		for (int i = 1; i <= numSteps; i++) {
			...
			if (el.center == null) // Handling a closing bond
			{
				addBond(mol, m.get(el.atoms[0]), m.get(el.atoms[1]),
						el.bonds[0].getOrder(),
						el.bonds[0].getFlag(CDKConstants.ISAROMATIC));
			} else {
				for (int k = 0; k < el.atoms.length; k++) {
					IAtom a = getAtomCopy(el.atoms[k]);
					mol.addAtom(a);
	...
	void addBond(IAtomContainer mol, IAtom at1, IAtom at2, IBond.Order order,
			boolean isAromatic) {
	...
	public IAtomContainer getCarbonSkelleton(List<QuerySequenceElement> sequence) {
		...
		for (int k = 0; k < el.atoms.length; k++) {
			...
			mol.addAtom(a);
			// System.out.println("## --> " +
			// SmartsHelper.atomToString(el.atoms[k]));
			m.put(el.atoms[k], a);
	...
	void connectFragmentToMolecule(IAtomContainer base,
			IAtomContainer fragment, int bondType, int basePos, int fragPos) {
		// TODO
	}

	void connectFragmentToMoleculeSpiro(IAtomContainer base,
			IAtomContainer fragment, int basePos, int fragPos) {
		// TODO
	}

	void condenseFragmentToMolecule(IAtomContainer base,
			IAtomContainer fragment, int bondType, int basePos, int fragPos) {
		// TODO
	}
	...
	public void produceStructuresExhaustively(IAtomContainer mol,
			List<StructInfo> vStr, int maxNumSeqSteps, int maxStrSize)
			throws Exception {
	...
	public void produceStructuresRandomly(IAtomContainer mol,
			List<StructInfo> vStr, int maxNumSeqSteps, int numStructs,
			int minGenStrSize) {
	...
	boolean checkForDuplication(String smarts, List<StructInfo> vStr)
			throws Exception {
		man.setQuery(smarts);
	...
	public void produceStructsFromMDL(String mdlFile, int maxNumSeqSteps,
			int maxNumRecord, int maxStrSize, List<StructInfo> vStr,
	...
	public void produceRandomStructsFromMDL(String mdlFile, int maxNumSeqSteps,
			int minGenStrSize, int maxNumRecord, List<StructInfo> vStr,
			String outFile) throws Exception {
	...
	void saveStructStatistics(List<String> smiles, List<Integer> stat,
	...
	public void performStructureStatistics(String smilesFile, String mdlFile,
		...
		while (f.getFilePointer() < length) {
			...
			if (smiles.size() % portionSize == 0) {
				...
				List<Integer> stat = doStatisticsForStructs(smiles, mdlFile,
						nUsedStr);
				for (int i = 0; i < smiles.size(); i++) {
	...
	List<Integer> doStatisticsForStructs(List<String> smiles, String mdlFile,
			int nUsedStr) throws Exception {
		...
		try {
			reader = new IteratingSDFReader(new FileReader(mdlFile), builder);
			int record = 0;
			boolean res;
			...
			while (reader.hasNext()) {
				...
				if (o instanceof IAtomContainer) {
					...
					AtomContainerManipulator
							.percieveAtomTypesAndConfigureAtoms(mol);
					CDKHueckelAromaticityDetector.detectAromaticity(mol);
					// The target is not "prepared" since the queries are not
					// complicated expressions
					// parser.prepareTargetForSMARTSSearch
					// System.out.println("  $ record " + record+ "  " +
					// SmartsHelper.moleculeToSMILES(mol));

					for (int i = 0; i < n; i++) {
						res = uit.isSubgraph(mol,
								queries.get(i));
	...
	public IAtomContainer connectStructures(String smiles1, int numAt1,
			String smiles2, int numAt2, IBond.Order order) throws Exception {
	...
	public IAtomContainer connectStructures_Spiro(IAtomContainer str1,
			int numAt1, IAtomContainer str2, int numAt2, IBond.Order order)
			throws Exception {
		// TODO
		return null;
	}

	public IAtomContainer condenseStructures(String smiles1, int str1At0,
			int str1At1, String smiles2, int str2At0, int str2At1)
			throws Exception {
		IAtomContainer mol1 = smilesParser.parseSmiles(smiles1);
		IAtomContainer mol2 = smilesParser.parseSmiles(smiles2);

		return condenseStructures(mol1, str1At0, str1At1, mol2, str2At0,
				str2At1);
	}

	public IAtomContainer condenseStructures(IAtomContainer str1, int str1At0,
			int str1At1, IAtomContainer str2, int str2At0, int str2At1)
			throws Exception {
		IAtom at0 = null;
		IAtom at1 = null;
	...
	public ArrayList<String> loadSmilesStringsFromFile(String smilesFile)
			throws Exception {
	...
	public void saveSmilesToFile(ArrayList<String> smiles, String fName)
			throws Exception {
		File file = new File(fName);
		RandomAccessFile f = new RandomAccessFile(file, "rw");
		f.setLength(0);
	...
	public ArrayList<ArrayList<String>> loadSmilesTuplesFromFile(
			String smilesFile) throws Exception {
	...
	public void saveSmilesTuplesToFile(
			ArrayList<ArrayList<String>> smilesTuples, String fName)
			throws Exception {
		File file = new File(fName);
		RandomAccessFile f = new RandomAccessFile(file, "rw");
		f.setLength(0);
	...
	public ArrayList<String> attachGroupToAllStructures(String groupSmiles,
			int groupAttachPos, ArrayList<String> inputSmiles, int strDefaultPos)
			throws Exception {
		ArrayList<String> outSmiles = new ArrayList<String>();
		for (int i = 0; i < inputSmiles.size(); i++) {
			IAtomContainer ac = connectStructures(groupSmiles, groupAttachPos,
					inputSmiles.get(i), strDefaultPos, IBond.Order.SINGLE);
