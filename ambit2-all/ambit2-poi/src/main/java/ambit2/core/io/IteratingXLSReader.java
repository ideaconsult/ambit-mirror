...
public class IteratingXLSReader extends
	...
	public IteratingXLSReader(InputStream input, Integer sheetIndex, Boolean hssf)
			throws CDKException {
		super();
		this.hssf = hssf;
	...
	protected void processHeader(Row row) {
		...
		TreeMap<Integer,Object> columns = new TreeMap<Integer,Object>();
		while (cols.hasNext()) {
		...
		Iterator<Map.Entry<Integer,Object>> i = columns.entrySet().iterator();
		while (i.hasNext()) {
			Map.Entry<Integer,Object> entry = i.next();
			Integer key = entry.getKey();
			setHeaderColumn(key.intValue(), entry.getValue().toString());
		}
	}
