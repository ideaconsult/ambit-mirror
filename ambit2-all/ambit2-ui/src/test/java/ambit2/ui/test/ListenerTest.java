...
public class ListenerTest {
	...
	@Test
	public void testListenerXLS() throws Exception {
		String[] labels = { "Code", "Compound", "SMILES", "MolWeigth", "CasRN",
				"log_P", "eLumo", "eHomo", "IL", "Dev.", "Pred", "abs dev",
				"Obs" };
		IIteratingChemObjectReader reader = new IteratingXLSReader(getClass()
				.getClassLoader().getResourceAsStream("Debnath_smiles.xls"), 0);
	...
	public void readFile(IIteratingChemObjectReader reader, String[] labels)
			throws Exception {
		// IF MEDIUM or LOW, the dialog box for field selections will appear
		AmbitSettingsListener listener = new AmbitSettingsListener(
				Importance.HIGH);
		...
		/*
		 * for (String s: labels) { Assert.assertNotNull(props.get(s)); }
		 */
	}
