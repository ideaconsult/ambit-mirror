...
public class FunctionalGroupDescriptor implements IMolecularDescriptor {
	protected static Logger logger = Logger
			.getLogger(FunctionalGroupDescriptor.class.getName());
	...
	protected void setGroups(List<FunctionalGroup> groups) {
		...
		for (FunctionalGroup group : groups)
			group.setVerboseMatch(isVerbose());
	}
	...
	public DescriptorValue calculate(IAtomContainer atomcontainer) {
		...
		try {
			IAtomContainer target = hp.process((IAtomContainer) atomcontainer
					.clone());

			List explanation = new ArrayList();

			for (int i = 0; i < groups.size(); i++) {
				VerboseDescriptorResult<Object, IntegerResult> result = groups
						.get(i).process(target);
			...
			return new DescriptorValue(getSpecification(), getParameterNames(),
					getParameters(), result, realNames.toArray(n));
		} catch (Exception x) {
	...
	public Object getParameterType(String name) {
		for (int i = 0; i < paramNames.length; i++)
			if (paramNames[i].equals(name))
				switch (i) {
				case 0:
					return new ArrayList<FunctionalGroup>();
				case 1:
					return false;
				default:
					return null;
	...
	public void setParameters(Object[] params) throws CDKException {
		if (params.length < 1)
			throw new CDKException(
					"FunctionalGroupDescriptor expects at least one parameter");

		if (!(params[0] instanceof List))
			throw new CDKException(
					"The first parameter must be of type List<FunctionalGroup> instead of "
							+ params[0].getClass().getName());

		if (params.length > 1)
			if (!(params[1] instanceof Boolean))
				throw new CDKException(
						"The second parameter must be of type Boolean instead of "
