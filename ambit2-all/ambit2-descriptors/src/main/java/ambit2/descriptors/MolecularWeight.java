 ...
 *  $RCSfile$
 *  $Author: rajarshi $
 *  $Date: 2008-10-26 00:48:10 +0300 (Sun, 26 Oct 2008) $
 *  $Revision: 12806 $
 *
...
/**
 * Temporary copied from CDK in order to specify the Carbon 12 atomic mass with
 * the right precision (see CDK bug ID ID: 2794591) IDescriptor based on the
 * weight of atoms of a certain element type.
 * 
 * If the wild-card symbol * is specified, the returned value is the molecular
 * weight. If an invalid element symbol is specified, the return value is 0 and
 ...
 * <td>elementSymbol</td>
 * <td>*</td>
 * <td>If *, returns the molecular weight, otherwise the weight for the given
 * element</td>
 * </tr>
 * </table>
 * 
...
public class MolecularWeight implements IMolecularDescriptor {
	 ...
	 * @throws CDKException
	 *             if more than 1 parameter is specified or if the parameter is
	 *             not of type String
	 * @see #getParameters
	 */
	...
	public DescriptorValue calculate(IAtomContainer container) {
		...
		if (elementName.equals("*")) {
			try {
				for (int i = 0; i < container.getAtomCount(); i++) {
					...
					weight += IsotopeFactory
							.getInstance(container.getBuilder())
							.getMajorIsotope(container.getAtom(i).getSymbol())
							.getExactMass();
					/*
					 * //Integer hcount =
					 ...
					 * =20024&atid=120024
					 */
					Integer hcount = container.getAtom(i)
				...
				return new DescriptorValue(getSpecification(),
						getParameterNames(), getParameters(), new DoubleResult(
								weight), getDescriptorNames(), e);
			}
		} else if (elementName.equals("H")) {
			try {
				IsotopeFactory factory = ambit2.descriptors.IsotopeFactory
						.getInstance(container.getBuilder());
				IIsotope h = factory.getMajorIsotope("H");
				for (int i = 0; i < container.getAtomCount(); i++) {
					if (container.getAtom(i).getSymbol().equals(elementName)) {
						weight += factory.getMajorIsotope(
								container.getAtom(i).getSymbol())
								.getExactMass();
					} else {
						/*
						 ...
						 * h.getExactMass());
						 */
						weight += (container.getAtom(i)
								.getImplicitHydrogenCount() * h.getExactMass());
					}
				}
			} catch (Exception e) {
				return new DescriptorValue(getSpecification(),
		...
		return new DescriptorValue(getSpecification(), getParameterNames(),
				getParameters(), new DoubleResult(weight), getDescriptorNames());
