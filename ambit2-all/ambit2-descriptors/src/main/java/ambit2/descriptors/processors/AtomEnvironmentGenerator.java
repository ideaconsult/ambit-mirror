 ...
 * {@link ambit2.data.descriptors.AtomEnvironmentDescriptor} for actual
 * generation. The result is contained in
 * {@link ambit2.data.descriptors.AtomEnvironmentList} and is assigned as a
 ...
 * @author Patrik Rydberg <b>Modified</b> Jan 3, 2012
 */
public class AtomEnvironmentGenerator extends
		AbstractPropertyGenerator<AtomEnvironmentList> {
	/**
	 * 
	 */
	...
	public AtomEnvironmentList generateProperty(IAtomContainer atomContainer)
		...
		try {
			...
			IAtomContainer mol = (IAtomContainer) ((IAtomContainer) atomContainer)
					.clone();
			...
			for (int a = 0; a < aeResult.length; a++) {
				...
				atomenv.setCentral_atom(aeDescriptor
						.atomTypeToString(aeResult[a][1]));
				atomenv.setAtomno(a);
				aeList.add(atomenv);
				...
				if (isCreateSubLevels())
					for (int i = 1; i < atomenv.getLevels(); i++) {
						AtomEnvironment subae = atomenv
								.getSubAtomEnvironment(i);
						subae.setSublevel(i);
