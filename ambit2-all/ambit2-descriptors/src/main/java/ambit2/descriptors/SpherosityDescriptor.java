 ...
 * 
 * @author Nina Jeliazkova nina@acad.bg <b>Modified</b> 2006-4-17
 */
public class SpherosityDescriptor implements IMolecularDescriptor {
	...
	public DescriptorValue calculate(IAtomContainer container) {
		try {
			...
			double spherosity = 3 * min
					/ (eval.get(0) + eval.get(0) + eval.get(0));
			return new DescriptorValue(getSpecification(), getParameterNames(),
					getParameters(), new DoubleResult(spherosity),
					getDescriptorNames());
		} catch (Exception x) {
			return new DescriptorValue(getSpecification(), getParameterNames(),
	...
	protected DescriptorValue getSizeDescriptorResult(IAtomContainer container)
			throws CDKException {
