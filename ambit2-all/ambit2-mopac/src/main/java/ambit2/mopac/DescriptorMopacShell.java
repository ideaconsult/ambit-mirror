 ...
 * Invokes mopac.exe from Mopac 7.1
 * http://openmopac.net/Downloads/Downloads.html.<br>
 * Doesn't work for molecules with number of heavy atoms > 60, number of all
 * atoms > 120.<br>
 * 
 * @author Nina Jeliazkova nina@acad.bg <b>Modified</b> 2006-4-8
 */
public class DescriptorMopacShell implements IMolecularDescriptor {
	static Logger logger = Logger.getLogger(DescriptorMopacShell.class
			.getName());

	protected String[] params = new String[] { "mopac_commands",
			"mopac_executable", "force_field" };
	...
	public String toString() {
		return "Calculates electronic descriptors by " + mopac_shell
				+ " (http://openmopac.net)";
	}
	...
	public void setParameters(Object[] arg0) throws CDKException {
		...
		try {
			...
			if (!file.exists())
				mopac_shell.addExecutable(AbstractMopacShell.defaultparams[1],
						null);
		} catch (Exception x) {
	...
	public DescriptorValue calculate(IAtomContainer mol) {
		...
		try {
			...
			if ((newmol == null) || (newmol.getAtomCount() == 0)) {
				throw new ShellException(mopac_shell, String.format(
						mopac_shell.getMsgemptymolecule(),
						mopac_shell.toString()));
			}
			...
			for (int i = 0; i < Mopac7Reader.parameters.length; i++)
				try {
					...
					if (value == null) {
						logger.warning(Mopac7Reader.parameters[i] + " = "
								+ value);
						r.add(Double.NaN);
					} else {
			...
			return new DescriptorValue(getSpecification(), getParameterNames(),
					getParameters(), r, Mopac7Reader.parameters);
		} catch (Exception x) {
			return new DescriptorValue(getSpecification(), getParameterNames(),
					getParameters(), r, Mopac7Reader.parameters, x);
		}
