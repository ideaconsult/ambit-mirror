...
public class AmbitRowsEventTest extends DbUnitTest {
	...
	protected AmbitRows<Dictionary> createDictionaryQuery() throws Exception {
		AmbitRows<Dictionary>  dictionary = new AmbitRows<Dictionary>() {
			@Override
			protected IQueryRetrieval createNewQuery(Dictionary target)
	...
	protected AmbitRows<Property> createPropertyQuery() throws Exception {
		AmbitRows<Property> properties = new AmbitRows<Property>() {
			@Override
			protected IQueryRetrieval createNewQuery(Property target)
					throws AmbitException {
				//retrieve values given property name
				Assert.assertTrue(
						"Property 1".equals(target.getName()) ||
	...
	public static void main(String[] args) {
		...
		try {
			...
			IDatabaseConnection c = test.getConnection("ambit2","33060","guest","guest");
			dictionary.setConnection(c.getConnection());
			properties.setConnection(c.getConnection());
	        ...
	        SelectionInList<Dictionary> selectionInList = new SelectionInList<Dictionary>(listModel,selectedItemHolder1);
	        ...
	        //ValueModel conditionModel = beanAdapter2.getValueModel("condition");
			...
			//p.add(new JComboBox(new ComboBoxAdapter<Dictionary>(selectionInList)));
