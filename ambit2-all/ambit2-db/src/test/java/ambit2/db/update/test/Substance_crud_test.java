...
public class Substance_crud_test extends CRUDTest<Object, SubstanceRecord> {
	...
	protected void createVerify(IQueryUpdate<Object, SubstanceRecord> query)
		...
		Assert.assertEquals(
				example_uuid,
				I5Utils.getPrefixedUUID(table.getValue(0, "prefix"),
						table.getValue(0, "u")));
		Assert.assertEquals(
				example_rs_uuid,
				I5Utils.getPrefixedUUID(table.getValue(0, "rs_prefix"),
						table.getValue(0, "rs_u")));
		...
		Assert.assertEquals("Multiconstituent",
				table.getValue(0, "substanceType"));
	...
	protected void deleteVerify(IQueryUpdate<Object, SubstanceRecord> query)
		...
		ITable table = c.createQueryTable("EXPECTED",
				"SELECT * FROM substance where idsubstance=1");
