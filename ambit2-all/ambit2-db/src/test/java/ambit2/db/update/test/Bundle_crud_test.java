...
public class Bundle_crud_test extends
		CRUDTest<Object, SubstanceEndpointsBundle> {
	@Override
	...
	public void setUpDatabaseFromResource(String resource) throws Exception {
		...
		ITable table = c.createQueryTable("EXPECTED",
				"SELECT * FROM bundle where idbundle=1");
		Assert.assertEquals(1, table.getRowCount());
		c.close();
	}

	@Override
	protected IQueryUpdate<Object, SubstanceEndpointsBundle> createQuery()
			throws Exception {
	...
	protected void createVerify(
			IQueryUpdate<Object, SubstanceEndpointsBundle> query)
			throws Exception {
		IDatabaseConnection c = getConnection();
		ITable table = c
				.createQueryTable(
	...
	@Override
	protected IQueryUpdate<Object, SubstanceEndpointsBundle> deleteQuery()
			throws Exception {
	...
	@Override
	protected IQueryUpdate<Object, SubstanceEndpointsBundle> updateQuery()
			throws Exception {
		SubstanceEndpointsBundle adataset = new SubstanceEndpointsBundle();
		adataset.setID(1);
	...
	protected void updateVerify(
		...
		Assert.assertEquals(ISourceDataset.license.CC0_1_0.getURI(),
				table.getValue(0, "licenseURI"));
		table = c.createQueryTable("EXPECTED",
				"SELECT * FROM bundle where name='Dataset 1'");
		Assert.assertEquals(0, table.getRowCount());
		c.close();
	}
	...
	public void testCreateNewVersion() throws Exception {
		SubstanceEndpointsBundle bundle = new SubstanceEndpointsBundle();
		bundle.setID(1);
		IQueryUpdate<Object, SubstanceEndpointsBundle> query = new CreateBundleVersion(
				bundle);
		setUpDatabaseFromResource(dbFile);
		IDatabaseConnection c = getConnection();
		executor.setConnection(c.getConnection());
	...
	protected void createNewVersionVerify(
			IQueryUpdate<Object, SubstanceEndpointsBundle> query)
			throws Exception {
		...
		Assert.assertEquals("EFDB21BBE79F3286A988B6F6944D3734",
				table.getValue(0, "bn"));
		Assert.assertEquals("EFDB21BBE79F3286A988B6F6944D3734",
				table.getValue(1, "bn"));
		Assert.assertEquals("draft", table.getValue(0, "published_status"));
	...
	public void testCreateNewCopy() throws Exception {
		...
		bundle.setID(1);
		IQueryUpdate<Object, SubstanceEndpointsBundle> query = new CreateBundleCopy(
				bundle);
		setUpDatabaseFromResource(dbFile);
		IDatabaseConnection c = getConnection();
		executor.setConnection(c.getConnection());
	...
	protected void createNewCopyVerify(
			IQueryUpdate<Object, SubstanceEndpointsBundle> query)
			throws Exception {
		...
		Assert.assertEquals("EFDB21BBE79F3286A988B6F6944D3734",
				table.getValue(0, "bn"));
		Assert.assertNotSame("EFDB21BBE79F3286A988B6F6944D3734",
				table.getValue(1, "bn"));
		Assert.assertEquals("draft", table.getValue(0, "published_status"));
