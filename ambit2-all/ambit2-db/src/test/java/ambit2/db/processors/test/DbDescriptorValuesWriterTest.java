...
public class DbDescriptorValuesWriterTest extends DbUnitTest {
	...
	/**
	 * Test method for
	 * {@link ambit2.db.processors.DbDescriptorValuesWriter#getStructure()}.
	 */
	@Test
	...
	/**
	 * Test method for
	 * {@link ambit2.db.processors.DbDescriptorValuesWriter#setStructure(ambit2.base.interfaces.IStructureRecord)}
	 * .
	 */
	@Test
	...
	public void testWriteDescriptorValue() throws Exception {
		...
		DescriptorValue v = new DescriptorValue(new DescriptorSpecification(
				...
				new DoubleResult(5.01), new String[] { "XLogP" });
		...
		values = c
				.createQueryTable("EXPECTED_VALUES",
						"SELECT value_num FROM property_values WHERE idstructure=100214");
		Assert.assertEquals(1, values.getRowCount());
		Assert.assertEquals(5.01, (Double) DataType.DOUBLE.typeCast(values
				.getValue(0, "value_num")), 1E-4);
		c.close();
	}
	...
	public void testWriteMultipleValues() throws Exception {
		...
		ITable values = c
				.createQueryTable("EXPECTED_VALUES",
						"SELECT * FROM property_values join properties using(idproperty)");
		Assert.assertEquals(2, values.getRowCount());
		...
		Assert.assertEquals(72.0644,
				((DoubleResult) value.getValue()).doubleValue(), 1E-4);
		writer.write(value);
		names = c
				.createQueryTable("EXPECTED_NAMES", "SELECT * FROM properties");
		Assert.assertEquals(6, names.getRowCount());
		values = c
				.createQueryTable(
						"EXPECTED_VALUES",
						"SELECT * FROM property_values join properties using(idproperty) WHERE abs(value_num-72.0644)<1E-4");
		Assert.assertEquals(1, values.getRowCount());
		values = c
				.createQueryTable(
						"EXPECTED_VALUES",
						"SELECT * FROM property_values join properties using(idproperty) WHERE abs(value_num-90)<1E-4");
		Assert.assertEquals(0, values.getRowCount());

		value = xlogp.calculate(MoleculeFactory.makeAlkane(12));
		Assert.assertEquals(144.1288,
				((DoubleResult) value.getValue()).doubleValue(), 1E-4);
		...
		values = c
				.createQueryTable(
						"EXPECTED_VALUES",
						"SELECT * FROM property_values join properties using(idproperty) WHERE abs(value_num-144.1288)<1E-4");
		Assert.assertEquals(1, values.getRowCount());
		values = c
				.createQueryTable(
						"EXPECTED_VALUES",
						"SELECT * FROM property_values join properties using(idproperty) WHERE abs(value_num-172)<1E-4");
		Assert.assertEquals(0, values.getRowCount());
	...
	public void testReadWriteProperty() throws Exception {
		...
		IDatabaseConnection c = getConnection();
		ITable names = c.createQueryTable("EXPECTED_FIELDS",
				"SELECT * FROM property_values");
		...
		while (reader.hasNext()) {
			...
			if (content == null)
				continue;
			IIteratingChemObjectReader mReader = new IteratingSDFReader(
					new StringReader(content), b);
		...
		c = getConnection();
		names = c.createQueryTable("EXPECTED_FIELDS",
				"SELECT * FROM property_values where value_num is not null");
		Assert.assertEquals(2, names.getRowCount());
		names = c
				.createQueryTable("EXPECTED_FIELDS",
						"SELECT * FROM property_values  where idstructure in (105095,109287)");
		Assert.assertEquals(2, names.getRowCount());
		c.close();
	...
	public IAtomContainer getTestMolecule() throws Exception {
		...
		InputStream in = SizeDescriptor.class.getClassLoader()
				.getResourceAsStream("ambit2/db/processors/sdf/224824.sdf");
	...
	public void testWriteDescriptorValueFromProfile() throws Exception {
		...
		ITable templates = c.createQueryTable("EXPECTED_TEMPLATES",
				"SELECT * FROM template");
		Assert.assertEquals(5, templates.getRowCount());
		ITable template_def = c.createQueryTable("EXPECTED_TEMPLATES",
		...
		values = c.createQueryTable("EXPECTED_VALUES",
				"SELECT * FROM property_values WHERE status ='OK' ");
		Assert.assertEquals(countValues, values.getRowCount());
		template_def = c.createQueryTable("EXPECTED_TEMPLATES",
				"SELECT type FROM catalog_references where type=\"Algorithm\"");
