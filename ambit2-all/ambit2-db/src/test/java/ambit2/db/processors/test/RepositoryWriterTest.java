...
public class RepositoryWriterTest extends DbUnitTest {
	...
	public void testWrite() throws Exception {
		...
		InputStream in = this.getClass().getClassLoader()
				.getResourceAsStream("ambit2/db/processors/test/input.sdf");
	...
	public void testWriteTXT() throws Exception {
		...
		InputStream in = this.getClass().getClassLoader()
				.getResourceAsStream("ambit2/db/processors/txt/test.txt");
		Assert.assertNotNull(in);
		IIteratingChemObjectReader reader1 = FileInputState.getReader(in,
				"test.txt");
		...
		ITable p_tuples = c.createQueryTable("EXPECTED",
				"SELECT * FROM property_tuples");
	...
	public void testWriteEmptySmiles() throws Exception {
		...
		InputStream in = this.getClass().getClassLoader()
				.getResourceAsStream("ambit2/db/processors/test/problem.sdf");
		Assert.assertNotNull(in);
		RawIteratingSDFReader reader = new RawIteratingSDFReader(
				new InputStreamReader(in));
	...
	public void testMultiStrucSameSmiles() throws Exception {
		...
		RawIteratingSDFReader reader = new RawIteratingSDFReader(
				new InputStreamReader(in));
		reader.setReference(LiteratureEntry
				.getInstance("Multi strucsame smiles"));
		write(reader, c.getConnection());
		c.close();
	...
	public void testMarkush() throws Exception {
		...
		InputStream in = this
				...
				.getClassLoader()
				.getResourceAsStream(
						"ambit2/db/processors/markush/68915-31-1.sdf");
		Assert.assertNotNull(in);
		RawIteratingSDFReader reader = new RawIteratingSDFReader(
				new InputStreamReader(in));
		...
		strucs = c.createQueryTable("EXPECTED",
				"SELECT * FROM structure where type_structure='MARKUSH'");
		Assert.assertEquals(1, strucs.getRowCount());
		srcdataset = c.createQueryTable("EXPECTED",
				"SELECT * FROM src_dataset where name='TEST INPUT'");
		Assert.assertEquals(1, srcdataset.getRowCount());
	...
	public void testECHAPreregistrationList() throws Exception {
		...
		InputStream in = this
				...
				.getClassLoader()
				.getResourceAsStream(
						"ambit2/db/processors/test/echa_preregistration.echaxml");
		Assert.assertNotNull(in);
		ECHAPreregistrationListReader reader = new ECHAPreregistrationListReader(
				in);
		write(reader, c.getConnection());
		c.close();
	...
	public void testSeekByCID() throws Exception {
		...
		InputStream in = this.getClass().getClassLoader()
				.getResourceAsStream("ambit2/db/processors/cid/712.sdf");
		Assert.assertNotNull(in);
		RawIteratingSDFReader reader = new RawIteratingSDFReader(
				new InputStreamReader(in));
	...
	public void testImportPropertiesBySMILES() throws Exception {
		...
		property_values = c.createQueryTable("EXPECTED",
				"SELECT * FROM property_values where idstructure=100215");
		Assert.assertEquals(2, property_values.getRowCount());

		InputStream in = this.getClass().getClassLoader()
				.getResourceAsStream("ambit2/db/processors/cid/property.csv");
		Assert.assertNotNull(in);
		...
		Assert.assertEquals(5, strucs.getRowCount());
		srcdataset = c.createQueryTable("EXPECTED",
				"SELECT * FROM src_dataset where name='Imported properties'");
		Assert.assertEquals(1, srcdataset.getRowCount());
		struc_src = c
				.createQueryTable(
						"EXPECTED",
	...
	public void testImportPropertiesByKey() throws Exception {
		...
		IRawReader<IStructureRecord> reader = new RawIteratingSDFReader(
				new InputStreamReader(in));
		// reader.setReference("predictions.sdf");
		write((RawIteratingSDFReader) reader, c.getConnection(),
				new PubchemCID(), true);
	...
	public void testImport3Same() throws Exception {
		...
		InputStream in = this.getClass().getClassLoader()
				.getResourceAsStream("ambit2/db/processors/match/99-72-9.sdf");
		Assert.assertNotNull(in);
		RawIteratingSDFReader reader = new RawIteratingSDFReader(
				new InputStreamReader(in));
		reader.setReference(LiteratureEntry.getInstance("input.sdf"));
		...
		in = this.getClass().getClassLoader()
				.getResourceAsStream("ambit2/db/processors/match/99-72-9.csv");
		Assert.assertNotNull(in);
		IIteratingChemObjectReader reader1 = FileInputState.getReader(in,
				"99-72-9.csv");
	...
	public void testZeroLeadingCAS() throws Exception {
		...
		InputStream in = this
				...
				.getClassLoader()
				.getResourceAsStream(
						"ambit2/db/processors/test/weirdcasformatting.sdf");
		Assert.assertNotNull(in);
		RawIteratingSDFReader reader = new RawIteratingSDFReader(
				new InputStreamReader(in));
		reader.setReference(LiteratureEntry.getInstance("input.sdf"));
		...
		ITable table = c.createQueryTable("EXPECTED",
				"SELECT value from property_string where value='1728-95-6'");
	...
	public void testImportByInChI() throws Exception {
		...
		in = this.getClass().getClassLoader()
				.getResourceAsStream("ambit2/db/processors/match/props.csv");
		Assert.assertNotNull(in);
		IIteratingChemObjectReader reader1 = FileInputState.getReader(in,
				".csv");
	...
	public void testImportDX() throws Exception {
		...
		ITable template_def = c.createQueryTable("EXPECTED",
				"SELECT * FROM template_def");
		...
		c = getConnection();
		in = this.getClass().getClassLoader()
				.getResourceAsStream("ambit2/core/data/dx/predictions1.sdf");
		Assert.assertNotNull(in);
		...
		template_def = c.createQueryTable("EXPECTED",
				"SELECT * FROM template_def");
	...
	public void testImportTox21() throws Exception {
		...
		in = this.getClass().getClassLoader()
				.getResourceAsStream("ambit2/core/pubchem/aid720516.csv");
		Assert.assertNotNull(in);
		reader = new RawIteratingWrapper<IIteratingChemObjectReader>(
				FileInputState.getReader(in, ".csv"));
		// reader.setReference(LiteratureEntry.getDXReference());
	...
	public void testWritePDB() throws Exception {
		...
		InputStream in = this.getClass().getClassLoader()
				.getResourceAsStream("ambit2/db/processors/pdb/test.pdb");
		Assert.assertNotNull(in);
		// IIteratingChemObjectReader pdbreader = FileInputState.getReader(in,
		// ".pdb");
		RawIteratingPDBReader reader = new RawIteratingPDBReader(
				new InputStreamReader(in));
		...
		chemicals = c
				.createQueryTable("EXPECTED",
						"SELECT * FROM chemicals where inchi is not null and formula is not null");
		Assert.assertEquals(1, chemicals.getRowCount());
		strucs = c.createQueryTable("EXPECTED", "SELECT * FROM structure");
		Assert.assertEquals(1, strucs.getRowCount());
		srcdataset = c
