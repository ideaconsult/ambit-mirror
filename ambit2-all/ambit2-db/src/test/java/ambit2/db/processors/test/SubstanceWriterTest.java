...
public class SubstanceWriterTest extends DbUnitTest {
	...
	public int write(SubstanceEndpointsBundle bundle,
			IRawReader<IStructureRecord> reader, Connection connection)
			throws Exception {
	...
	public int write(SubstanceEndpointsBundle bundle,
			IRawReader<IStructureRecord> reader, Connection connection,
			PropertyKey key, boolean splitRecord, boolean clearMeasurements,
			boolean clearComposition) throws Exception {
		DBSubstanceWriter writer;
		if (bundle != null)
			writer = new DBBundleStudyWriter(bundle,
					DBSubstanceWriter.datasetMeta(), new SubstanceRecord());
		else
			writer = new DBSubstanceWriter(DBSubstanceWriter.datasetMeta(),
					new SubstanceRecord(), clearMeasurements, clearComposition);
	...
	public SubstanceStudyParser getJSONReader(String json) throws Exception {
		...
		try {
			in = this.getClass().getClassLoader()
					.getResourceAsStream("ambit2/core/data/json/" + json);
	...
	public void testWriteJSONStudies() throws Exception {
		...
		substance = c.createQueryTable("EXPECTED",
				"SELECT * FROM substance_protocolapplication");
		Assert.assertEquals(0, substance.getRowCount());
		try {

			IRawReader<IStructureRecord> parser = getJSONReader("study.json");
			write(null, parser, c.getConnection(),
					new ReferenceSubstanceUUID(), false, false, false);
			parser.close();
	...
	public void testWriteJSONmatrixUpdate() throws Exception {
		...
		try {
			...
			write(bundle, parser, c.getConnection(),
					new ReferenceSubstanceUUID(), false, false, false);
			parser.close();
			...
			substance = c
					.createQueryTable("EXPECTED",
							"SELECT * FROM bundle_substance_experiment where idbundle=1");
			Assert.assertEquals(2, substance.getRowCount());
		} finally {
			c.close();
		}
	...
	public void testWriteJSONSubstanceStudies() throws Exception {
		...
		try {
			...
			write(null, parser, c.getConnection(),
					new ReferenceSubstanceUUID(), false, true, true);
			parser.close();
	...
	public void testDeleteEffects() throws Exception {
		...
		try {
			...
			Assert.assertEquals(1, substance.getRowCount());
			substance = c.createQueryTable("EXPECTED",
					"SELECT * FROM bundle_substance_protocolapplication");
			Assert.assertEquals(1, substance.getRowCount());
			substance = c
					.createQueryTable("EXPECTED",
							"SELECT * FROM bundle_substance_experiment where deleted = 0");
	...
	public int delete_by_writer(SubstanceEndpointsBundle bundle,
			IRawReader<IStructureRecord> reader, Connection connection)
			throws Exception {
	...
	public int delete(SubstanceEndpointsBundle bundle,
			IRawReader<IStructureRecord> reader, Connection connection)
			throws Exception {
		/*
