...
public abstract class AbstractPropertyWriter<Target, Result> extends
	...
	protected abstract void descriptorEntry(Target target, Property property,
	...
	public Result write(Target target) throws SQLException, AmbitException,
		...
		for (Property property : names) {
			...
			if (!found) {
				if ((property.getLabel() == null)
						|| property.getLabel().equals(property.getName())) {
					...
					try {
						...
						for (PropertyAnnotation a : property.getAnnotations()) {
							if (a.getObject() instanceof Property) {
								Property linkedProperty = (Property) a
										.getObject();
								...
								if (linkedProperty.getId() > 0) {
									...
									pa.setObject(String.format("/feature/%d",
											linkedProperty.getId()));
