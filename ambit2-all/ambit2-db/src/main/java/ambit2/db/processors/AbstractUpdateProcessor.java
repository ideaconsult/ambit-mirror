...
public class AbstractUpdateProcessor<Group, Target> extends AbstractRepositoryWriter<Group, Target> {
	...
	switch (getOperation()) {
	case CREATE: {
	    setQueryCreate(query);
	    break;
	}
	case DELETE: {
	    setQueryDelete(query);
	    break;
	}
	case UPDATE: {
	...
	switch (getOperation()) {
	case CREATE:
	    return (getQueryCreate() == null ? null : getQueryCreate().getObject());
	case DELETE:
	    return (getQueryDelete() == null ? null : getQueryDelete().getObject());
	case UPDATE:
	    return (getQueryUpdate() == null ? null : getQueryUpdate().getObject());
	default:
	    return null;
	}
    }
	...
	if (getQueryCreate() != null)
	    getQueryCreate().setObject(target);
	if (getQueryUpdate() != null)
	    getQueryUpdate().setObject(target);
	if (getQueryDelete() != null)
	    getQueryDelete().setObject(target);
    }

    protected Target execute(Group group, IQueryUpdate<Group, Target> query) throws java.sql.SQLException,
	    javax.naming.OperationNotSupportedException, AmbitException {
	if (query == null)
    ...
    public Target create(Group group) throws java.sql.SQLException, javax.naming.OperationNotSupportedException,
	    AmbitException {
	return execute(group, queryCreate);
    };

    public Target delete(Group group) throws java.sql.SQLException, javax.naming.OperationNotSupportedException,
	    AmbitException {
	return execute(group, queryDelete);
    };

    public Target update(Group group) throws java.sql.SQLException, javax.naming.OperationNotSupportedException,
	    AmbitException {
