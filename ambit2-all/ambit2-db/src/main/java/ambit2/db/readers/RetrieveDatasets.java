...
public class RetrieveDatasets extends AbstractQuery<IStructureRecord,ISourceDataset,StringCondition,ISourceDataset>  implements IQueryRetrieval<ISourceDataset>{
    ...
    public static final String select_datasets_bystruc = "SELECT id_srcdataset,name,src_dataset.user_name,idreference,title,url,licenseURI,rightsHolder FROM structure join  struc_dataset using(idstructure) join src_dataset using(id_srcdataset) join catalog_references using(idreference) where %s order by name";
