...
public abstract class AbstractImageReporter<Q extends IQueryRetrieval<IStructureRecord>, OUTPUT> extends
	QueryReporter<IStructureRecord, Q, OUTPUT> {
    /**
	 * 
	 */
	...
	return new File(tmpDir, String.format("%s/%s/%d_%d.%s", dbName, dimensions, item.getIdchemical(),
		item.getIdstructure(), ext));
    }
	...
	try {
	    ...
	    logger.log(
		    Level.WARNING,
		    String.format("Error creating image /compound/%d/conformer/%d\t%s", item.getIdchemical(),
