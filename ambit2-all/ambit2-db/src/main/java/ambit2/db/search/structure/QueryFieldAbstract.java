...
public abstract class QueryFieldAbstract<T,C extends IQueryCondition,NC extends IQueryCondition> extends AbstractStructureQuery<Property,T, C> {
	...
	public final static String sqlField_ci = 
		"select ? as idquery,s1.idchemical,s1.idstructure,if(s1.type_structure='NA',0,1) as selected,s1.preference as metric,null as text\n" +	
		"FROM structure s1\n"+
		"LEFT JOIN structure s2 ON s1.idchemical = s2.idchemical  AND (1E10*s1.preference+s1.idstructure) > (1E10*s2.preference+s2.idstructure)\n"+
		"join (\n"+
	...
	/**
	 * This is faster than the one below, but will work if the summary table exists and for case insensitive searches only
	 */
