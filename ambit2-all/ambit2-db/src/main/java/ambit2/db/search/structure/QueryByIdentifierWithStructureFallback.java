 ...
 * 
 */
public class QueryByIdentifierWithStructureFallback extends
	AbstractStructureQuery<IStructureKey, IStructureRecord, StringCondition> implements IStoredProcStatement {
    private enum storedproc_search_mode {
	name, alias, idproperty, any, inchi, inchikey
    }
    ...
    /*
     * IN chemical_id INT, //if >0 only retrieves the preferred structure IN
     * search_mode INT, -- 0 : name ; 1 : alias; 2: idproperty ; 3: any ; 4:
     * inchi ; 5: inchikey IN property_name VARCHAR(255), IN property_alias
     * VARCHAR(255), IN property_id INT, IN query_value TEXT, -- string value or
     * inchi IN query_value_num DOUBLE, IN query_inchi TEXT, IN maxrows INT) *
     * call findByProperty( 0, 3, "http://www.opentox.org/api/1.1#CASRN","",0,
     * "testid",0,
	...
	Property key = getFieldname() == null ? null
		: getFieldname().getQueryKey() instanceof Property ? (Property) getFieldname().getQueryKey() : null;
	...
	params.add(new QueryParam<Double>(Double.class, identifier instanceof Number ? ((Number) identifier)
		.doubleValue() : null));
