...
public class ReadSubstanceStudy<PA extends ProtocolApplication<Protocol, String, String, IParams, String>> extends
	AbstractQuery<String, PA, EQCondition, PA> implements IQueryRetrieval<PA> {
	...
	try {
		...
		record.setSubstanceUUID(rs.getString("substance_prefix") + "-"
			+ I5Utils.addDashes(rs.getString("su").toString().toLowerCase()));
	    } catch (Exception xx) {
		...
		record.setCompanyUUID(rs.getString("owner_prefix") + "-"
			+ I5Utils.addDashes(rs.getString("ou").toString().toLowerCase()));
	    } catch (Exception xx) {
		...
		record.setReferenceSubstanceUUID(rs.getString("rs_prefix") + "-"
			+ I5Utils.addDashes(rs.getString("rsu").toString().toLowerCase()));
	    } catch (Exception xx) {
