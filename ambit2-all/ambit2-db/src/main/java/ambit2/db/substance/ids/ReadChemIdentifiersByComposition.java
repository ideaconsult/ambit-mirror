...
public class ReadChemIdentifiersByComposition extends AbstractQuery<SubstanceRecord,IStructureRecord,SetCondition,IStructureRecord> 
	...
	"select idchemical,idstructure,comments,if(status='TRUNCATED',text,value),value_num,title,url\n"+
		...
		if (getFieldname() != null && getFieldname().getRelatedStructures() != null)
			for (CompositionRelation r : getFieldname().getRelatedStructures()) 
				if (r.getSecondStructure()!=null && r.getSecondStructure().getIdchemical()>0) {
					if (params==null) {params = new StringBuilder();params.append("?");}
					else params.append(",?");
				}
		if (params==null) throw new AmbitException("No idchemical");
	...
	public List<QueryParam> getParameters() throws AmbitException {
		...
		if (getFieldname() != null && getFieldname().getRelatedStructures() != null)
			for (CompositionRelation r : getFieldname().getRelatedStructures()) 
				if (r.getSecondStructure()!=null && r.getSecondStructure().getIdchemical()>0) {
					if (params==null) params = new ArrayList<QueryParam>();
