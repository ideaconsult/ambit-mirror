...
public class ReadSubstanceComposition
		extends
		AbstractQuery<SubstanceRecord, CompositionRelation, EQCondition, CompositionRelation>
		implements IQueryRetrieval<CompositionRelation> {

	/**
	 * 
	 */
	private static final long serialVersionUID = -1980335091441168568L;
	protected CompositionRelation record = new CompositionRelation(
			new SubstanceRecord(), new StructureRecord(), new Proportion());
	...
	public List<QueryParam> getParameters() throws AmbitException {
		...
		if (getFieldname() != null) {
			if (getFieldname().getIdsubstance() > 0) {
				...
				if (bundle != null && bundle.getID() > 0)
					params.add(new QueryParam<Integer>(Integer.class, bundle
							.getID()));

				params.add(new QueryParam<Integer>(Integer.class,
						getFieldname().getIdsubstance()));
				...
				String[] uuid = new String[] { null,
						o_uuid == null ? null : o_uuid.toString() };
				if (o_uuid != null)
	...
	public CompositionRelation getObject(ResultSet rs) throws AmbitException {
		if (record == null)
			record = new CompositionRelation(new SubstanceRecord(),
					new StructureRecord(), new Proportion());
		else
			record.clear();
		return readCompositionRelation(record, rs, bundle);
	}

	public static CompositionRelation readCompositionRelation(
			CompositionRelation record, ResultSet rs,
			SubstanceEndpointsBundle bundle) throws AmbitException {
		if (record == null)
			record = new CompositionRelation(new SubstanceRecord(),
					new StructureRecord(), new Proportion());
		else
		...
		try {
			try {
				String uuid = I5Utils.getPrefixedUUID(
						rs.getString("cmp_prefix"), rs.getString("cmp_huuid"));
			...
			record.getRelation().setTypical_unit(
					rs.getString("proportion_typical_unit"));
			record.getRelation().setTypical_value(
					rs.getDouble("proportion_typical_value"));
			record.getRelation().setReal_lower(
					rs.getString("proportion_real_lower"));
			record.getRelation().setReal_upper(
					rs.getString("proportion_real_upper"));
			record.getRelation().setReal_unit(
					rs.getString("proportion_real_unit"));
			record.getRelation().setReal_lowervalue(
			...
			try {
				String uuid = I5Utils.getPrefixedUUID(rs.getString("refstruc"),
						rs.getString("refstrucuuid"));
