...
public class CreateBundle extends AbstractObjectUpdate<SubstanceEndpointsBundle> {
	...
	public static final String[] create_sql = {
		...
		"INSERT IGNORE INTO bundle (idbundle, name,user_name,idreference,licenseURI,rightsHolder,maintainer,description,created,bundle_number,version) SELECT ?,?,?,idreference,?,?,?,?,now(),unhex(replace(?,'-','')),? FROM catalog_references WHERE title=?"
	};
	...
	public List<QueryParam> getParameters(int index) throws AmbitException {
		...
		case 1: {
			...
			params2.add(new QueryParam<Integer>(Integer.class, getObject().getVersion()));			
