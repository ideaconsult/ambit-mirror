...
public class SubstanceProperty extends Property {
	...
	public SubstanceProperty(String topcategory, String endpointCategory,
			String name, String reference) {
		this(topcategory, endpointCategory, name, null, LiteratureEntry
				.getInstance(reference, ""));
	}

	public SubstanceProperty(String topcategory, String endpointCategory,
			String name, String units, ILiteratureEntry reference) {
		...
		setLabel(String.format("http://www.opentox.org/echaEndpoints.owl#%s",
				endpointCategory == null ? "UNKNOWN_TOXICITY"
						: endpointcategory.replace("_SECTION", "")));
	}
			...
			return String
					.format("/property/%s/%s%s%s/%s%s%s",
							...
							identifier == null ? UUID.nameUUIDFromBytes(
									...
									: identifier,
							extendedURI ? "/" : "",
							extendedURI ? URLEncoder.encode(UUID
									.nameUUIDFromBytes(
											reference.getTitle().getBytes())
									.toString()) : "");
	...
	@Override
	public int hashCode() {
		return Objects.hashCode(topcategory, endpointcategory, getTitle(),
				getName());
	}
