...
public class Property implements Serializable,
	...
	public void setName(String name) {
		this.name = name == null ? null : name.length() >= 256 ? name
				.substring(0, 255) : name;
	}
	...
	public static synchronized Property getTradeNameInstance(String synonym) {
		Property p = getInstance(synonym,
				LiteratureEntry.getTradeNameReference());
		p.setLabel(opentox_TradeName);
		return p;
	}

	public static synchronized Property getI5UUIDInstance() {
		Property p = getInstance(IUCLID5_UUID,
	...
	public static synchronized Property getInChIStdInstance() {
		return Property.getInstance(Property.opentox_InChI_std,
				new LiteratureEntry(Property.opentox_InChI_std,
						"http://www.iupac.org/inchi/"));
	}
	...
	public static synchronized Property getInstance(String name,
		...
		Property p = new Property(name, LiteratureEntry.getInstance(reference,
				url));
		p.setLabel(name);
		p.setEnabled(true);
	...
	public int hashCode() {
		try {
			...
			var_code = getReference() == null ? 0 : (null == getReference()
					.getTitle() ? 0 : getReference().getTitle().hashCode());
			hash = 31 * hash + var_code;
	...
	public boolean equals(Object obj) {
		if (obj instanceof Property) {
			if (((Property) obj).getReference() == null)
				...
				return ((Property) obj).getName().equals(getName())
						&& ((Property) obj).getReference().equals(
								getReference());
		} else
			return false;
	}
	...
	public void assign(Property newProperty) {
		...
		this.reference = new LiteratureEntry(newProperty.getTitle(),
				newProperty.getUrl());
		...
		if (getId() > 0)
			...
			try {
				return String.format("/feature/%s",
						URLEncoder.encode(getName() + getTitle(), "UTF-8"));
			} catch (Exception x) {
				return String
						.format("/feature/%s",
								UUID.nameUUIDFromBytes(
										(getName() + getTitle()).getBytes())
										.toString());
			}
	}
}
