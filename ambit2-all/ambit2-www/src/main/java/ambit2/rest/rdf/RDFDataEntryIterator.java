...
public abstract class RDFDataEntryIterator<Item,Feature> extends RDFObjectIterator<Item> {
	...
	protected Template createTemplate() {
		...
		return new Template(String.format("%s%s",baseReference==null?"":baseReference,CompoundResource.compoundID));
	}
	...
	protected void parseObjectURI(RDFNode uri, Item record) {
		...
		try { 
			...
			try {
			idchemical = Integer.parseInt(vars.get(CompoundResource.idcompound).toString()); } 
			catch (Exception x) {};
		
			try { idstructure = Integer.parseInt(vars.get(ConformerResource.idconformer).toString()); } 
			catch (Exception x) {};
		} catch (Exception x) {};
