 ...
 * 
 */
public class WafflesModelBuilder extends
		ModelBuilder<File, Algorithm, ModelQueryResults> {
	...
	public WafflesModelBuilder(Reference applicationRootReference,
			ModelURIReporter model_reporter, AlgorithmURIReporter alg_reporter,
			String[] targetURI, String[] parameters, String referer) {
	...
	public ModelQueryResults process(Algorithm algorithm) throws AmbitException {
		...
		try {
			Class clazz = this.getClass().getClassLoader()
					.loadClass(algorithm.getContent().toString());
		...
		in.put(WafflesLearnOption.command.name(),
				WafflesLearnCommand.train.name());
		...
		in.put(WafflesLearnOption.algorithm.name(), waffles.getAlgorithm()
				.name());
		if (dataOptions != null)
		...
		try {
			...
			if ((out == null)
					|| (out.getProperty(waffles.getOutProperty()) == null))
				throw new AmbitException(String.format("Model not generated",
						waffles.getOutProperty()));
			if (!"0".equals(out.getProperty(ShellWafflesLearn
					.getExitcodeProperty())))
				throw new AmbitException(String.format(
						"Model not generated [exit code = %s] %s", out
								.getProperty(ShellWafflesLearn
										.getExitcodeProperty()), out
								.getProperty(waffles.getOutProperty())));

			String name = String.format("%s.%s.%s", waffles.getAlgorithm()
					.name(), UUID.randomUUID().toString(), waffles.getClass()
					.getName());
			...
			LiteratureEntry entry = new LiteratureEntry(name,
					algorithm == null ? waffles.getClass().getName()
							: r.getURI(applicationRootReference.toString(),
									algorithm));

			LiteratureEntry prediction = new LiteratureEntry(model.getName(),
					model_reporter.getURI(applicationRootReference.toString(),
							model));
			...
			for (int i = 0; i < header.numAttributes(); i++) {
				...
				for (String t : getTargetURI())
					if (header.attribute(i).name().equals(t)) {
						...
						labels.append(i);
						// dependent property
						Property property = createPropertyFromReference(
								new Reference(header.attribute(i).name()),
								entry,referer);
						property.setOrder(i + 1);
						dependent.add(property);
						// predicted property
						Property predictedProperty = new Property(
								property.getName(), prediction);
						...
						if (header.attribute(i).isNominal()) {
							Enumeration e = header.attribute(i)
									.enumerateValues();
				...
				if (!isTarget) {
					Property property = createPropertyFromReference(
							new Reference(header.attribute(i).name()), entry,referer);
					property.setOrder(i + 1);
					predictors.add(property);
				}
			}

			model.setContent(serializeModel(
					out.getProperty(waffles.getOutProperty()), header));
	...
	protected String serializeModel(String json, Instances newInstances)
			throws IOException {
		// serialize model
		Form form = new Form();
