 ...
 * 
 * @author Nina Jeliazkova nina@acad.bg <b>Modified</b> 2006-3-5
 */
public class CompoundImageTools implements IStructureDiagramHighlights,
		ICompoundImageTools {
	protected static Logger logger = Logger.getLogger(CompoundImageTools.class
			.getName());
	...
	public enum Mode2D {
		...
		kekule {
			...
			public String getDescription() {
				return String.format("SMILES parser with %s\n%s",
						SilentChemObjectBuilder.class.getName(), toString());
			}
		},
		...
		aromatic {
			...
			@Override
			public String getDescription() {
				return String.format("SMILES parser with %s %s",
						SilentChemObjectBuilder.class.getName(),
	...
	/**
	 * Creates gradient border of this color. Set the same as background color
	 * if you don't want shadowing effect.
	 * 
	 * @param borderColor
	 */
...
*/
	...
	/*
	 * private IRenderer createRenderer(Dimension cellSize,Color
	 * background,boolean rings, boolean atomNumbers) {
	 * List<IGenerator<IAtomContainer>> generators = new
	 * ArrayList<IGenerator<IAtomContainer>>(); generators.add(new
	 * BasicSceneGenerator());
	 * 
	 * generators.add(new BasicBondGenerator()); if (rings) generators.add(new
	 * RingGenerator()); generators.add(new MyBasicAtomGenerator());
	 * generators.add(new MySelectAtomGenerator()); generators.add(new
	 * BasicAtomGenerator()); if (atomNumbers) generators.add(new
	 * AtomNumberGenerator());
	 * 
	 * generators.add(new SelectBondGenerator());
	 * 
	 ...
	 * 
	 * 
	 * IRenderer renderer = new AtomContainerRenderer(generators, new
	 * AWTFontManager()) ; RendererModel r2dm = renderer.getRenderer2DModel();
	 * 
	 * RendererModelWrapper.setCompactShape(r2dm,BasicAtomGenerator.Shape.OVAL);
	 * 
	 * if (atomNumbers) RendererModelWrapper.setDrawNumbers(r2dm,atomNumbers);
	 * 
	 * RendererModelWrapper.setUseAntiAliasing(r2dm,true);
	 * 
	 * RendererModelWrapper.setBackColor(r2dm,new
	 * Color(background.getRed(),background.getGreen(),background.getBlue(),0));
	 * 
	 * RendererModelWrapper.setShowExplicitHydrogens(r2dm,false);
	 * 
	 * RendererModelWrapper.setShowAromaticity(r2dm,true); return renderer; }
	 */
	...
	public synchronized BufferedImage generateImage(String value,
		...
		if (value.startsWith(AmbitCONSTANTS.INCHI)) {
			...
			if ((c == null) || (c.getAtomContainer() == null)
				...
				throw new CDKException(String.format("%s %s %s",
						c.getReturnStatus(), c.getMessage(), c.getLog()));
			return getImage(c.getAtomContainer(), selector, build2d,
					atomNumbers, false, mode2D);
		} else {
			if (parser == null)
			...
			return molecule == null ? null : getImage(molecule, selector,
					build2d, atomNumbers, false, mode2D);
		}
	}
			...
			if (molecule != null)
				for (int i = 0; i < molecule.getBondCount(); i++)
					if (molecule.getBond(i).getFlag(CDKConstants.ISAROMATIC))
						if (IBond.Order.DOUBLE.equals(molecule.getBond(i)
								.getOrder())) {
							rings = false;
							break;
						}
	...
	protected BufferedImage createBuffer() {
		return new BufferedImage(imageSize.width, imageSize.height,
	...
	private void paintBorderShadow(Graphics2D g2, int shadowWidth, Shape shape) {
		...
		for (int i = sw; i >= 2; i -= 2) {
			...
			g2.setColor(getMixedColor(borderColor, pct, background, 1.0f - pct));
	...
	public synchronized void generate2D(IAtomContainer molecule,
			boolean generateCoordinates, IAtomContainerSet molecules) {
		if (molecule != null) {
			...
			if (!generateCoordinates) {
				// IAtomContainer c =
				// AtomContainerManipulator.removeHydrogensPreserveMultiplyBonded(molecule);
				// molecules.addAtomContainer(c);
				// there still could be multiple parts !
				IAtomContainerSet mset = ConnectivityChecker
			...
			try {
				...
				for (int i = 0; i < mset.getAtomContainerCount(); i++) {
					IAtomContainer c = AtomContainerManipulator
							.removeHydrogensPreserveMultiplyBonded(mset
									.getAtomContainer(i));
					if (generateCoordinates) {
						...
						m = (IAtomContainer) c;// mset.getMolecule(i);
					}
	...
	public synchronized void paint(IRenderer renderer,
		...
		if ((molecules != null) && (molecules.getAtomContainerCount() > 0)) {
			...
			if (molecules.getAtomContainerCount() > 1) {
				...
				all = MoleculeTools.newMolecule(SilentChemObjectBuilder
						.getInstance());
				...
				for (IAtomContainer m : molecules.atomContainers()) {
					...
					box.setRect(
							...
							box.getY(),
							1 + box.getWidth()
									+ (r.getWidth() == 0 ? 1.5 : r.getWidth()),
							r.getHeight() > box.getHeight() ? r.getHeight()
									: box.getHeight());
				...
				for (IAtomContainer m : molecules.atomContainers()) {
					...
					b.setRect(
							...
							1 + b.getWidth()
									+ (r.getWidth() == 0 ? 1.5 : r.getWidth()),
							r.getHeight() > b.getHeight() ? r.getHeight() : b
									.getHeight());
				...
				all = molecules.getAtomContainer(0);

			Rectangle drawArea = new Rectangle(offset, offset, imageSize.width,
					imageSize.height);
					...
					protected void imageMap(ImageMapAreaElement atomSymbol,
						...
						if ((imageMap != null)) {
							...
							imageMap.append(String.format(formatJson,
									atomSymbol.text, atomSymbol.text, x, y, w,
									h));
							comma = ",";
						}
					}
...
/**
 * Couldn't find how to highlight atoms with filled -in ovals, so this is my
 * workaround
 * 
 * @author nina
 * 
...
class MyBasicAtomGenerator extends BasicAtomGenerator {
	public IRenderingElement generate(IAtomContainer ac, RendererModel model) {
		...
		for (IAtom atom : ac.atoms()) {
			...
			if (p == null)
				continue;
			double r = RendererModelWrapper.getAtomRadius(model)
					/ RendererModelWrapper.getScale(model);
			double d = 2 * r;
			if (RendererModelWrapper.getCompactShape(model) == Shape.SQUARE) {
				elementGroup.add(new RectangleElement(p.x - r, p.y - r, d, d,
						true, CompoundImageTools.whiteTransparent));
			} else {
				elementGroup.add(new OvalElement(p.x, p.y, r, true,
						CompoundImageTools.whiteTransparent));
			}
		}
		return elementGroup;
	}
...
/**
 * Atom selection size and colors are taken from properties, which might be set
 * by different calculation procedures. Currently used to visualize SOM
 * (SmartCYP and SOME) Web services use
 * http://host/compound/{id}?model_uri=http://host/model/{id}
 * http://apps.ideaconsult
 * .net:8080/ambit2/compound/100?model_uri=http://apps.ideaconsult
 * .net:8080/ambit2/model/48&w=400&h=400 where colors are defined by the model
 * itself, e.g. http://apps.ideaconsult.net:8080/ambit2/model/48?media=image/png
 * 
 * @author nina
 * 
 */
class MySelectAtomGenerator extends SelectAtomGenerator {
	...
	public IRenderingElement generate(IAtomContainer arg0, RendererModel model) {
		...
		BasicAtomGenerator.Shape shape = RendererModelWrapper
				.getSelectionShape(model);
		IChemObjectSelection selection = model.getSelection();

		if (selection == null)
			return selectionElements;
		if (this.autoUpdateSelection || selection.isFilled()) {
			double r = RendererModelWrapper.getSelectionRadius(model)
					/ RendererModelWrapper.getScale(model);
			...
			if (selectedAC != null) {
				for (IAtom atom : selectedAC.atoms()) {
					...
					double m = 1;
					Object size = atom
							.getProperty(CompoundImageTools.SELECTED_ATOM_SIZE);
					if (size != null)
						try {
					...
					Object clr = atom
							.getProperty(CompoundImageTools.SELECTED_ATOM_COLOR);
					if ((clr != null) && (clr instanceof Color)) {
						Color c = (Color) clr;
						atomColor = new Color(c.getRed(), c.getGreen(),
								c.getBlue(), 128); // semitransparent
					}
					...
					switch (shape) {
					case SQUARE:
						element = new RectangleElement(p.x - r, p.y - r, d, d,
								true, atomColor);
						break;
					case OVAL:
					default:
						element = new OvalElement(p.x, p.y, d * m, true,
								atomColor);
					}
