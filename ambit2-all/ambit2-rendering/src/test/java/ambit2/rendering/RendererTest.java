...
public class RendererTest {
	...
	public void testImage() throws Exception {
		...
		IAtomContainerHighlights p = new IAtomContainerHighlights() {
			...
			public IChemObjectSelection process(IAtomContainer mol) throws AmbitException {
				...
				final IAtomContainer selectedMol = MoleculeTools.newMolecule(SilentChemObjectBuilder.getInstance());
	...
	public void testsdf2image(String filename, String imgfile) throws Exception {
		try (InputStream in = RendererTest.class.getClassLoader().getResourceAsStream(filename)) {
			...
			while (reader.hasNext()) {
				...
				/**
				 * if generating 2D coordinates via StructureDiagramGenerator, the problem goes
				 * away, but it is still valid use case to generate a structure diagram from
				 * coordinates retrieved from a file
				 */
	...
	public void testListParams() {
		...
		for (Object generator : renderer.getGenerators()) {
			...
			for (Object parameter : g.getParameters()) {
				System.out.println("parameter: " + parameter.getClass().getName().substring(40) + " -> "
	...
	public void testFullerene() throws Exception {
		...
		Logger.getGlobal().log(java.util.logging.Level.FINEST,
				String.format("%d %d %d", aMatrix.length, aMatrix.length, count));
