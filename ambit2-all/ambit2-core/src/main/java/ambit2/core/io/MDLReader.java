...
/**
 * Reads a molecule from the original MDL MOL or SDF file {@cdk.cite DAL92}. An
 * SD files is read into a {@link IChemSequence} of {@link IChemModel}'s. Each
 * ChemModel will contain one Molecule. If the MDL molfile contains a property
 * block, the {@link MDLV2000Reader} should be used.
 * 
 * <p>
...
public class MDLReader extends DefaultChemObjectReader {
	...
	private static ILoggingTool logger = LoggingToolFactory
			.createLoggingTool(MDLReader.class);
	...
	public IChemObject read(IChemObject object) throws CDKException {
		if (object instanceof IChemFile) {
			...
			return readMolecule((IAtomContainer) object);
		} else {
	...
	private IChemModel readChemModel(IChemModel chemModel) throws CDKException {
		...
		if (setOfMolecules == null) {
			setOfMolecules = MoleculeTools.newMoleculeSet(chemModel
					.getBuilder());
		}
		IAtomContainer m = readMolecule(MoleculeTools.newMolecule(chemModel
				.getBuilder()));
		if (m != null) {
	...
	private IChemFile readChemFile(IChemFile chemFile) throws CDKException {
		IChemSequence chemSequence = MoleculeTools.newChemSequence(chemFile
				.getBuilder());

		IChemModel chemModel = MoleculeTools
				.newChemModel(chemFile.getBuilder());
		IAtomContainerSet setOfMolecules = MoleculeTools
				.newMoleculeSet(chemFile.getBuilder());
		IAtomContainer m = readMolecule(MoleculeTools.newMolecule(chemFile
				.getBuilder()));
		if (m != null) {
			setOfMolecules.addAtomContainer(m);
		}
		...
		setOfMolecules = MoleculeTools.newMoleculeSet(chemFile.getBuilder());
		chemModel = MoleculeTools.newChemModel(chemFile.getBuilder());
		String str;
		try {
			String line;
			while ((line = input.readLine()) != null) {
				...
				str = new String(line);
				if (str.equals("$$$$")) {
					m = readMolecule(MoleculeTools.newMolecule(chemFile
							.getBuilder()));
					...
					if (m != null) {
						...
						setOfMolecules = MoleculeTools.newMoleculeSet(chemFile
								.getBuilder());
						chemModel = MoleculeTools.newChemModel(chemFile
								.getBuilder());
					...
					if (m != null) {
						...
						while ((line = input.readLine()) != null
							...
							// chars; in that case it
							// is assumed to continue on the next line. See MDL
							// documentation.
							if (line.length() < 80)
		...
		try {
			...
			if (mode == Mode.STRICT) {
				if (line.contains("V2000") || line.contains("v2000")) {
					throw new CDKException(
							"This file must be read with the MDLV2000Reader.");
				}
				if (line.contains("V3000") || line.contains("v3000")) {
					throw new CDKException(
							"This file must be read with the MDLV3000Reader.");
				}
			}
			...
			for (int f = 0; f < atoms; f++) {
				...
				if (isotopeFactory.isElement(element)) {
					atom = isotopeFactory.configure(MoleculeTools.newAtom(
							molecule.getBuilder(), element));
				} else if ("A".equals(element)) {
					atom = MoleculeTools.newPseudoAtom(molecule.getBuilder(),
							element);
					...
					if (rGroup.length > 1) {
						try {
							Rnumber = Integer
									.parseInt(rGroup[(rGroup.length - 1)]);
				...
				if (!(atom instanceof IPseudoAtom)) {
					try {
						...
						if (massDiff != 0) {
							...
							atom.setAtomicNumber(major.getAtomicNumber()
									+ massDiff);
						}
					} catch (Exception exception) {
			...
			for (int f = 0; f < bonds; f++) {
				line = input.readLine();
				linecount++;
				atom1 = java.lang.Integer.valueOf(line.substring(0, 3).trim())
						.intValue();
				atom2 = java.lang.Integer.valueOf(line.substring(3, 6).trim())
						.intValue();
				order = java.lang.Integer.valueOf(line.substring(6, 9).trim())
						.intValue();
				if (line.length() > 12) {
					int mdlStereo = Integer.valueOf(line.substring(9, 12)
							.trim());
					if (mdlStereo == 1) {
						// MDL up bond
					...
					logger.warn("Missing expected stereo field at line: "
							+ line);
				}
				...
				IBond newBond = null;
				if (order == 1) {
					newBond = MoleculeTools.newBond(molecule.getBuilder(), a1,
							a2, IBond.Order.SINGLE, stereo);
				} else if (order == 2) {
					newBond = MoleculeTools.newBond(molecule.getBuilder(), a1,
							a2, IBond.Order.DOUBLE, stereo);
				} else if (order == 3) {
					newBond = MoleculeTools.newBond(molecule.getBuilder(), a1,
							a2, IBond.Order.TRIPLE, stereo);
				} else if (order == 4) {
					// aromatic bond
					newBond = MoleculeTools.newBond(molecule.getBuilder(), a1,
							a2, IBond.Order.SINGLE, stereo);
