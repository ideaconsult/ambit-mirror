 ...
 * 0002-Because-the-DeduceBondSystemTool-does-not-consider-r.patch
...
public class DeduceBondSystemTool {
	protected long timeout = 100000; //100 sec
...
//		logger.debug("number= "+number);			
        ...
        if (MasterList.size() > 0) {
            IMolecule iMolecule = loop(System.currentTimeMillis(), molecule, 0, MasterList, choices, som,0);
...
//						logger.debug(mol.getBondOrderSum(a));
    ...
    private IMolecule loop(long starttime, IMolecule molecule, int index, 
    		               List MasterList, int [] choices, IMoleculeSet som,int level) throws CDKException {

        //logger.debug(System.currentTimeMillis());
    	//System.out.println(String.format("[Level %d] %s",level,index));
        ...
        if (diff > timeout) { 
        	...
        	throw new CDKException(String.format("Timed out after %s ms.",timeout));
